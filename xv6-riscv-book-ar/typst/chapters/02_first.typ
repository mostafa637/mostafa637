...
عندما تحاول عَمَلِيَّةٌ تعمل في وضع المستخدم تنفيذ تعليمة ممتازة (مثل تعديل سجل التحكم بالتصفح `satp`)، يرفض العتاد التعليمة ولا ينفذها ويولّد *استثناء* (Exception) يُحاط به فورًا وتنتقل السيطرة إلى معالج النواة.
...
عند تنفيذ `ecall`:
  ...
  + تُنَفِّذُ النواة الخدمة المطلوبة وتتحقق من صحة المدخلات.
...
وتُقسم إلى وحدات برمجية واضحة:
  ...
  + `kernel/trap.c`: معالجة المَصَايِدُ، الاستثناءات، المُقَاطَعَاتُ العتادية، واستدعاءات النظام.
...
يحتفظ هذا الهيكل بجميع معلومات حالة العملية:
  ...
  + إطار المِصْيَدَة (`struct trapframe *trapframe`) للحفظ والاسترجاع لسجلات المستخدم.
  + سياق العملية (`struct context context`) لحفظ سجلات النواة أثناء تبديل المُجَدْوِل.
  + قائمة الملفات المفتوحة (`struct file *ofile[NOFILE]`).
  + الدليل الحالي للعملية (`struct inode *cwd`).


لكل عَمَلِيَّةٌ مكدسان اثنان: مكدس المستخدم (User Stack) ومكدس النواة (Kernel Stack).
عندما تُنَفِّذُ العملية تعليمات في فضاء المستخدم، يُستخدم مكدس المستخدم.
...
عند تشغيل الحاسوب (أو محاكي QEMU):
  ...
  + تقوم النواة بتهيئة الأقفال، ووحدة إدارة الذاكرة، ومجمع المُقَاطَعَاتُ PLIC، والذاكرة المخبئية.
  + تدعو `main()` الدالة `userinit()` لإنشاء أول عَمَلِيَّةٌ مستخدم.
  + تُنَفِّذُ العملية الأولى كوداً مجمعاً صغيراً بداخل `initcode.S` يتضمن استدعاء النظام `exec("/init")` ليعمل برنامج البدء الرسمي الشارح لمغلف الأوامر `sh`.
