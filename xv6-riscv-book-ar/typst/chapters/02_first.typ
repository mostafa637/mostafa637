...
في هذا الفصل،
نبين كيف ينظم `xv6` نظام التشغيل حول هذا الهدف،
...
عندما تحاول عملية تعمل في وضع المستخدم تنفيذ تعليمة ممتازة (مثل تعديل سجل التحكم بالتصفح `satp`)،
...
يوفر عتاد RISC-V تعليمة خاصة تُدعى `ecall` (Environment Call).
عند تنفيذ `ecall`:
...
يفترض نموذج الأمان في `xv6` أن النواة موثوقة تمامًا ومحصنة ضد الثغرات البرمجية،
...
ومع ذلك،
تشترك جميع هذه الأنظمة مع `xv6` في الأعمدة الثنائية ذاتها:
العزل العتادي بين وضع المستخدم ووضع المشرف،
واستخدام استدعاءات النظام المحكومة عبر التعليمة `ecall`.
