...
يقدم هذا الفصل كيفية تبديل السياق في #lstinline("xv6")،
وهيكلة دالة المجدول (Scheduler)، وحالات العملية،
والتفاعل بين المجدول وأقفال العمليات.

== تعدد المشاركة
<sec:multiplexing>

تحقق النواة التعددية المشاركة عبر فصل المعالجة الفيزيائية عن حالة العملية الافتراضية.
عندما تنتهي شريحة الوقت للعملية الحالية أو تتوقف منتظرة الإدخال والإخراج،
تحفظ النواة سياق العملية وترتقي بعملية أخرى لتستغل المعالج.

== نظرة عامة على تبديل السياق
<sec:context_switch_overview>

يتطلب تحويل المعالج من عملية أولى إلى عملية ثانية حفظ حالة السجلات العتادية الخاصة بالعملية الأولى لاسترجاعها لاحقًا،
وتحميل السجلات المحفوظة الخاصة بالعملية الثانية.

في #lstinline("xv6")،
لا يتم التبديل المباشر بين عملية ومستقلة،
بل يتم التبديل بين:
...
عندما ترغب عملية بالإنهاء أو التخلي عن وحدة المعالجة (مثل دعوة #lstinline("yield()") أو الدخول في النوم عبر #lstinline("sleep()"))،
تقوم بدعوة #lstinline("sched()"):
تتحقق #lstinline("sched()") من شروط الأمان (حيازة #lstinline("p->lock")،
تعطيل المقاطعات التعددية)،
ثم تنفذ #lstinline("swtch(\&p->context, \&c->context)") للعودة فورًا إلى حلقة #lstinline("scheduler()") الخاصة بالمعالج.

== الكود البرمجي: mycpu و myproc
<sec:code_mycpu_myproc>

تسترجع النواة معرّف وحدة المعالجة الحالية عبر السجل العتادي #lstinline("tp") (Thread Pointer)،
حيث تعيد الدالة #lstinline("mycpu()") مؤشر هيكل المعالج الحالي #lstinline("struct cpu")،
بينما تعيد الدالة #lstinline("myproc()") مؤشر العملية المربوطة بالمعالج الحالية.

== العالم الحقيقي
<sec:real_world_sched>

يستخدم #lstinline("xv6") المجدول بالدوران العام (Round-Robin Scheduler) لسهولة فهمه وبساطته.
بينما تستخدم أنظمة التشغيل الحديثة مجدولات متعددة المستويات ومجهزة بالأولويات المقسمة،
مثل مجدول العدالة الكاملة (Completely Fair Scheduler - CFS) في Linux،
ومجدولات التغذية الراجعة متعددة المستويات (MLFQ).
