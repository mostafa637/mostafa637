...
#import "listings-languages.typ": listings-language, listings-language-aliases, listings-driver-names
#import "fix-bidi.typ": process-bidi, bidi-direction, fix-bidi-content
...
#let listings-defaults = (
  ...
  continuation-symbol: "↪",
continuation-style: (font: "DejaVu Sans Mono", size: 7pt, fill:
rgb("68727e")),
...
if chosen != none { chosen } else { _get(config, "language", default:
none) }
}

#let _canonical-language(name) = listings-language(name)
...
let aliases = if type(definition) == dictionary { definition.at("aliases", default:
()) } else { () }
...
let keywords = _custom-list(definition.at("keywords", default:
())) + _custom-list(dialect.at("keywords", default: ()))
let comments = _custom-list(definition.at("comments", default:
())) + _custom-list(dialect.at("comments", default: ()))
let strings = _custom-list(definition.at("strings", default:
())) + _custom-list(dialect.at("strings", default: ()))
...
let frame = if type(configured-shape) == dictionary { configured-shape } else { _get(options, "frame", default:
"none") }
...
let inherited = if _get(options, "breakautoindent", default:
true) { chars.slice(0, leading).join() } else { "" }
...
if type(requested) != int or requested <= 0 or not _get(options, "breaklines", default:
true) {
    lines
  } else {
    let wrapped = ()
...
let lines = _split-lines(formatted, keep-final:
_get(options, "showlines", default: false))
...
let result = if _get(options, "keepspaces", default:
true) { line } else { _collapse-source-spaces(line) }
...
result = result.replace("\t", str(tab-marker) + " " * _get(options, "tabsize", default:
8))
  } else {
...
(start: calc.min(lines.len(), a), finish:
calc.max(calc.min(lines.len(), b), calc.min(lines.len(), a)))
  }
}
...
(lines: selected, offset: a, source-indices: source-indices, range-starts:
(0,))
  } else {
...
(start: calc.max(0, pair.at(0) - 1), finish:
calc.min(lines.len(), pair.at(1)))
...
(lines: result, offset: offset, source-indices:
source-indices, range-starts: range-starts)
  }
}
...
let logical = _line-number-value(index, options, offset:
offset, source-index: source-index)
if numbers == none or numbers == "none" or type(step) != int or step <= 0 { false }
else if line == "" and not _get(options, "numberblanklines", default:
true) { false }
else if index == 0 and _get(options, "numberfirstline", default:
false) { true }
...
let configured = if kind == "keyword" { _get(options, "keywordstyle", default:
none) }
...
let shown-token = if kind == "string" and _get(options, "showstringspaces", default:
true) and not _get(options, "showspaces", default: false) {
    token.replace(" ", "␠")
  } else { token }
...
if j + closer-chars.len() <= chars.len() and chars.slice(j, j + closer-chars.len()) == closer-chars {
      ...
      i = j
} else if (c == "\"" and _arabic-extra-prefix-at(chars, i, deleted-strings) != "\"") or (c == "'" and _arabic-extra-prefix-at(chars, i, deleted-strings) != "'") or (_arabic-extra-prefix-at(chars, i, _get(options, "string", default:
()) + _get(options, "morestring", default:
())) != none and _arabic-extra-prefix-at(chars, i, deleted-strings) == none) {
      if plain != "" { pieces.push(plain); plain = "" }
let extra-string = _arabic-extra-prefix-at(chars, i, _get(options, "string", default:
()) + _get(options, "morestring", default: ()))
...
pieces.push(_arabic-token-style(chars.slice(i, j).join(), options, "string"))
      ...
      let j = i + 1
while j < chars.len() and _arabic-word-char(chars.at(j), options:
options) { j = j + 1 }
      let token = chars.slice(i, j).join()
...
let defined = if type(definition) == dictionary { definition.at(key, default:
none) } else { none }
...
let defined = if type(definition) == dictionary { definition.at("keywords", default:
()) } else { () }
...
} else if type(item) == dictionary and item.at("word", default:
item.at("name", default: none)) == token {
...
let class-style = if type(styles) == dictionary { styles.at(str(class-id), default:
none) } else { none }
  let configured = _get(options, "keywordstyle", default: none)
let defined = if type(definition) == dictionary { definition.at("keywordstyle", default:
none) } else { none }
let style = if class-style != none { class-style } else if configured != none { configured } else if defined != none { defined } else { (fill:
rgb("245bdb"), weight: "bold") }
...
let selected = key => if local(key) == none { definition.at(key, default:
none) } else { local(key) }
    options + (
...
tag: (_get(options, "tag", default:
false) or definition.at("tag", default: false)),
texcl: (_get(options, "texcl", default:
false) or definition.at("texcl", default: false)),
podcomment: (_get(options, "podcomment", default:
false) or definition.at("podcomment", default: false)),
    )
  }
}
...
let comments = if type(definition) == dictionary { _custom-list(definition.at("comments", default:
())) } else { () }
let strings = if type(definition) == dictionary { _custom-list(definition.at("strings", default:
())) } else { () }
let definition-delims = if type(definition) == dictionary { _custom-list(definition.at("delim", default:
())) } else { () }
  let deleted-comments = _get(options, "deletecomment", default: ())
  let deleted-strings = _get(options, "deletestring", default: ())
...
let tex-comments = if _get(scanner-options, "texcl", default:
false) { ("%",) } else { () }
let pod-line = _get(scanner-options, "podcomment", default:
false) and chars.len() > 0 and chars.at(0) == "="
let pieces = if pod-line and _get(scanner-options, "printpod", default:
true) {
(_custom-style(options, definition, "commentstyle", (fill:
rgb("5b856b")), line),)
  } else { () }
  let plain = ""
  let i = if pod-line { chars.len() } else { 0 }
  while i < chars.len() {
let comment-rule = _custom-rule-at(chars, i, comments + tex-comments + _custom-list(_get(options, "comment", default:
())) + _custom-list(_get(options, "morecomment", default: ())))
let string-rule = _custom-rule-at(chars, i, strings + _custom-list(_get(options, "string", default:
())) + _custom-list(_get(options, "morestring", default: ())))
let delim-rule = _custom-rule-at(chars, i, definition-delims + _custom-list(_get(options, "delim", default:
())) + _custom-list(_get(options, "moredelim", default: ())))
let tag-enabled = _get(scanner-options, "tag", default:
false) and chars.at(i) == "<"
...
pieces.push(_custom-style(options, definition, "commentstyle", (fill:
rgb("5b856b")), chars.slice(i, finish).join()))
      i = finish
...
pieces.push(_custom-style(options, definition, "stringstyle", (fill:
rgb("a15c38")), chars.slice(i, finish).join()))
      i = finish
    } else if tag-enabled {
      if plain != "" { pieces.push(plain); plain = "" }
      let cursor = i + 1
...
if delim-style == none { pieces.push(chars.slice(i, finish).join()) } else { pieces.push(text(..(delim-style + _arabic-shared-font(options)), chars.slice(i, finish).join())) }
      ...
      let j = i + 1
while j < chars.len() and _custom-word-char(chars.at(j), options:
scanner-options) { j = j + 1 }
      let token = chars.slice(i, j).join()
...
let identifier-style = _get(scanner-options, "identifierstyle", default:
none)
...
let rendered = _arabic-code-line(_visible-whitespace(line, options), options)
    text(
dir: bidi-direction(line, default-dir:
_get(options, "bidi-default-dir", default: rtl)),
      lang: _get(options, "bidi-default-lang", default: "ar"),
    )[ #rendered ]
  } else {
    ...
    if mode == true or mode == "auto" {
      fix-bidi-content(
        rendered,
        default-dir: _get(options, "bidi-default-dir", default: rtl),
        default-lang: _get(options, "bidi-default-lang", default: "ar"),
      )
    } else {
      rendered
    }
...
let delimiters = if type(configured) == array and configured.len() >= 2 { configured } else if type(escape-char) == str and escape-char.len() > 0 { (escape-char, escape-char) } else if _get(options, "mathescape", default:
false) { ("$", "$") } else { none }
...
let renderer = if _get(options, "mathescape", default:
false) and configured == none {
...
_column-formatted(_code-style(_get(options, "basicstyle", default:
none), raw-content), options)
}
...
#let _line-cells(lines, options, language, offset: 0, source-indices: none) = {
  ...
  let cells = ()
  for (index, line) in lines.enumerate() {
let source-index = if source-indices == none { none } else { source-indices.at(index, default:
none) }
let show-number = _number-this-line(index, line, options, offset:
offset, source-index: source-index)
let number = if show-number { str(_line-number-value(index, options, offset:
offset, source-index: source-index)) } else { "" }
...
let prepared = _line-cells(lines, options, language, offset:
offset, source-indices: source-indices)
table(columns: prepared.at(0), column-gutter:
_get(options, "numbersep", default: 0.8em), row-gutter:
_get(options, "lineskip", default: 0pt), inset: (x: 0pt, y: 0pt), stroke:
none, ..prepared.at(1))
}

#let _render-code-lines(lines, options, language, offset: 0, source-indices: none) = {
if _get(options, "numbers", default:
"none") == none or _get(options, "numbers", default: "none") == "none" {
if _get(options, "escapeinside", default:
none) != none or _get(options, "escapechar", default:
"") != "" or _get(options, "mathescape", default: false) {
...
let raw-content = if language == none or language == "" { raw(shown, block:
true) } else { raw(shown, lang: language, block: true) }
_column-formatted(_code-style(_get(options, "basicstyle", default:
none), raw-content), options)
    }
  } else {
_render-numbered-lines(lines, options, language, offset:
offset, source-indices: source-indices)
  }
}
...
_render-code-lines(selection.lines, options, language, offset:
selection.offset, source-indices: selection.source-indices)
  }
}
...
text(.._get(options, "title-style", default: (size: 8pt, weight:
"bold", fill: rgb("3d4652"))))[
        #title
      ],
    )
  }
...
if _get(options, "numberbychapter", default:
false) { listings-chapter-counter.step() } else { listings-counter.step() }
    let display-name = _get(options, "listing-name", default: "Listing")
    let logical-name = _get(options, "name", default: none)
    block(width: 100%)[
#metadata((kind: "listing", caption: caption, label:
_get(options, "label", default: none), title:
_get(options, "title", default: none), name: logical-name, listed:
not _get(options, "nolol", default: false)))
      #block(
        width: 100%,
        above: _get(options, "abovecaptionskip", default: 0.5em),
        below: _get(options, "belowcaptionskip", default: 0.5em),
...
let inset = (top: framesep + _get(options, "framextopmargin", default:
0pt), bottom: framesep + _get(options, "framexbottommargin", default:
0pt), left: framesep, right: framesep)
  let bg = _background(options)
  let stroke = _frame-stroke(options)
  let radius = _listing-radius(options)
let left = _get(options, "xleftmargin", default:
0pt) + _get(options, "framexleftmargin", default: 0pt)
let right = _get(options, "xrightmargin", default:
0pt) + _get(options, "framexrightmargin", default: 0pt)
let shadow = _get(options, "frame", default:
"none") == "shadowbox" and type(_get(options, "frameshape", default:
none)) != dictionary
...
let inner = block(width: _listing-width(options), inset: inset, fill:
bg, stroke: _get(options, "framerule", default:
0.4pt) + rulecolor, radius: radius, breakable: true, above:
_get(options, "aboveskip", default: 0.5em), below:
_get(options, "belowskip", default: 0.5em))[#body]
block(width: _listing-width(options), inset: (top: rulesep, bottom:
rulesep, left: rulesep, right: rulesep), fill: gap-color, stroke:
_get(options, "framerule", default: 0.4pt) + rulecolor, radius:
radius + rulesep, breakable: true)[#inner]
  } else {
block(width: _listing-width(options), inset: inset, fill: bg, stroke:
stroke, radius: radius, breakable: true, above:
_get(options, "aboveskip", default: 0.5em), below:
_get(options, "belowskip", default: 0.5em))[#body]
  }
...
(left: stroke.at("left", default: none), right:
stroke.at("right", default: none))
  } else {
    (left: stroke, right: stroke)
  }
let top-stroke = if stroke == none { none } else if type(stroke) == dictionary { stroke.at("top", default:
none) } else { stroke }
let bottom-stroke = if stroke == none { none } else if type(stroke) == dictionary { stroke.at("bottom", default:
none) } else { stroke }
let top-edge = if top-stroke == none { [] } else { box(width:
100%, height: 0pt, stroke: (top: top-stroke))[] }
let bottom-edge = if bottom-stroke == none { [] } else { box(width:
100%, height: 0pt, stroke: (bottom: bottom-stroke))[] }
let wrapped = block(width: _listing-width(options), inset: inset, fill:
bg, stroke: side-stroke, radius: radius, breakable: true, above:
_get(options, "aboveskip", default: 0.5em), below:
_get(options, "belowskip", default: 0.5em))[
    #top-edge
    #body
    #bottom-edge
  ]
...
let marker-style = _get(options, "continuation-style", default: (font:
"DejaVu Sans Mono", size: 7pt, fill: rgb("68727e")))
...
let line-cells = _line-cells(lines, options, language, offset:
offset, source-indices: source-indices)
...
listings-layout-state.update((width: size.width, height:
size.height, lines: computed))
      none
    })
    let measured = listings-layout-state.final()
let measured-lines = if type(measured) == dictionary { measured.at("lines", default:
40) } else { 40 }
      ...
      [
        #for (index, chunk) in chunks.enumerate() {
          ...
          let ending = if index + 1 < total {
            []
} else if _get(options, "captionpos", default:
"b") == "b" and caption != none {
            [#caption]
          } else {
            []
          }
...
_everydisplay(options, _listing-frame(options, [#intro #_render-code-lines(chunk, options, language, offset:
offset + index * chunk-size, source-indices: chunk-indices) #ending]))
        }
      ]
    } else {
      let start-page = here().position().page
let intro = if _get(options, "captionpos", default:
"b") == "t" and caption != none {
        [#caption #title]
      } else {
        [#title]
      }
let ending = if _get(options, "captionpos", default:
"b") == "b" and caption != none {
        [#caption]
      } else {
        []
      ...
      _everydisplay(options, _listing-frame-natural(options, [
        #intro
#_render-natural-code-table(selected, options, language, offset:
offset, source-indices: source-indices)
        #ending
      ]))
    }
...
let options = _resolve-style-options(_merge-options(config, options + (language:
language,)), styles)
  let chosen-language = _canonical-language(_lang(options, config))
...
if name == none { body } else { block(width:
100%)[#body #label(str(name))] }
}
...
let body = _listing-body(code, options + (caption: none, label:
none, float: false), language)
  let supplement = _get(options, "listing-name", default: "Listing")
  let listing-caption = if caption == none { none } else {
if _get(options, "numberbychapter", default:
false) { listings-chapter-counter.step() } else { listings-counter.step() }
metadata((kind: "listing", caption: caption, label: label-name, title:
_get(options, "title", default: none), name:
_get(options, "name", default: none), listed:
not _get(options, "nolol", default: false)))
    [#supplement #_listing-number-display(options): #caption]
  }
  let result = if listing-caption == none {
figure(body, kind: "listing", supplement: none, numbering:
none, placement: _float-placement(options))
  } else {
figure(body, kind: "listing", supplement: none, numbering: none, caption:
listing-caption, placement: _float-placement(options))
  }
...
lstlisting(code, language: language, options: options, config:
...
let ending = if index + 1 == chunks.len() and _get(options, "captionpos", default:
"b") == "b" and caption != none {
        [#caption]
      } else { [] }
rendered.push(_listing-frame(options, [#intro #_render-code-lines(chunk, options, chosen-language, offset:
selection.offset + chunk-start, source-indices: chunk-indices) #ending]))
    }
    let tracks = ()
...
_attach-label(_everydisplay(options, body), _get(options, "label", default:
none))
  }
}
...
_line-number-value(selection.lines.len() - 1, options, offset:
selection.offset, source-index: selection.source-indices.last())
  }
}

#let lstlisting(code, language: none, options: (:), config: listings-defaults, styles: (:)) = {
let merged = _resolve-style-options(_merge-options(config, options + (language:
language,)), styles)
  let first = _get(merged, "firstnumber", default: "auto")
  let logical-name = _get(merged, "name", default: none)
  if logical-name == none {
_lstlisting-render(code, language: language, options: options, config:
config, styles: styles)
  } else {
    context {
      let counts = listings-name-state.get()
let previous = if type(counts) == dictionary { counts.at(str(logical-name), default:
0) } else { 0 }
let resolved = if first == "last" { options + (firstnumber:
previous + 1) } else { options }
let rendered = _lstlisting-render(code, language: language, options:
resolved, config: config, styles: styles)
let effective = _merge-options(merged, if first == "last" { (firstnumber:
previous + 1,) } else { (:) })
...
output.push(lstlisting(part, language: language, options: options, config:
config, styles: styles))
    }
    output.join()
  }
...
let records = query(metadata).filter(item => type(item.value) == dictionary and item.value.at("kind", default:
none) == "listing" and item.value.at("listed", default: true))
...
lstlisting(code, language: language, options: options + (wrap-chars:
wrap-chars, breaklines: true), config: config, styles: styles)
}
...
let body = lstlisting(code, language: language, options:
merged + (caption: none, label: none, float: false), config:
config, styles: styles)
  let supplement = _get(merged, "listing-name", default: "Listing")
  let actual-placement = _listing-float-placement(merged, placement)
  let listing-caption = if caption == none { none } else {
if _get(merged, "numberbychapter", default:
false) { listings-chapter-counter.step() } else { listings-counter.step() }
metadata((kind: "listing", caption: caption, label: label-name, title:
_get(merged, "title", default: none), name: _get(merged, "name", default:
none), listed: not _get(merged, "nolol", default: false)))
    [#supplement #_listing-number-display(merged): #caption]
  }
  let result = if listing-caption == none {
figure(body, kind: "listing", supplement: none, numbering:
none, placement: actual-placement)
  } else {
figure(body, kind: "listing", supplement: none, numbering: none, caption:
listing-caption, placement: actual-placement)
  }
  if label-name == none {
    result
  } else {
...
let listing = lstlisting(code, language: language, options:
options + (caption: none, label: none), config: config, styles: styles)
...
_code-style(_get(options, "basicstyle", default:
none), _arabic-code-rendered-line(str(code), options))
  } else if custom != none {
_code-style(_get(options, "basicstyle", default:
none), _custom-code-line(_visible-whitespace(str(code), options), options, custom))
  } else if chosen-language == none or chosen-language == "" {
    raw(str(code))
  } else {
    raw(str(code), lang: chosen-language)
  }
  box(
...
let decoded = decoder(source, _get(options, "inputencoding", default:
none))
...
lstlisting(source, language: language, options: options, config:
config, styles: styles)
}
...
registry + ((name): (aliases: aliases, keywords: keywords, comments:
comments, strings: strings, delim: delim, keywordstyle:
keywordstyle, keywordstyles: keywordstyles, commentstyle:
commentstyle, stringstyle: stringstyle, tagstyle:
tagstyle, identifierstyle: identifierstyle, emph: emph, emphstyle:
emphstyle, index: index, alsoletter: alsoletter, alsodigit:
alsodigit, alsoother: alsoother, tag: tag, texcl: texcl, podcomment:
podcomment, dialect: dialect, dialects: dialects, defaultdialect:
defaultdialect))
}

#let lstnewenvironment(name, base-options: (:), config: listings-defaults, styles: (:)) = {
(code, language: none, options: (:)) => lstlisting(code, language:
language, options: base-options + options, config: config, styles: styles)
}
