...
#idx("adjoinset")
وتأخذ الدالة #py("adjoin_set") كائنًا ومجموعةً كوسيطين وترجع مجموعة تحتوي على عناصر المجموعة الأصلية بالإضافة إلى العنصر الملحق.
#idx("unionset")
وتحسب الدالة #py("union_set") اتحاد مجموعتين، وهو المجموعة التي تحتوي على كل عنصر يظهر في أي من الوسيطين.
#idx("intersectionset")
وتحسب الدالة #py("intersection_set") تقاطع مجموعتين، وهو المجموعة التي تحتوي فقط على العناصر التي تظهر في كلا الوسيطين. ومن وجهة نظر تجريد البيانات، نحن أحرار في تصميم أي تمثيل ينفذ هذه العمليات بطريقة متسقة مع التفسيرات المعطاة أعلاه.#footnote[إذا أردنا أن نكون أكثر صرامة، فيمكننا تحديد "متسق مع التفسيرات المعطاة أعلاه" ليعني أن العمليات تحقق مجموعة من القواعد مثل هذه:
...
فعند التحقق من وجود عنصر ما، لم نعد مضطرين لمسح المجموعة بأكملها. وإذا وصلنا إلى عنصر مجموعة أكبر من العنصر الذي نبحث عنه، فإننا نعرف أن العنصر ليس في المجموعة:
...
ميزة تمثيل الشجرة هي التالية: نفترض أننا نريد الفحص عما إذا كان العدد $x$ محتواً في مجموعة. نبدأ بمقارنة $x$ مع العنصر في العقدة العلوية. فإذا كان $x$ أقل من هذا، فإننا نعلم أننا بحاجة فقط إلى البحث في الشجرة الفرعية اليسرى؛ وإذا كان $x$ أكبر، فسنحتاج فقط إلى البحث في الشجرة الفرعية اليمنى. والآن، إذا كانت الشجرة "متوازنة"، فستكون كل واحدة من هذه الأشجار الفرعية نصف حجم الشجرة الأصلية تقريباً. وبالتالي، اختزلنا في خطوة واحدة مشكلة البحث في شجرة بحجم $n$ إلى البحث في شجرة بحجم $n/2$. وبما أن حجم الشجرة يتنصف في كل خطوة، ففي وسعنا التوقع أن عدد الخطوات اللازمة للبحث في شجرة بحجم $n$ ينمو ك$Theta ( log n)$.#footnote[تنصيف حجم المشكلة في كل خطوة هو الخاصية المميزة للنمو اللوغاريتمي (#idx("logarithmic growth"))، كما رأينا مع خوارزمية الرفع السريع إلى قوة في القسم @sec:exponentiation وطريقة البحث في نصف النطاق في القسم @sec:proc-general-methods.] وبالنسبة للمجموعات الكبيرة، سيكون هذا تسريعاً كبيراً مقارنة بالتمثيلات السابقة.
...
تحول الدالة التالية #py("linked_list_to_tree") (#idx("binary tree", sub: "converting a linked list to a") #idx("linked list", sub: "converting to a binary tree")) قائمة مترابطة مرتبة إلى شجرة ثنائية متوازنة. وتأخذ الدالة المساعدة #py("partial_tree") كوسيطين عددًا صحيحًا $n$ وقائمة مترابطة من $n$ من العناصر على الأقل، وتبني شجرة متوازنة تحوي أول $n$ من عناصر القائمة المترابطة. والنتيجة المعادة من #py("partial_tree") هي زوج (مكوَّن بـ #py("pair")) يكون رأسه (#py("head")) هو الشجرة المبنية وذيله (#py("tail")) هو القائمة المترابطة من العناصر غير المضمنة في الشجرة.
...
#py("intersection_set")
للمجموعات المنفذة كأشجار ثنائية (متوازنة).#footnote[التمارين @ex:tree-to-list–@ex:tree-ops تعود إلى #idx("Hilfinger, Paul") #en[Paul Hilfinger].]
])
...
تأمل قاعدة بيانات تحتوي على عدد كبير من السجلات الفردية (#idx("record, in a data base"))، مثل ملفات الموظفين لشركة أو المعاملات في نظام محاسبي. يقضي نظام إدارة البيانات النموذجي مقداراً كبيراً من الوقت في الوصول إلى البيانات في السجلات أو تعديلها، وبالتالي فإنه يتطلب طريقة كفؤة للوصول إلى السجلات. ويتم ذلك عن طريق تحديد جزء من كل سجل ليخدم ك#idx("key of a record", sub: "in a data base") #emph[مفتاح] (#en[key]) معرف. ويمكن أن يكون المفتاح أي شيء يحدد السجل بشكل فريد. بالنسبة لملف الموظفين، قد يكون رقم تعريف الموظف. وبالنسبة لنظام المحاسبة، قد يكون رقم المعاملة. ومهما كان المفتاح، فعندما نعرّف السجل كبنية بيانات، يجب أن نضمن دالة محدِّد #idx("key") #py("key") تسترجع المفتاح المرتبط بسجل معطى.

والآن نمثل قاعدة البيانات كمجموعة من السجلات. ولتحديد موقع السجل ذي المفتاح المعطى، نستخدم دالة #py("lookup")، والتي تأخذ كوسيطين مفتاحًا وقاعدةَ بيانات، وترجع السجل الذي يحوي ذلك المفتاح، أو #py("False") إذا لم يكن هناك مثل هذا السجل.
وتُنفَّذ دالة #py("lookup") بنفس الطريقة تقريباً التي تُنفَّذ بها #py("is_element_of_set").
...
بالطبع، هناك طرق أفضل لتمثيل المجموعات الكبيرة من تمثيلها كقوائم مترابطة غير مرتبة.
...
وفي تصميم مثل هذا النظام، يمكن لمنهجية تجريد البيانات أن توفر مساعدة كبيرة. إذ يمكن للمصمم إنشاء تنفيذ أولي باستخدام تمثيل بسيط ومباشر مثل القوائم المترابطة غير المرتبة.
وسيكون هذا غير مناسب للنظام النهائي، ولكنه يمكن أن يكون مفيداً في توفير قاعدة بيانات "سريعة وغير متقنة" يمكن فحص بقية النظام بها. ولاحقاً، يمكن تعديل تمثيل البيانات ليكون أكثر تطوراً. وإذا جرى الوصول إلى قاعدة البيانات بدلالة محددات ومُنشِئات مجردة، فإن هذا التغيير في التمثيل لن يتطلب أي تغييرات على بقية النظام.
