...
كتوضيح للتلاعب بالرموز ومثالٍ إضافي على تجريد البيانات، تأمل تصميم دالة تؤدي التفاضل الرمزي للتعبيرات الجبرية. نود أن تأخذ الدالة وسيطين تعبيرًا جبريًا ومتغيرًا وأن ترجع مشتقة التعبير بالنسبة للمتغير. فعلى سبيل المثال، إذا كانت وسائط الدالة هي $a x^(2) + b x + c$ و $x$، فيجب أن ترجع الدالة $2a x + b$. والتفاضل الرمزي له أهمية تاريخية خاصة في لغة البرمجة #en[Lisp].#footnote[استخدمت النسخة الأصلية من هذا الكتاب لغة البرمجة #en[Scheme]، وهي لهجة من #en[Lisp].]
...
لإبقاء الأمور بسيطة، سننظر في برنامج تفاضل رمزي بسيط للغاية يتعامل مع التعبيرات المبنية باستخدام عمليتي الجمع والضرب بعاملين فقط. ويمكن إجراء التفاضل لأي تعبير من هذا القبيل عن طريق تطبيق قواعد الاختزال التالية (#idx("differentiation", sub: "rules for")):
...
لاحظ أن القاعدتين الأخيرتين تعاوديتان بطبيعتهما. أي أنه للحصول على مشتقة مجموع، نجد أولاً مشتقات الحدود ونجمعها. وقد يكون كل حد بدوره تعبيراً يلزمه التفكيك. والتفكيك إلى قطع أصغر وأصغر سينتج في النهاية قطعاً إما ثوابت وإما متغيرات، والتي ستكون مشتقاتها إما $0$ وإما $1$.

لتجسيد هذه القواعد في دالة، نلجأ إلى القليل من #idx("wishful thinking") #emph[التفكير المتمني] (#en[wishful thinking])، كما فعلنا في تصميم تنفيذ الأعداد الكسرية. فلو كانت لدينا وسيلة لتمثيل التعبيرات الجبرية، فسينبغي أن نكون قادرين على تحديد ما إذا كان التعبير مجموعاً أم جداءً أم ثابتاً أم متغيراً. وينبغي أن نكون قادرين على استخراج أجزاء التعبير. بالنسبة للمجموع، على سبيل المثال، نريد التمكن من استخراج الحد المضاف الأول (#py("addend")) والحد المضاف الثاني (#py("augend")). وينبغي أن نكون قادرين أيضاً على بناء تعبيرات من الأجزاء. لنفترض أن لدينا بالفعل دوال لتنفيذ المحددات والمُنشِئات والمحمولات التالية:
...
تتضمن دالة #py("deriv") هذه خوارزمية التفاضل الكاملة. وبما أنها معبر عنها بدلالة البيانات المجردة، فسوف تعمل بغض النظر عن كيفية اختيارنا لتمثيل التعبيرات الجبرية، طالما أننا نصمم مجموعة مناسبة من المحددات والمُنشِئات. وهذه هي المسألة التي يجب أن نتناولها بعد ذلك.

#subheading([تمثيل التعبيرات الجبرية])

#idx("algebraic expression", sub: "representing")

يمكننا تخيّل طرق عديدة لاستخدام بنية القوائم المترابطة لتمثيل التعبيرات الجبرية. فعلى سبيل المثال، يمكننا استخدام قوائم مترابطة من الرموز تعكس الترميز الجبري المعتاد، ممثلين $a x + b$ كالتالي:
#py("llist(\"a\", \"*\", \"x\", \"+\", \"b\")").
ومع ذلك، سيكون من الأكثر ملاءمة أن نعكس البنية الرياضية للتعبير في قيمة #en[Python] التي تمثله؛ أي تمثيل $a x + b$ ك#py("llist(\"+\", llist(\"*\", \"a\", \"x\"), \"b\")"). وتسمى ممارسة وضع العامل الثنائي أمام معمولاته بـ #idx("prefix notation") #emph[الترميز البادئي] (#en[prefix notation])، على النقيض من الترميز الإقحامي المقدم في القسم @sec:expressions. ومع الترميز البادئي، يكون تمثيل بياناتنا لمشكلة التفاضل كما يلي:
...
ولكننا نود أن يعرف البرنامج أن $x dot.op 0 = 0$ و $1 dot.op y = y$ و $0 + y = y$. وكان ينبغي أن تكون الإجابة للمثال الثاني مجرد #py("y"). وكما يظهر المثال الثالث، يصبح هذا أمراً خطيراً عندما تكون التعبيرات معقدة.

صعوبتنا تشبه كثيراً الصعوبة التي واجهناها في تنفيذ الأعداد الكسرية: #idx("algebraic expression", sub: "simplifying") #idx("simplification of algebraic expressions") لم نختزل الإجابات إلى أبسط صورة. ولإنجاز اختزال الأعداد الكسرية، احتجنا فقط إلى تغيير المُنشِئات والمحددات في التنفيذ. ويمكننا اعتماد استراتيجية مماثلة هنا. فلن نغير #py("deriv") على الإطلاق. وبدلاً من ذلك، سنغير #py("make_sum") بحيث إذا كان كلا المضافين أعداداً، فإن #py("make_sum") سجمعهما وترجع مجموعهما. وأيضاً، إذا كان أحد المضافين 0، فإن #py("make_sum") سترجع المضاف الآخر.
...
وبالمثل، سنغير #py("make_product") لتدمج القواعد التي تقول إن ضرب 0 في أي شيء يساوي 0 وإن ضرب 1 في أي شيء يعطي الشيء نفسه:
...
أظهر كيفية توسيع برنامج التفاضل الأساسي ليتعامل مع أنواع أكثر من التعبيرات.
...
وسّع برنامج التفاضل للتعامل مع مجاميع وجداءات عدد اعتباطي من الحدود (حدين أو أكثر). وعندئذ يمكن التعبير عن المثال الأخير أعلاه كالتالي:

#snippet(```python
deriv(llist("*", "x", "y", llist("+", "x", 3)), "x")
```)

حاول القيام بذلك عن طريق تغيير تمثيل المجاميع والجداءات فقط، دون تغيير دالة #py("deriv") على الإطلاق. فعلى سبيل المثال، سيكون #py("addend") للمجموع هو الحد الأول، ويكون #py("augend") هو مجموع بقية الحدود.
])

#exercise(label-name: <ex:2_58>, [
نفترض أننا نريد تعديل برنامج التفاضل بحيث يعمل مع الترميز الرياضي المعتاد، حيث تكون #py("\"+\"") و #py("\"*\"") عاملين إقحاميين لا بادئيين (#idx("infix notation", sub: "prefix notation vs.") #idx("prefix notation", sub: "infix notation vs.")). وبما أن برنامج التفاضل معرف بدلالة البيانات المجردة، يمكننا تعديله ليعمل مع تمثيلات مختلفة للتعبيرات فقط عن طريق تغيير المحمولات والمحددات والمُنشِئات التي تعرّف تمثيل التعبيرات الجبرية التي سيعمل عليها برنامج التفاضل.

+ أظهر كيفية القيام بذلك من أجل تفاضل التعبيرات الجبرية المقدمة في شكل إقحامي، كما في هذا المثال: #snippet(```python llist("x", "+", llist(3, "*", llist("x", "+", llist("y", "+", 2)))) ```) لتبسيط المهمة، افترض أن #py("\"+\"") و #py("\"*\"") تأخذان دائمًا معمولين وأن التعبيرات محاطة بالأقواس بالكامل.
+ تتفاقم الصعوبة بشكل كبير إذا سمحنا بترميز أقرب إلى الترميز الإقحامي المعتاد، والذي يحذف الأقواس غير الضرورية ويفترض أن الضرب له أسبقية أعلى من الجمع، كما في هذا المثال: #snippet(```python llist("x", "+", 3, "*", llist("x", "+", "y", "+", 2)) ```) هل يمكنك تصميم محمولات ومحددات ومُنشِئات مناسبة لهذا الترميز بحيث لا يزال برنامج التفاضل الخاص بنا يعمل؟
])
