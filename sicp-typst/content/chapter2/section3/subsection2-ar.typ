...
لإبقاء الأمور بسيطة، سننظر في برنامج تفاضل رمزي بسيط للغاية يتعامل مع التعبيرات المبنية باستخدام عمليتي الجمع والضرب بعاملين فقط. ويمكن إجراء التفاضل لأي تعبير من هذا القبيل عن طريق تطبيق قواعد التخفيض التالية (#idx("differentiation", sub: "rules for")):
...
يمكننا تخيل طرق عديدة لاستخدام بنية القوائم المترابطة لتمثيل التعبيرات الجبرية. فعلى سبيل المثال، يمكننا استخدام قوائم مترابطة من الرموز تعكس الترميز الجبري المعتاد، ممثلين $a x + b$ كالتالي:
#py("llist(\"a\", \"*\", \"x\", \"+\", \"b\")").
ومع ذلك، سيكون من الأكثر ملاءمة أن نعكس البنية الرياضية للتعبير في قيمة #en[Python] التي تمثله؛ أي تمثيل $a x + b$ ك#py("llist(\"+\", llist(\"*\", \"a\", \"x\"), \"b\")"). وتسمى ممارسة وضع العامل الثنائي أمام معمولاته بـ #idx("prefix notation") #emph[الترميز البادئي] (#en[prefix notation])، على النقيض من الترميز الإقحامي المعتاد المألوف. ومع الترميز البادئي، يكون تمثيل بياناتنا لمشكلة التفاضل كما يلي:
