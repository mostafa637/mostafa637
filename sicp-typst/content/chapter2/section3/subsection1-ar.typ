...
ويمكننا تكوين بيانات مركبة باستخدام السلاسل النصية، فنحصل على قوائم مترابطة مثل:
...
وإذا كان الوسيط الأول غير مضمَّن في القائمة المترابطة (أي ليس مساوياً بـ #py("==") لأي عنصر في القائمة المترابطة)، فإن #py("member") ترجع #py("None"). وخلاف ذلك، ترجع القائمة الفرعية من القائمة المترابطة بدءاً من أول ورود للسلسلة النصية أو العدد:
...
هو #py("False"). ولنكون أكثر دقة، تعرّف #en[Python] عامل المساواة #py("==") تعاودياً بدلالة المساواة الأساسية #py("==") للأعداد والسلاسل النصية بالقول إن #py("a") و #py("b") متساويان إذا كان كلٌّ منهما سلسلة نصية أو عدداً وكانا متساويين، أو إذا كان كلٌّ منهما زوجاً بحيث #py("head(a)") مساوٍ لـ #py("head(b)") و #py("tail(a)") مساوٍ لـ #py("tail(b)"). وتستخدم دالة #py("member") أعلاه #py("==") وبالتالي تفحص المساواة الهيكلية.
...
وداخل السلاسل النصية المقتبسة بعلامات تنصيص مفردة، يمكننا استخدام علامات تنصيص مزدوجة، والعكس بالعكس، لذا فإن #py("'say \"your name\" aloud'") و #py("\"say 'your name' aloud\"") هما سلسلتان نصيتان صالحتان تملكان محارف مختلفة في الموضعين 4 و 14 إذا بدأنا العد من 0. وبناءً على الخط المستخدم، قد لا يسهل تمييز علامتي تنصيص مفردتين عن علامة تنصيص مزدوجة. هل يمكنك اكتشاف أيها أيهما واستنتاج قيمة التعبير التالي؟
