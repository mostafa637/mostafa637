...
ومن الآن فصاعداً، سنسمح بسلسلتين نصيتين كمعاملات لـ #py("==") و #py("!="). يرجع المحمول #py("==") #py("True") إذا وفقط إذا كانت السلسلتان النصيتان متماثلتين، ويرجع #py("!=") #py("True") إذا وفقط إذا لم تكن السلسلتان النصيتان متماثلتين.#footnote[يمكننا اعتبار سلسلتين نصيتين "متماثلتين" إذا كانتا تتكونان من المحارف نفسها وبالترتيب نفسه. ومثل هذا التعريف يتجاوز مسألة عميقة لسنا مستعدين للتعامل معها بعد: معنى "التماثل" في لغة البرمجة. وسوف نعود إلى هذا في الفصل @chap:state (القسم @sec:costs-of-assignment).]
باستخدام #py("==")، يمكننا تنفيذ دالة مفيدة تسمى #py("member").
تأخذ هذه الدالة معاملين: سلسلة نصية وقائمة مترابطة من السلاسل النصية، أو عدداً وقائمة مترابطة من الأعداد.
وإذا كان المعامل الأول غير محتوى في القائمة المترابطة (أي ليس مساوياً بـ #py("==") لأي عنصر في القائمة المترابطة)، فإن #py("member") ترجع #py("None"). وخلاف ذلك، ترجع القائمة الفرعية من القائمة المترابطة بدءاً من التواجد الأول للسلسلة النصية أو العدد:
