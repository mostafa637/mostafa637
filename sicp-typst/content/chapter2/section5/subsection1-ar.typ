...
والعمليات الحسابية في هذه الحزمة هي الدوال الحسابية الأولية (لذا لا توجد حاجة لتعريف دوال إضافية للتعامل مع الأعداد غير الموسومة). وبما أن هذه العمليات تأخذ كل منها معاملين، فإنها تُثبَّت في الجدول مفهرسة بـ القائمة المترابطة #py("llist(\"python_number\", \"python_number\")"):
...
عرف محمولاً عاماً #idx("isequaltozero (generic)") #idx("zero test (generic)") #py("is_equal_to_zero") يفحص ما إذا كان معامله صفراً، وثبته في حزمة الحساب العام. وينبغي أن تعمل هذه العملية للأعداد العادية والأعداد الكسرية والأعداد المركبة.
])
