...
إحدى الطرق للنظر إلى تجريد البيانات هي أنها تطبيقٌ لمبدأ #idx("principle of least commitment") #idx("least commitment, principle of") "الالتزام الأدنى". في تنفيذ نظام الأعداد المركبة في القسم @sec:representations-complex-numbers، يمكننا استخدام إما تمثيل #en[Ben] المستطيلي وإما تمثيل #en[Alyssa] القطبي. ويسمح لنا حاجز التجريد المكون بوساطة المحددات والمُنشِئات بتأجيل اختيار تمثيل ملموس لكائنات بياناتنا إلى آخر لحظة ممكنة، وبالتالي الاحتفاظ بأقصى قدر من المرونة في تصميم نظامنا.

ويمكن أخذ مبدأ التزام الحد الأدنى إلى أبعد من ذلك. فإذا أردنا، فيمكننا الحفاظ على الغموض في التمثيل حتى #emph[بعد] تصميم المحددات والمُنشِئات، واختيار استخدام كل من تمثيل #en[Ben] #emph[و] تمثيل #en[Alyssa]. وإذا تم تضمين كلا التمثيلين في نظام واحد، فسنحتاج إلى طريقة ما لتمييز البيانات في الشكل القطبي عن البيانات في الشكل المستطيلي. وإلا، إذا طُلب منا، على سبيل المثال، إيجاد #py("magnitude") للزوج $(3,4)$، فلن نعرف ما إذا كان يجب الإجابة بـ 5 (بتفسير العدد في الشكل المستطيلي) أو 3 (بتفسير العدد في الشكل القطبي). والطريقة المباشرة لإنجاز هذا التمييز هي تضمين #idx("type tag") #emph[وسم نوع] (#en[type tag])—السلسلة النصية #py("\"rectangular\"") أو #py("\"polar\"")—كجزء من كل عدد مركب. وعندما نحتاج إلى التلاعب بعدد مركب، يمكننا استخدام الوسم لتحديد المحدد المراد تطبيقه.
...
يُنفذ كل محدد عام كدالة تفحص وسم وسيطها وتستدعي الدالة المناسبة للتعامل مع البيانات من ذلك النوع. فعلى سبيل المثال، للحصول على الجزء الحقيقي لعدد مركب، يفحص #py("real_part") الوسم لتحديد ما إذا كان سيستخدم #py("real_part_rectangular") لـ #en[Ben] أم #py("real_part_polar") لـ #en[Alyssa]. وفي كلا الحالتين، نستخدم #py("contents") لاستخراج البيانات الخام غير الموسومة وإرسالها إلى الدالة المستطيلية أو القطبية حسب المطلوب:
...
لتنفيذ العمليات الحسابية للأعداد المركبة، يمكننا استخدام الدوال نفسها #py("add_complex") و #py("sub_complex") و #py("mul_complex") و #py("div_complex") من القسم @sec:representations-complex-numbers، لأن المحددات التي تستدعيها هي عامة، وبالتالي ستعمل مع أي من التمثيلين. فعلى سبيل المثال، دالة #py("add_complex") لا تزال:
...
أخيراً، يجب أن نختار ما إذا كنا سنبني الأعداد المركبة باستخدام تمثيل #en[Ben] أم تمثيل #en[Alyssa]. وأحد الخيارات المعقولة هو بناء الأعداد المستطيلية كلما كانت لدينا أجزاء حقيقية وتخيلية وبناء الأعداد القطبية كلما كانت لدينا سعات وزوايا:
