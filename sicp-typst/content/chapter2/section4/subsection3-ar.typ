...
- #idx("get") #py("get(")#meta("op")#py(",")#meta("type")#py(")") \ يبحث عن الإدخال #meta("op"), #meta("type") في الجدول ويرجع العنصر الموجود هناك. وإذا لم يُعثر على أي عنصر، فإن #py("get") ترجع قيمة أولية فريدة يُشار إليها بالكلمة المفتاحية #idx("None (keyword)") #py("None") وتتعرف عليها المحمولات الأولية #idx("isnone (primitive function)") #py("is_none").
...
إليك كيفية استخدام البرمجة الموجهة بالبيانات في نظام الأعداد المركبة. ينفذ #en[Ben]، الذي طور التمثيل المستطيلي، شفرته تماماً كما فعل أصلًا. إذ يعرف مجموعة من الدوال أو #idx("package") #idx("package", sub: "rectangular representation") #idx("rectangular package") #emph[حزمة]، ويربط هذه ببقية النظام عن طريق إضافة إدخالات إلى الجدول تخبر النظام بكيفية العمل على الأعداد المستطيلية. ويتم ذلك عن طريق استدعاء الدالة التالية:
...
لاحظ أن الدوال الداخلية هنا هي الدوال نفسها من القسم @sec:representations-complex-numbers التي كتبها #en[Ben] عندما كان يعمل في عزلة. ولا يلزم إجراء أي تغييرات لربطها ببقية النظام. علاوة على ذلك، وبما أن تعاريف الدوال هذه داخلية لدالة التثبيت، فلا يتوجب على #en[Ben] القلق بشأن تعارض الأسماء مع دوال أخرى خارج الحزمة المستطيلية. ولربط هذه ببقية النظام، يثبت #en[Ben] دالة #py("real_part") الخاصة به تحت اسم العملية #py("real_part") والنوع #py("llist(\"rectangular\")")، وبالمثل لمحددات الاختيارات الأخرى.#footnote[نستخدم القائمة المترابطة #py("llist(\"rectangular\")") بدلاً من السلسلة النصية #py("\"rectangular\"") للسماح بإمكانية وجود عمليات بمعاملات متعددة، ليست جميعها من النوع نفسه.] وتعرّف الواجهة أيضاً البواني المراد استخدامها بوساطة النظام الخارجي.#footnote[النوع الذي تُثبَّت البواني تحته لا يلزم أن يكون قائمة مترابطة لأن الباني يُستخدم دائماً لصنع كائن من نوع محدد واحد.] وهذه متطابقة مع بواني #en[Ben] المعرفة داخلياً، باستثناء أنها ترفق الوسم.
...
على الرغم من أن كلًا من #en[Ben] و #en[Alyssa] ما زالا يستخدما دالهما الأصلية المعرفة بالأسماء نفسها لبعضهما البعض (مثل #py("real_part"))، فإن هذه الإعلانات أصبحت الآن داخلية لدوال مختلفة (انظر القسم @sec:block-structure)، لذا لا يوجد تعارض في الأسماء.

تصل محددات اختيارات الحساب المركب إلى الجدول عن طريق دالة "عملية" العامة المسماة #py("apply_generic")، والتي تطبق عملية عامة على بعض المعاملات. تبحث الدالة #py("apply_generic") في الجدول تحت اسم العملية وأنواع المعاملات وتطبق الدالة الناتجة إذا كانت موجودة:#footnote[تستخدم الدالة #py("apply_generic") الدالة #idx("applyinunderlyingjavascript") #py("apply_in_underlying_javascript") المعطاة في القسم @sec:running-eval (الحاشية @foot:vector-array)، والتي تأخذ معاملين، دالة وقائمة مترابطة، وتطبق الدالة باستخدام العناصر الموجودة في القائمة المترابطة كمعاملات. فعلى سبيل المثال، #snippet(```python apply_in_underlying_javascript(sum_of_squares, llist(1, 3)) ```) ترجع 10.]
...
باستخدام #py("apply_generic")، يمكننا تعريف محددات الاختيارات العامة الخاصة بنا كما يلي:
...
ويمكننا أيضاً استخراج البواني من الجدول لتُستخدم بوساطة البرامج الخارجية عن الحزم في صنع أعداد مركبة من أجزاء حقيقية وتخيلية ومن سعات وزوايا. وكما في القسم @sec:manifest-types، نبني الأعداد المستطيلية كلما كانت لدينا أجزاء حقيقية وتخيلية، والأعداد القطبية كلما كانت لدينا سعات وزوايا:
...
يمكننا اعتبار هذا البرنامج مكملاً لإرسالٍ بناءً على نوع التعبير المراد تفاضله. وفي هذه الحالة يكون "وسم النوع" للبيانات هو رمز العامل الجبري (مثل "+") والعملية التي يتم أداؤها هي #py("deriv"). ويمكننا تحويل هذا البرنامج إلى أسلوب موجه بالبيانات عن طريق إعادة كتابة دالة المشتقة الأساسية كـ:
...
+ اشرح ما تم القيام به أعلاه. لماذا لا يمكننا استيعاب المحمولين #py("is_number") و #py("is_variable") في الإرسال الموجه بالبيانات؟
...
+ في هذا المتلاعب الجبري البسيط، يكون نوع التعبير هو العامل الجبري الذي يربطه معاً. ولكن نفترض أننا فهرسنا الدوال بالطريقة المعاكسة، بحيث بدا سطر الإرسال في #py("deriv") كـ #snippet(```python get(operator(exp), "deriv")(operands(exp), variable) ```) ما هي التغييرات المقابلة لنظام المشتقة المطلوبة؟
])
...
كأمثلة، نفترض أن سجلات الموظفين لكل قسم تتكون من ملف واحد يحتوي على مجموعة من السجلات المفهرسة بأسماء الموظفين. وتختلف بنية المجموعة من قسم إلى آخر. علاوة على ذلك، فإن سجل كل موظف هو نفسه مجموعة (مبنية بشكل مختلف من قسم إلى آخر) تحتوي على معلومات مفهرسة تحت معرفات مثل #py("address") و #py("salary"). وعلى وجه الخصوص:

+ نفذ للمقر الرئيسي دالة #py("get_record") ترجع سجل موظف محدد من ملف موظفين محدد. وينبغي أن تكون الدالة قابلة للتطبيق على ملف أي قسم. واشرح كيف ينبغي تشكيل ملفات الأقسام الفردية. وعلى وجه الخصوص، ما هي معلومات النوع التي يجب تقديمها؟
+ نفذ للمقر الرئيسي دالة #py("get_salary") ترجع معلومات الراتب من سجل موظف معطى من ملف موظفين لأي قسم. وكيف ينبغي تشكيل السجل لجعل هذه العملية تعمل؟
+ نفذ للمقر الرئيسي دالة #py("find_employee_record"). وينبغي لهذه الدالة البحث في جميع ملفات الأقسام عن سجل موظف معطى وإرجاع السجل. وافترض أن هذه الدالة تأخذ كمعاملات اسم موظف وقائمة مترابطة لجميع ملفات الأقسام.
...
ودالة #py("apply_generic") المقابلة، والتي تطبق عملية عامة على معامل، تغذي الآن ببساطة اسم العملية لكائن البيانات وتترك الكائن يقوم بالعمل:#footnote[إحدى قيود هذا التنظيم هي أنه يسمح فقط بالدوال العامة ذات المعامل الواحد.]
...
يسمى أسلوب البرمجة هذا #emph[تمرير الرسائل] (#en[message passing]). ويأتي الاسم من الصورة القائلة بأن كائن البيانات هو كيان يستقبل اسم العملية المطلوبة كـ "رسالة". وقد رأينا بالفعل مثالاً على تمرير الرسائل في القسم @sec:data-، حيث رأينا كيف يمكن تعريف #py("pair") و #py("head") و #py("tail") دون كائنات بيانات ولكن فقط مع الدوال. وهنا نرى أن تمرير الرسائل ليس خدعة رياضية بل تقنية مفيدة لتنظيم الأنظمة ذات العمليات العامة. وفي بقية هذا الفصل سنستمر في استخدام البرمجة الموجهة بالبيانات، بدلاً من تمرير الرسائل، لمناقشة العمليات الحسابية العامة. وفي الفصل @chap:state سنعود إلى تمرير الرسائل، وسنرى أنها يمكن أن تكون أداة قوية لبناء برامج المحاكاة.

#exercise(label-name: <ex:2_75>, [
نفذ الباني #idx("makefrommagang", sub: "message-passing") #py("make_from_mag_ang") بأسلوب تمرير الرسائل. وينبغي أن تكون هذه الدالة مماثلة لدالة #py("make_from_real_imag") المعطاة أعلاه.
])

#exercise(label-name: <ex:extend-generic>, [
مع تطور نظام كبير ذي عمليات عامة، قد يلزم أنواع جديدة من كائنات البيانات أو عمليات جديدة. ولكل من الاستراتيجيات الثلاث—العمليات العامة ذات الإرسال الصريح (#idx("dispatching", sub: "comparing different styles"))، والأسلوب الموجه بالبيانات، وأسلوب تمرير الرسائل—صف التغييرات التي يجب إجراؤها على نظام ما من أجل إضافة أنواع جديدة أو عمليات جديدة. وأي تنظيم سيكون الأكثر ملاءمة لنظام يجب أن تُضاف فيه أنواع جديدة غالباً؟ وأيها سيكون الأكثر ملاءمة لنظام يجب أن تُضاف فيه عمليات جديدة غالباً؟
])
