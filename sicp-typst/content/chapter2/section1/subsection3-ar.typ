...
ولكن ما المقصود بالـ #emph[بيانات] بالضبط؟ لا يكفي القول «كل ما يُنفَّذ بواسطة المحدّدات والمُنشِئات المعطاة». فمن الواضح أن ليس أي مجموعة اختيارية من ثلاث دوال يمكن أن تعمل كأساس مناسب لتنفيذ الأعداد الناطقة. إذ نحتاج إلى ضمان أنه،
...
أظهر أنه يمكننا تمثيل أزواج الأعداد الصحيحة غير السالبة باستخدام الأعداد والعمليات الحسابية فقط إذا مثلنا الزوج $a$ و $b$ كعدد صحيح هو حاصل الضرب $2^(a) 3^(b)$. وأعطِ التعاريف المناظرة للدوال #py("pair") و #py("head") و #py("tail").
])

#idx("pair(s)", sub: "functional representation of")

#exercise(label-name: <ex:church-numerals>, [
في حال لم يكن تمثيل الأزواج كدوال (التمرين @ex:lambda-cons) مذهلاً بما فيه الكفاية، تأمَّل أنه، في لغة يمكنها التعامل مع الدوال، يمكننا الاستغناء عن الأعداد (على الأقل فيما يتعلق بالأعداد الصحيحة غير السالبة) بتنفيذ 0 وعملية إضافة 1 كالتالي:
