...
بدأنا تنفيذ الأعداد الناطقة في القسم @sec:rationals بتنفيذ عمليات الأعداد الناطقة #py("add_rat") و #py("sub_rat") وما إلى ذلك بدلالة ثلاث دوال غير محدَّدة: #py("make_rat") و #py("numer") و #py("denom"). وفي تلك النقطة، كان بإمكاننا التفكير في العمليات كما لو كانت مُحدَّدة بدلالة كائنات بيانات — بسط ومقام وأعداد ناطقة — والتي حُدِّد سلوكها بالدوال الثلاث الأخيرة.

ولكن ما المقصود بالـ #emph[بيانات] بالضبط؟ لا يكفي القول «كل ما يُنفَّذ بواسطة المحدّدات والمُنشِئات المعطاة». فمن الواضح أنّ ليس أيَّ مجموعة اعتباطية من ثلاث دوال يمكن أن تشكّل أساسًا مناسبًا لتنفيذ الأعداد الناطقة. إذ نحتاج إلى ضمان أنه،
...
#emph[المواصفات الجبرية (#en[algebraic specification])]. وتعتبر «الدوال» كعناصر لنظام جبري مجرد يُحدَّد سلوكه ببديهيات تتوافق مع «شروطنا»، وتستخدم تقنيات الجبر المجرد للتحقق من التأكيدات حول كائنات البيانات. وقد استُعرضت كلتا الطريقتين في ورقة ليسكوف وزيليس
#idx("Liskov, Barbara Huberman")
(1975).]

#idx("data", sub: "functional representation of") #idx("functional representation of data")
ويمكن لوجهة النظر هذه أن تصلح ليس فقط لتحديد كائنات البيانات «عالية المستوى»، مثل الأعداد الناطقة، بل والكائنات منخفضة المستوى أيضًا.
تأمَّل مفهوم
...
أي أنّ العمليات تحقّق الشرط أنه، لأي كائنين #py("x") و #py("y")، إذا كان #py("z") هو #py("pair(x, y)") فإنّ #py("head(z)") هو #py("x") و #py("tail(z)") هو #py("y"). وبالفعل، ذكرنا أنّ هذه الدوال الثلاث ضُمِّنَت كأوليات في لغتنا. ومع ذلك، فإنّ أي ثلاثية من الدوال تحقّق الشرط أعلاه يمكن استخدامها كأساس لتنفيذ الأزواج. وتُوضَّح هذه النقطة بشكل مذهل بحقيقة أنه كان بإمكاننا تنفيذ #py("pair") و #py("head") و #py("tail") دون استخدام أي هياكل بيانات على الإطلاق بل فقط باستخدام الدوال.
إليك التعاريف:#footnote[الدالة #idx("error (primitive function)", sub: "optional second argument") #py("error") المقدمة في القسم @sec:proc-general-methods تأخذ كوسيط ثانٍ اختياري سلسلةً نصية يُعرَض قبل الوسيط الأول — على سبيل المثال، إذا كان #py("m") هو 42: #output(```python Error in line 7: argument not 0 or 1 -- pair: 42 ```)]
...
أظهر أنه يمكننا تمثيل أزواج الأعداد الصحيحة غير السالبة باستخدام الأعداد والعمليات الحسابية فقط إذا مَثَّلنا الزوج $a$ و $b$ بالعدد الصحيح المُساوي لحاصل الضرب $2^(a) 3^(b)$. وأعطِ التعاريف المناظرة للدوال #py("pair") و #py("head") و #py("tail").
])

#idx("pair(s)", sub: "functional representation of")

#exercise(label-name: <ex:church-numerals>, [
في حال لم يكن تمثيل الأزواج كدوال (التمرين @ex:lambda-cons) مذهلاً بما يكفي، تأمَّل أنه، في لغة يمكنها التعامل مع الدوال، يمكننا الاستغناء عن الأعداد (على الأقل فيما يتعلق بالأعداد الصحيحة غير السالبة) بتنفيذ 0 وعملية إضافة 1 كالتالي:
...
عرِّف #py("one") و #py("two") مباشرة (وليس بدلالة #py("zero") و #py("add_1")).
