...
#emph[التفكير بالتمنّي (#en[wishful thinking])]. فلم نذكر بعد كيف يُمثَّل العدد الناطق، أو كيف يجب تنفيذ الدوال #py("numer") و #py("denom") و #py("make_rat"). ومع ذلك، لو كنا نملك هذه الدوال الثلاث، فبإمكاننا عندئذ الجمع والطرح والضرب والقسمة واختبار المساواة باستخدام العلاقات التالية:
...
#py("pair").
تأخذ هذه الدالة وسيطين وترجع كائن بيانات مركب يحتوي على الوسيطين كأجزاء. ومع إعطاء زوج، يمكننا استخراج الأجزاء باستخدام الدالتين الأوليتين
...
في القسم @sec:hierarchical-data سنرى كيف تتيح هذه القدرة على دمج الأزواج استخدامَ الأزواج كلبنات بناء عامة الغرض لإنشاء جميع أنواع هياكل البيانات المعقدة. والأداة الأولية الوحيدة للبيانات المركبة هي #emph[الزوج]، المُنفَّذ بالدوال #py("pair") و #py("head") و #py("tail")، وهو الغراء الوحيد الذي نحتاجه. وتُسمّى كائنات البيانات المنشأة من الأزواج
#idx("list structure")
...
طريقة طبيعية لإكمال نظام الأعداد الناطقة.
ببساطة، مَثِّل العدد الناطق كزوج من عددين صحيحين: بسط ومقام. وعندئذ تُنفَّذ #py("make_rat") و #py("numer") و #py("denom") بسهولة كما يلي:#footnote[طريقة أخرى لتعريف المحدّدات والمُنشِئ هي:
...
كما هو مطلوب. وقد تحقق هذا التعديل بتغيير المُنشِئ #py("make_rat") دون تغيير أيٍّ من الدوال (مثل #py("add_rat") و #py("mul_rat")) التي تنفذ العمليات الفعلية.

#exercise(label-name: <ex:2_1>, [
عرِّف نسخة أفضل من #py("make_rat") تتعامل مع الوسائط الموجبة والسالبة كلاهما.
ينبغي أن توحّد الدالة #py("make_rat") الإشارة بحيث إذا كان العدد الناطق موجباً، يكون البسط والمقام كلاهما موجباً، وإذا كان العدد الناطق سالباً، يكون البسط وحده سالباً.
])
