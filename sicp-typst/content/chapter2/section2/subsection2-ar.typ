...
أداة طبيعية للتعامل مع بنى الأشجار، حيث يمكننا غالباً اختزال العمليات على الأشجار إلى عمليات على فروعها، والتي تختزل بدورها إلى عمليات على فروع الفروع، وهكذا حتى نصل إلى أوراق (#en[leaves]) الشجرة. وك مثال على ذلك، قارن دالة
#py("length")
...
ولكن في خطوة الاختزال، حيث ننتزع رأس (#py("head")) القائمة المترابطة، يجب أن نأخذ في الاعتبار أن الرأس (#py("head")) نفسه قد يكون شجرة نحتاج إلى عد أوراقها. وبالتالي، فإن خطوة الاختزال المناسبة هي:
...
#py("is_pair")،
والذي يفحص ما إذا كان وسيطه زوجًا. إليك الدالة الكاملة:#footnote[ترتيب المحمولين مهم، لأن #py("None") يحقق #py("is_none") وهو ليس زوجاً أيضاً.]
...
أعطِ تركيبات من الرؤوس (#py("head")s) والذيول (#py("tail")s) التي ستستخرج 7 من كل قائمة من القوائم المترابطة التالية، المعطاة بترميز القوائم المترابطة:
...
نفترض أننا عرفنا #py("x") و #py("y") لتكونا قائمتين مترابطتين:
...
#py("deep_reverse")
تأخذ قائمةً مترابطةً كوسيط وترجع كقيمتها القائمة المترابطة مع عكس عناصرها وعكس جميع القوائم الفرعية فيها عكساً عميقاً أيضاً. على سبيل المثال،
...
#py("fringe")
تأخذ شجرةً كوسيط (ممثَّلة كقائمة مترابطة) وترجع قائمة مترابطة تكون عناصرها جميع أوراق الشجرة مرتبة من اليسار إلى اليمين. على سبيل المثال،
...
تتكون المعلَّقة الثنائية (#idx("mobile") #en[binary mobile]) من فرعين: فرع أيسر وفرع أيمن. كل فرع هو قضيب ذو طول معين، يعلق منه إما وزن أو معلقة ثنائية أخرى. يمكننا تمثيل المعلَّقة الثنائية باستخدام البيانات المركبة عن طريق بنائها من فرعين (على سبيل المثال، باستخدام #py("llist")):
...
يُبنى الفرع من طول (#py("length")) (يجب أن يكون رقماً) جنباً إلى جنب مع بنية (#py("structure"))، والتي قد تكون إما رقماً (يمثل وزناً بسيطاً) أو معلقةً أخرى:
...
+ اكتب المحددات المقابلة #py("left_branch") و #py("right_branch")، والتي ترجع فرعي المعلَّقة، و #py("branch_length") و #py("branch_structure")، والتي ترجع مكونات الفرع.
+ باستخدام المحددات التي كتبتها، عرِّف دالة #py("total_weight") ترجع الوزن الإجمالي للمعلَّقة.
+ يُقال عن المعلَّقة إنها #idx("balanced mobile") #emph[متوازنة] إذا كان عزم الدوران الناتِج عن فرعها العلوي الأيسر يساوي عزم الدوران الناتِج عن فرعها العلوي الأيمن (أي إذا كان طول القضيب الأيسر مضروباً في الوزن المعلق منه يساوي الجداء المقابل للجانب الأيمن) وكانت كل معلقة فرعية معلقة من فرعيها متوازنة. صمم محمولاً يفحص ما إذا كانت المعلَّقة الثنائية متوازنة.
+ نفترض أننا غيرنا تمثيل المعلقات بحيث تكون الدالتان المُنشِئتان: #snippet(```python def make_mobile(left, right): return pair(left, right) def make_branch(length, structure): return pair(length, structure) ```) كم يتوجب عليك تغيير برامجك للتحويل إلى التمثيل الجديد؟
])
...
#py("scale_tree")،
المماثلة لـ #py("scale_linked_list") من القسم @sec:sequences، تأخذ كوسائط عاملًا عدديًا وشجرة أوراقها أعداد. وترجع شجرة بالشكل نفسه، حيث يُضرب كل عدد في العامل. والخطة التعاودية لـ
#py("scale_tree")
مشابهة لخطة
#py("count_leaves"):
...
نطبق #py("map") على المتتالية، مغيرين مقياس كل شجرة فرعية بدورها، ونرجع القائمة المترابطة للنتائج. وفي الحالة الأساسية، حيث تكون الشجرة ورقة، نضرب ببساطة في العامل:
...
عمِّم إجابتك في التمرين @ex:square-tree لإنتاج دالة
#idx("treemap")
#py("tree_map")
بحيث يمكن الإعلان عن #py("square_tree") كالتالي:
...
مجموعة كقائمة مترابطة من العناصر المتمايزة، ويمكننا تمثيل مجموعة جميع المجموعات الجزئية لمجموعة كقائمة مترابطة من القوائم المترابطة. على سبيل المثال، إذا كانت المجموعة هي
#py("llist(1, 2, 3)")،
