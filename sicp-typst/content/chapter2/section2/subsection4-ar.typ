...
#idx("Henderson, Peter")
#en[Peter Henderson] لإنشاء صور مثل خشبية "حد المربع" (#en[Square Limit]) لـ
...
#idx("painter(s)")
#emph[رسّام] (#en[painter]). يرسم الرسّام صورة يتم إزاحتها وتغيير مقياسها لتناسب إطاراً معيناً على شكل متوازي أضلاع. فعلى سبيل المثال، هناك رسّام أولي سنسميه #py("wave")
...
#blockquote["بينما أقف هنا اليوم وأرى ما أضحي عليه المعهد، ... أستحضر بدايات العلم. أتذكر منذ مائة وخمسين عاماً عندما نشر ستيفن هيلز كتيباً حول موضوع غاز الإضاءة، ذكر فيه أن أبحاثه أثبتت أن 128 حبة من الفحم القاري—"

#idx("coal, bituminous")
"الفحم القاري"، كانت هذه كلماته الأخيرة على الأرض. وهنا انحنى للأمام، وكأنه يستشير بعض الملاحظات على الطاولة أمامه، ثم استعاد ببطء وقفته المنتصبة، ورفع يديه، وانتقل من مشهد متاع الأرض وانتصاراتها إلى "غد الموت"، حيث تُحل أسرار الحياة، وتجد الروح المجردة راحة سرمدية في التأمل في الأسرار الجديدة التي لا تسبر غورها للمستقبل اللانهائي.]
...
فعلى سبيل المثال، يمكننا تجريد النمط في #py("wave4") كـ
...
#sicp-figure(image("/images/img_javascript/2.14.svg", width: 45%), caption: [العملية التعاودية #py("right_split") مدمجة مع الرسّامين #py("wave") و #py("rogers"). ويؤدي دمج أربعة أشكال لـ #py("corner_split") إلى إنتاج متناظر لـ #py("square_limit") كما هو موضح في الشكل @fig:sqlimit-designs.], label-name: <fig:split-plans-2>)
...
بالإضافة إلى تجريد أنماط دمج الرسّامين، يمكننا العمل على مستوى أعلى، بتجريد أنماط دمج عمليات الرسّامين. أي أنه يمكننا النظر إلى عمليات الرسّامين كعناصر للتلاعب بها ويمكننا كتابة وسائل دمج لهذه العناصر—دوال تأخذ عمليات الرسّامين كمعاملات وتنشئ عمليات رسّامين جديدة.

فعلى سبيل المثال، يرتب كل من #py("flipped_pairs") و #py("square_limit") أربع نسخ من صورة الرسّام في نمط مربعي؛ ويختلفان فقط في كيفية توجيه النسخ. وإحدى الطرق لتجريد هذا النمط من دمج الرسّامين هي باستخدام الدالة التالية، التي تأخذ أربع عمليات رسّامين ذات معامل واحد وتنتج عملية رسّام تحول رسّاماً معطى بتلك العمليات الأربع وترتب النتائج في مربع.#footnote[تتكون عملية الرسّام المعادة بوساطة #py("square_of_four") من عبارات متعددة، لذا لا يمكن كتابتها كتعبير لامبدا، والذي يجب أن يكون جسمه في #en[Python] تعبيراً واحداً. وبدلاً من ذلك، نستخدم إعلان دالة محلية باسم #py("combine"). #idx("lambda expression", sub: "restricted to a single expression in Python")]<foot:lambda_with_block>
...
ويمكن التعبير عن #py("square_limit") كـ:#footnote[تدور الدالة #py("rotate180") الرسّام بمقدار 180 درجة. وبدلاً من #py("rotate180") كان بإمكاننا قول #py("compose(flip_vert, flip_horiz)")، باستخدام دالة #py("compose") من التمرين @ex:compose.]
#idx("squarelimit", decl: true)
#snippet(```python
def square_limit(painter, n):
...
يمكن التعبير عن الدالتين #py("right_split") و #py("up_split") كحالات من عملية انقسام عامة.
أعلن عن دالة
#idx("split")
...
ينتج دوال #py("right_split") و #py("up_split") بالسلوكيات نفسها للتي تم الإعلان عنها بالفعل.
])
...
وإذا كانت الحواف متعامدة، فسيصل الإطار مستطيلاً.
وإلا فسيصل الإطار متوازي أضلاع أكثر عمومية.
...
#idx("makeframe")
#py("make_frame")،
يأخذ ثلاثة متجهات وينتج إطاراً، وثلاثة محددات اختيارات مقابلة:
#idx("originframe")
#py("origin_frame")،
#idx("edge1frame")
#py("edge1_frame")،
...
لاحظ أن تطبيق #py("frame_coord_map") على إطار يرجع دالة، عند إعطائها متجهاً، ترجع متجهاً. وإذا كان متجه المعامل في المربع الوحدة، فإن المتجه الناتج سيكون في الإطار. فعلى سبيل المثال،
...
#idx("makevect")
#py("make_vect")
ومحددي الاختيارات المقابلين:
#idx("xcorvect")
#py("xcor_vect")
و
#idx("ycorvect")
#py("ycor_vect").
وبدلالة محددي الاختيارات والباني، نفذ الدوال:
#idx("addvect")
#py("add_vect")،
#idx("subvect")
#py("sub_vect")،
...
لكل باني، قدم محددي الاختيارات المناسبين لإنتاج تنفيذ للإطارات.
...
يُمثَّل الرسّام كدالة (#idx("painter(s)", sub: "represented as functions"))، عند إعطائها إطاراً كمعامل، ترسم صورة معينة تم إزاحتها وتغيير مقياسها لتناسب الإطار. بمعنى أنه إذا كان #py("p") رسّاماً و #py("f") إطاراً، فإننا ننتج صورة #py("p") في #py("f") عن طريق استدعاء #py("p") مع #py("f") كمعامل.
...
تُعطى القطع المستقيمة باستخدام إحداثيات بالنسبة للمربع الوحدة. ولكل قطعة مستقيمة في القائمة المترابطة، يحول الرسّام نقطتي نهاية القطعة لخريطة إحداثيات الإطار ويرسم خطاً بين النقطتين المحولتين.

تمثيل الرسّامين كدوال يقيم حاجز تجريد قوياً في لغة الصور. فيمكننا إنشاء وخلط جميع أنواع الرسّامين الأوليين، بناءً على مجموعة متنوعة من قدرات الرسومات. ولا تهم تفاصيل تنفيذها. أي دالة يمكن أن تخدم كرسّام، بشرط أن تأخذ إطاراً كمعامل وترسم شيئاً مغيراً مقياسه ليلائم الإطار.#footnote[فعلى سبيل المثال، تم بناء الرسّام #py("rogers") في الشكل @fig:rogers من صورة متدرجة الرمادي. ولكل نقطة في إطار معطى، يحدد الرسّام #py("rogers") النقطة في الصورة التي تم رسمها إليها تحت خريطة إحداثيات الإطار، ويظللها وفقاً لذلك.
...
تعمل العملية على الرسّامين (مثل #py("flip_vert") أو #py("beside")) عن طريق إنشاء رسّام يستدعي الرسّامين الأصليين بالنسبة للإطارات المشتقة من إطار المعامل. وبالتالي، على سبيل المثال، لا يتعين على #py("flip_vert") معرفة كيفية عمل الرسّام من أجل قلبه—عليها فقط معرفة كيفية قلب الإطار رأساً على عقب: فالرسّام المقلوب يستخدم ببساطة الرسّام الأصلي، ولكن في الإطار المقلوب.

تعتمد عمليات الرسّامين على الدالة #py("transform_painter")، التي تأخذ كمعاملات رسّاماً ومعلومات حول كيفية تحويل إطار وتنتج رسّاماً جديداً. والرسّام المحول، عند استدعائه على إطار، يحول الإطار ويستدعي الرسّام الأصلي على الإطار المحول. والمعاملات لـ #py("transform_painter") هي نقاط (ممثلة كمتجهات) تحدد زوايا الإطار الجديد: وعند رسمها في الإطار، تحدد النقطة الأولى نقطة أصل الإطار الجديد وتحدد النقطتان الأخريان نهايتي متجهي حافتيه. وبالتالي، فإن المعاملات داخل المربع الوحدة تحدد إطاراً محتواً داخل الإطار الأصلي.
...
باستخدام #py("transform_painter")، يمكننا تعيين تحويلات جديدة بسهولة. على سبيل المثال، يمكننا الإعلان عن رسّام يقلص صورته إلى الربع العلوي الأيمن من الإطار الذي يُعطى له:
...
وتدير تحويلات أخرى الصور عكس اتجاه عقارب الساعة بمقدار 90 درجة#footnote[الدالة #py("rotate90") هي دوران خالص للإطارات المربعة فقط، لأنها تقوم أيضاً بتمديد وتقليص الصورة لتقابل الإطار المدور.]
...
فدالة #py("beside")، على سبيل المثال، تأخذ رسّامين، وتحولهما للرسم في النصفين الأيسر والأيمن من إطار المعامل على التوالي، وتنتج رسّاماً مركباً جديداً. وعندما يُعطى الرسّام المركب إطاراً، فإنه يستدعي الرسّام المحول الأول للرسم في النصف الأيسر من الإطار ويستدعي الرسّام المحول الثاني للرسم في النصف الأيمن من الإطار:
...
#idx("fliphoriz")
#py("flip_horiz")،
الذي يقلب الرسّامين أفقياً، والتحويلات التي تدير الرسّامين عكس اتجاه عقارب الساعة بمقدار 180 درجة و 270 درجة.
])
...
#idx("below")
#py("below") للرسّامين.
تأخذ الدالة #py("below") رسّامين كمعاملات. والرسّام الناتج، عند إعطائه إطاراً، يرسم باستخدام الرسّام الأول في الجزء السفلي من الإطار وباستخدام الرسّام الثاني في الجزء العلوي.
عرف #py("below") بطريقتين مختلفين—أولاً عن طريق كتابة دالة مماثلة لدالة #py("beside") المعطاة أعلاه، ومجدداً بدلالة #py("beside") وعمليات الدوران المناسبة (من التمرين @ex:rotate).
])

#subheading([مستويات اللغة للتصميم المتين])

تستغل لغة الصور بعض الأفكار الحرة التي قدمناها حول التجريد مع الدوال والبيانات. فالتجريدات الأساسية للبيانات، الرسّامون، يتم تنفيذها باستخدام تمثيلات دالية، مما يمكن اللغة من التعامل مع قدرات الرسم الأساسية المختلفة بطريقة موحدة. وتحقق وسائل الدمج خاصية الإغلاق، مما يتيح لنا بناء تصاميم معقدة بسهولة. وأخيراً، فإن جميع أدوات تجريد الدوال متاحة لنا لتجريد وسائل دمج الرسّامين.
...
يتخلل التصميم المتدرج هندسة الأنظمة المعقدة. فعلى سبيل المثال، في هندسة الحاسوب، يُدمج بين المقاومات والترانزستورات (وتُوصَف باستخدام لغة الدوائر التناظرية) لإنتاج أجزاء مثل بوابات "و" (#en[AND]) وبوابات "أو" (#en[OR])، والتي تشكل الأوليات للغة تصميم الدوائر الرقمية.#footnote[يوصف القسم @sec:circuit-simulator إحدى هذه اللغات.] وتُدمج هذه الأجزاء لبناء المعالجات، وبنى النواقل، وأنظمة الذاكرة، والتي تُدمج بدورها لتشكيل الحواسيب، باستخدام لغات مناسبة لمعمارية الحاسوب. وتُدمج الحواسيب لتشكيل أنظمة موزعة، باستخدام لغات مناسبة لوصف التوصيلات الشبكية، وهكذا.
...
وعملنا أيضاً على مستوى أعلى، معتبرين #py("beside") و #py("below") كأوليات يتم التلاعب بها في لغة تلتقط عملياتها، مثل #py("square_of_four")، الأنماط الشائعة لدمج المدمجات الهندسية.
...
+ غير النمط المنشأ بوساطة #py("corner_split") (على سبيل المثال، باستخدام نسخة واحدة فقط من صور #py("up_split") و #py("right_split") بدلاً من نسختين).
