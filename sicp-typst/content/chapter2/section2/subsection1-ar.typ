...
#emph[المتتالية] (#en[sequence])—وهي مجموعة مرتبة من كائنات البيانات. وهناك بالطبع طرق عديدة لتمثيل المتتاليات باستخدام الأزواج. وتُستعرض إحدى الطرق المباشرة للغاية في
...
#py("pair")
بإنشاء قائمة مترابطة مثل القائمة الأصلية، ولكن مع عنصر إضافي في
البداية.
...
يمكن اعتبار القيمة #py("None")، المستخدمة لإنهاء سلسلة الأزواج، متتاليةً لا تحتوي على عناصر، أي
...
#idx("walking down a linked list with tail") #idx("linked list", sub: "walking down with tail") استخدام #py("tail") تباعاً للمرور على عناصر القائمة المترابطة.
فعلى سبيل المثال، الدالة
#idx("linked list", sub: "nth element of")
#py("llist_ref")
تأخذ كوسائط قائمةً مترابطةً ورقمًا $n$
ترجع العنصر رقم $n$ في القائمة المترابطة.
...
- خلاف ذلك، يجب أن ترجع #py("llist_ref") العنصر رقم $(n-1)$ من #py("tail") القائمة المترابطة.
...
#py("is_none")،
والذي يفحص ما إذا كان وسيطه هو القائمة المترابطة الفارغة.
وتوضح الدالة
#idx("length")
...
تنفذ الدالة #py("length") خطة تعاودية بسيطة. وخطوة الاختزال هي:

- طول (#py("length")) أي قائمة مترابطة هو 1 زائد طول (#py("length")) ذيل (#py("tail")) القائمة المترابطة.

ويُطبَّق ذلك تباعاً حتى نصل إلى الحالة الأساسية:
...
#py("append")، التي تأخذ قائمتين مترابطتين كوسائط وتدمج عناصرهما لإنشاء قائمة مترابطة جديدة:
...
#py("reverse")
تأخذ قائمةً مترابطةً كوسيط وترجع قائمة مترابطة تحتوي على العناصر نفسها ولكن بترتيب معكوس:
...
تأمل برنامج عدّ طرق فك النقود (#idx("counting change")) في القسم @sec:tree-recursion. سيكون من الجيد التمكن من تغيير العملة المستخدمة في البرنامج بسهولة، لكي نتمكن من حساب عدد طرق فك الجنيه الإسترليني، على سبيل المثال. وكما كُتب البرنامج، فإن المعرفة بالعملة تتوزع جزئياً في الدالة
...
نريد إعادة كتابة الدالة #py("cc") بحيث يكون وسيطها الثاني قائمةً مترابطة من قيم القطع النقدية بدلاً من عدد صحيح يحدد القطع المستخدمة. يمكن أن يكون لدينا حينئذ قوائم مترابطة تعرّف كل نوع من العملات:
...
للقيام بذلك، سيتطلب الأمر تغيير برنامج #py("cc") نوعاً ما. وسيكون له الشكل نفسه، لكنه سيتعامل مع وسيطه الثاني بشكل مختلف، على النحو التالي:
...
في وجود الدوال العليا، ليس ضرورياً ضرورة تامة للدوال أن تمتلك معلمات متعددة؛ فمعلمة واحدة تكفي. وإذا كان لدينا دالة مثل #py("plus") تتطلب بطبيعتها وسيطين، يمكننا كتابة تنويعة للدالة نمرر إليها الوسائط واحدًا تلو الآخر. ويمكن أن يؤدي تطبيق التنويعة على الوسيط الأول إلى إرجاع دالة يمكننا تطبيقها بعد ذلك على الوسيط الثاني، وهكذا. وهذه الممارسة—المسماة
#idx("currying")
#emph[الكَرْنَة] (#en[currying]) نسبةً إلى عالم الرياضيات والمنطق الأمريكي
...
اكتب دالة #py("brooks") تأخذ دالةً مكرّنةً كوسيط أول وقائمةً مترابطةً من الوسائط كوسيط ثانٍ، حيث تُطبّق الدالة المكرّنة عليها واحداً تلو الآخر بالترتيب المعطى. فعلى سبيل المثال، ينبغي أن يكون لاستدعاء #py("brooks") التالي التأثير نفسه لـ #py("plus_curried(3)(4)"):
...
إحدى العمليات المفيدة للغاية هي تطبيق تحويل ما على كل عنصر في قائمة مترابطة وإنشاء قائمة مترابطة من النتائج. فعلى سبيل المثال، تقوم الدالة التالية بتغيير مقياس كل رقم في قائمة مترابطة بعامل معين:
...
تأخذ الدالة #py("map") كوسائط دالةً ذات معلمة واحدة وقائمةً مترابطة، وترجع قائمة مترابطة من النتائج الناتجة عن تطبيق الدالة على كل عنصر في القائمة المترابطة:
#idx("map", decl: true)
#snippet(```python
def map(fun, items):
    return (None if is_none(items)
...
الآن يمكننا إعطاء تعريف جديد لـ #py("scale_linked_list") بدلالة #py("map"):
#idx("scalelinkedlist", decl: true)
#snippet(```python
def scale_linked_list(items, factor):
...
الدالة #py("map") بنية مهمة، ليس فقط لأنها تجسد نمطاً شائعاً، بل لأنها تؤسس مستوى أعلى من التجريد في التعامل مع القوائم المترابطة.
في التعريف الأصلي لـ #py("scale_linked_list")، يلفت البناء التعاودي للبرنامج الانتباه إلى معالجة عناصر القائمة المترابطة عنصراً بعنصر.
...
تأخذ الدالة #py("square_linked_list") قائمة مترابطة من الأعداد كوسيط وترجع قائمة مترابطة من مربعات تلك الأعداد.
...
إليك تعريفين مختلفين لـ #py("square_linked_list"). أكملهما بملء التعبيرات الناقصة:
...
ثم يحاول #en[Louis] إصلاح خلله عن طريق المبادلة بين وسيطَي #py("pair"):
...
مشابهة لـ
#py("map").
فهي تأخذ كوسائط دالةً وقائمةً مترابطةً من العناصر. ومع ذلك، بدلاً من تشكيل قائمة مترابطة من النتائج، تقوم #py("for_each") فقط بتطبيق الدالة على كل عنصر بدورها، من اليسار إلى اليمين. والقيم المعادة عن طريق تطبيق الدالة على العناصر لا تُستخدم على الإطلاق—تُستخدم #py("for_each") مع الدوال التي تؤدي إجراءً، مثل الطباعة. فعلى سبيل المثال،
