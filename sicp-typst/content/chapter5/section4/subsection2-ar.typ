...
تتحدد تطبيق الدالة بواسطة تركيب يحتوي على تعبير دالة وتعبيرات وسائط. تعبير الدالة هو تعبير فرعي قيمته دالة، وتعبيرات الوسائط هي تعبيرات فرعية قيمها هي الوسائط التي يجب تطبيق الدالة عليها. يتعامل #py("evaluate") ما فوق الدائري مع التطبيقات عن طريق استدعاء نفسه عوديًا لتقييم كل عنصر في التركيب، ثم تمرير النتائج إلى #py("apply")، التي تنفذ تطبيق الدالة الفعلي. يقوم مُقيِّم التحكم الصريح بنفس الشيء؛ وتُنفذ هذه الاستدعاءات العودية بواسطة تعليمات #py("go_to")، جنبًا إلى جنب مع استخدام المكدس لحفظ المسجّلات التي ستستعاد بعد عودة الاستدعاء العودي.
...
سيؤدي ذلك إلى النتيجة ولكنه قد يحفظ مسجّلات أكثر من اللازم؛ وقد يكون هذا اعتبارًا مهمًا في نظام تكون فيه عمليات المكدس مكلفة. قد يؤدي حفظ المسجّلات التي لن تلزم محتوياتها لاحقًا أيضًا إلى الاحتفاظ ببيانات غير مفيدة كان يمكن تجميعها كمهملات، مما يحرر مساحة لإعادة استخدامها.]

كما هو الحال في المُقيِّم ما فوق الدائري، تُحول تركيبات المُعاملات إلى تطبيقات للدوال الأوّلية المقابلة للمُعاملات. يتم ذلك عند #py("ev_operator_combination")، والتي تنفذ هذا التحويل في مكانه في #py("comp") وتنتقل إلى #py("ev_application").#footnote[نفترض أن محول البناء #py("operator_combination_to_application") متاح كعملية آلة. في تنفيذ فعلي مبني من الصفر، سنستخدم مُقيِّم التحكم الصريح الخاص بنا لتفسير برنامج بايثون ينفذ تحويلات على مستوى المصدر مثل هذا التحويل و #py("function_decl_to_constant_decl") في مرحلة بناء تعمل قبل التنفيذ.]<foot:syntax-transformer>
...
هذا الترتيب لا يحدده المُقيِّم ما فوق الدائري، الذي يرث بنية التحكم الخاصة به من JavaScript الأساسية التي نُفذ بها.#footnote[يحدد ترتيب تقييم تعبير الوسيط بواسطة الدالة #py("list_of_values") في المُقيِّم ما فوق الدائري بواسطة ترتيب تقييم وسائط #py("pair")، والتي تُستخدم لبناء قائمة الوسائط.
...
أوّلية تُنفذ للحصول على وسائطها من #py("argl") ووضع نتيجتها في #py("val"). لتحديد كيفية التعامل الآلة مع الأوّليات، كان سيتعين علينا تقديم تسلسل من تعليمات وحدة التحكم لتنفيذ كل أوّلية والترتيب لـ #py("primitive_apply") ليوجه إلى التعليمات الخاصة بالأوّلية المحددة بمحتويات #py("fun"). نظرًا لأننا مهتمون ببنية عملية التقييم بدلاً من تفاصيل الأوّليات، فسنستخدم بدلاً من ذلك عملية #py("apply_primitive_function") تطبق الدالة في fun على الوسائط في #py("argl"). لأغراض محاكاة المُقيِّم باستخدام المحاكي في القسم @sec:simulator، نستخدم الدالة #py("apply_primitive_function")، التي تستدعي نظام JavaScript الأساسي لتنفيذ التطبيق، كما فعلنا للمُقيِّم ما فوق الدائري في القسم @sec:core-of-evaluator. بعد حساب قيمة التطبيق الأوّلي، نستعيد #py("continue") وننتقل إلى نقطة الإدخال المحددة.
...
الدوال المركبة. لتطبيق دالة مركبة، نواصل بطريقة مماثلة لما فعلناه في المُقيِّم ما فوق الدائري. نبني إطارًا يربط معاملات الدالة بالوسائط، ونستخدم هذا الإطار لتوسيع البيئة المحمولة بواسطة الدالة، ونُقيّم جسم الدالة في هذه البيئة الممتدة.

في هذه النقطة تكون الدالة المركبة في المسجّل #py("fun") وتكون وسائطها في #py("argl"). نستخرج معاملات الدالة في #py("unev") وبيئتها في #py("env"). ثم نستبدل البيئة في #py("env") بالبيئة المبنية بتوسيعها بروابط المعاملات بالوسائط المعطاة. ثم نستخرج جسم الدالة في #py("comp").
...
إرجاع من الدالة إرجاع قيمة تعبير الإرجاع كـ قيمة للجسم. ولكن قد تكون تعليمة الإرجاع متداخلة بشكل عشوائي عميق في الجسم؛ لذا فإن المكدس في اللحظة التي تظهر فيها تعليمة الإرجاع ليس بالضرورة هو المكدس المطلوب للإرجاع من الدالة. إحدى الطرق لجعل من الممكن تعديل المكدس للإرجاع هي وضع #emph[علامة] (#en[marker]) على المكدس يمكن العثور عليها بواسطة كود الإرجاع. يُنفذ هذا بواسطة التعليمة
...
لاستعادة المكدس إلى المكان المشار إليه بواسطة العلامة قبل تقييم تعبير الإرجاع.#footnote[التعليمات الخاصة #py("push_marker_to_stack") و #py("revert_stack_to_marker") ليست ضرورية تمامًا وكان يمكن تنفيذها عن طريق دفع ودفع قيمة علامة صراحة على المكدس. أي شيء لا يمكن الخلط بينه وبين قيمة في البرنامج يمكن استخدامه كـ علامة. انظر التمرين @ex:push_marker_to_stack1.]
...
#idx("register-machine language", sub: "revertstacktomarker")
#idx("revertstacktomarker (in register machine)")
#py("revert_stack_to_marker") كـ تعليمات آلة مسجّلات، متبعًا تنفيذ #py("save") و #py("restore") في القسم @sec:ex-proc. أضف الدالتين #py("push_marker") و #py("pop_marker") للوصول إلى المكدسات، محاكيًا تنفيذ #py("push") و #py("pop") في القسم @sec:machine-model. لاحظ أنك لا تحتاج إلى إدراج علامة فعلية في المكدس. بدلاً من ذلك، يمكنك إضافة متغير حالة محلي إلى نموذج المكدس لتتبع موقع آخر #py("save") قبل كل #py("push_marker_to_stack").
