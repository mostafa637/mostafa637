...
بدأنا بنموذج التعويض غير الرسمي نسبياً، ثم مددناه في الفصل @chap:state إلى نموذج البيئة، والذي مكننا من التعامل مع الحالة والتغيير. في المُقيِّم ما فوق الدائري في الفصل @chap:meta، استخدمنا #en[Python] نفسها كلغة لجعل بنية البيئة المبنية أثناء تقييم مكون أكثر صراحة.
...
تشكل التعليمات التالية بداية تسلسل وحدة التحكم لمُقيِّم التحكم الصريح:#footnote[نفترض هنا أن #py("user_read") و #py("parse") والعمليات الطباعية المختلفة متاحة كعمليات آلة أوّلية، وهو أمر مفيد لمُحاكاتنا، ولكنه غير واقعي تمامًا في الممارسة العملية. هذه في الواقع عمليات معقدة للغاية. في الممارسة العملية، فإن القراءة والطباعة ستُنفذ باستخدام عمليات إدخال وإخراج منخفضة المستوى مثل نقل الأحرف المنفردة من وإلى جهاز.]
...
يجب علينا تعريف دوال #en[Python] لمُحاكاة العمليات المستخدمة كأوّليات بواسطة المُقيِّم. هذه هي نفس الدوال التي استخدمناها للمُقيِّم ما فوق الدائري في القسم @sec:mc-eval، جنبًا إلى جنب مع عدد قليل من الدوال الإضافية المحددة في الحواشي السفلية في جميع أنحاء القسم @sec:eceval.
...
+ أعطِ صيغة لإجمالي عدد عمليات الدفع المستخدمة لحساب $"Fib"(n)$ لـ $n \ge 2$. يجب أن تجد أن عدد عمليات الدفع (والذي يتناسب جيداً مع الوقت المستخدم) ينمو أسياً مع $n$. تلميح: لتكن $S(n)$ عدد عمليات الدفع المستخدمة في حساب $"Fib"(n)$. يجب أن تكون قادرًا على الجدال بأن هناك صيغة تعبر عن $S(n)$ بدلالة $S(n-1)$ و $S(n-2)$ وبعض الثابت غير المباشر $k$ المستقل عن $n$. أعطِ الصيغة، واذكر ما هو $k$. ثم أظهر أنه يمكن التعبير عن $S(n)$ كـ $a "Fib"(n+1) + b$ وأعطِ قيم $a$ و $b$.
])
...
+ الأسوأ من ذلك بكثير هو مشكلة التعامل مع الأخطاء التي يصدر إشارة بها تطبيق الدوال الأوّلية مثل محاولة القسمة على صفر أو محاولة استخراج الـ #py("head") لسلسلة نصية. في نظام مكتوب بشكل احترافي وعالي الجودة، يتم فحص كل تطبيق أوّلي للأمان كجزء من الأوّلية. على سبيل المثال، يمكن لكل استدعاء لـ #py("head") فحص ما إذا كان الوسيط زوجًا أولاً. إذا لم يكن الوسيط زوجًا، فسيرجع التطبيق كود حالة متميز إلى المُقيِّم، والذي سيرفع الفشل عندئذٍ. يمكننا الترتيب لذلك في محاكي آلة المسجّلات الخاص بنا عن طريق جعل كل دالة أوّلية تفحص القابلية للتطبيق وتُرجع كود حالة متميز مناسب عند الفشل. عندئذٍ يمكن لكود #py("primitive_apply") في المُقيِّم فحص كود الحالة والانتقال إلى #py("signal_error") إذا لزم الأمر. ابنِ هذه البنية واجعلها تعمل. هذا مشروع رئيسي.
])
