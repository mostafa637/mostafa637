...
لاحظ كيف تستخدم #py("ev_lambda") المسجّلين #py("unev") و #py("comp") لحفظ المعاملات وجسم تعبير لامدا حتى يمكن تمريرهما إلى عملية #py("make_function")، إلى جانب البيئة في #py("env").

#idx("explicit-control evaluator for Python", sub: "expressions with no subexpressions to evaluate")

#subheading([الشرطيات])

كما هو الحال مع المُقيِّم ما فوق الدائري، تتم معالجة الأشكال النحوية عن طريق تقييم أجزاء من المكون بشكل انتقائي. بالنسبة لـ
...
جزء مُقيِّم التحكم الصريح الذي يبدأ عند #py("ev_sequence")، والذي يتعامل مع تسلسلات العبارات، يناظر دالة #py("eval_sequence") للمُقيِّم ما فوق الدائري.

الإدخالان عند #py("ev_sequence_next") و #py("ev_sequence_continue") يشكلان حلقة تُقيّم بالتتابع كل عبارة في تسلسل.
تُمكث قائمة العبارات غير المُقيّمة في #py("unev").
عند #py("ev_sequence") نضع تسلسل العبارات المراد تقييمها في #py("unev"). إذا كان التسلسل فارغًا، نضبط #py("val") على #py("undefined") ونقفز إلى #py("continue") عبر #py("ev_sequence_empty"). بخلاف ذلك نبدأ حلقة تقييم التسلسل، أولاً بحفظ قيمة #py("continue") على المكدس، لأن المسجّل #py("continue") سيُستخدم للتدفق المحلي للتحكم في الحلقة، والقيمة الأصلية مطلوبة للاستمرار بعد تسلسل العبارات. قبل تقييم كل عبارة، نفحص ما إذا كانت هناك عبارات إضافية مراد تقييمها في التسلسل. إذا كان الأمر كذلك، نحفظ باقي العبارات غير المُقيّمة (الموجودة في #py("unev")) والبيئة التي يجب تقييمها فيها (الموجودة في #py("env")) ونستدعي #py("eval_dispatch") لتقييم العبارة التي وضعت في #py("comp"). يستعاد المسجّلان المحفوظان بعد هذا التقييم، عند #py("ev_sequence_continue").
...
على عكس #py("eval_sequence") في المُقيِّم ما فوق الدائري، لا تحتاج #py("ev_sequence") إلى فحص ما إذا تم تقييم تعليمة إرجاع من أجل إنهاء تقييم التسلسل. "التحكم الصريح" في هذا المُقيِّم يسمح لتعليمة الإرجاع بالقفز مباشرة إلى مواصلة تطبيق الدالة الحالي دون استئناف تقييم التسلسل. وبالتالي لا يحتاج تقييم التسلسل إلى الاهتمام بالإرجاعات، أو حتى الإلمام بوجود تعليمات إرجاع في اللغة. نظرًا لأن تعليمة الإرجاع تقفز خارج كود تقييم التسلسل، فإن استعادات المسجّلات المحفوظة عند #py("ev_sequence_continue") لن تنفذ. سنرى لاحقًا كيف تزيل تعليمة الإرجاع هذه القيم من المكدس.
