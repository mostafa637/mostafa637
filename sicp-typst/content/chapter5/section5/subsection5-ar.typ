...
ولا نهتم بما يفعله الكود المُترجَم بعد تنفيذ الإعلان، لذا فإن اختيارنا لـ #py("\"next\"") كـ واصف ارتباط هو أمر عشوائي.
...
تتعامل الدالة #py("compile") مع هذا باستدعاء #py("compile_lambda_expression")، والتي تترجم جسم الدالة، وتظلله كـ نقطة إدخال جديدة، وتولد التعليمة التي ستدمج جسم الدالة عند نقطة الإدخال الجديدة مع بيئة وقت التشغيل وتسند النتيجة إلى #py("val"). ثم يتخطي التسلسل حول كود الدالة المُترجَم، والذي يُدرج عند هذه النقطة. يبدأ كود الدالة نفسه بتوسيع بيئة إعلان الدالة بإطار يربط المعامل #py("n") بوسيط الدالة. ثم يأتي جسم الدالة الفعلي. نظرًا لأن هذا الكود لقيمة الاسم لا يعدل المسجّل #py("env")، فإن عمليتي الحفظ والاستعادة الاختياريتين الموضحتين أعلاه لا تُولدان. (الكود الخاص بالدالة عند #py("entry1") لا يُنفذ عند هذه النقطة، لذا فإن استخدامه لـ #py("env") غير ذي صلة).
...
ترجم هذه الدالة وقارن الكود الناتج مع ذلك المـُنتج لـ #py("factorial"). اشرح أي اختلافات تجدها. هل ينفذ أي من البرنامجين بكفاءة أكبر من الآخر؟
])
...
ما هو البرنامج الذي تـُمّ تجميعه لإنتاج الكود الموضح في الشكل @fig:compilation-example1؟
])
...
ترجم بعض التعبيرات البسيطة وحدد عمليات المكدس غير الضرورية المـُولدة.
قارن الكود بذلك المـُنتج مع بقاء آلية #py("preserving") سليمة.
])
