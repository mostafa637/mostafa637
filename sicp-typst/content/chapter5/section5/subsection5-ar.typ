...
تتعامل الدالة #py("compile") مع هذا باستدعاء #py("compile_lambda_expression")، والتي تترجم جسم الدالة، وتظلله كنقطة إدخال جديدة، وتولد التعليمة التي ستدمج جسم الدالة عند نقطة الإدخال الجديدة مع بيئة وقت التشغيل وتسند النتيجة إلى #py("val"). ثم يتخطي التسلسل حول كود الدالة المُترجَم، والذي يُدرج عند هذه النقطة. يبدأ كود الدالة نفسه بتوسيع بيئة إعلان الدالة بإطار يربط المعامل #py("n") بوسيط الدالة. ثم يأتي جسم الدالة الفعلي. نظرًا لأن هذا الكود لقيمة الاسم لا يعدل المسجّل #py("env")، فإن عمليتي الحفظ والاستعادة الاختياريتين الموضحتين أعلاه لا تُولدان. (الكود الخاص بالدالة عند #py("entry1") لا يُنفذ عند هذه النقطة، لذا فإن استخدامه لـ #py("env") غير ذي صلة).
