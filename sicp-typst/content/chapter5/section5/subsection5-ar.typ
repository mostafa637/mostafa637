...
ثم يختبر ما إذا كانت #py("fun") تحتوي على دالة أولية أم دالة مركبة، ويوجه إلى فرع أولي أو فرع مركب بناءً على ذلك.
يستأنف كلا الفرعين عند التسمية #py("after_call").
يجب على الفرع المركب إعداد #py("continue") للقفز إلى ما بعد الفرع الأولي ودفع علامة إلى المكدس لمطابقة عملية التراجع في تعليمة الإرجاع المُترجَمة للدالة.
...
كل من هذه الاستدعاءات يقيم #py("fun") و #py("argl") وفرعيه الأولي والمركب الخاصين به. يُظهر الشكل @fig:comp-factorial1 الترجمة الكاملة لإعلان دالة #py("factorial").
...
مُترجِمنا ذكي بشأن تجنب عمليات المكدس غير الضرورية، ولكنه ليس ذكيًا على الإطلاق عندما يتعلق الأمر بترجمة استدعاءات الدوال الأولية في اللغة بدلالة العمليات الأولية الموفرة بواسطة الآلة. على سبيل المثال، ضع في اعتبارك كمية الكود المُترجَم لحساب #py("a + 1"):
يعد الكود قائمة وسائط في #py("argl")، ويضع دالة الجمع الأولية (التي يجدها بالبحث عن الرمز #py("\"+\"") في البيئة) في #py("fun")، ويختبر ما إذا كانت الدالة أولية أم مركبة. يولد المُترجِم دائمًا كودًا لتنفيذ الاختبار، بالإضافة إلى كود للفرعين الأولي والمركب (واحدهما فقط سُينفذ).
لم نُظهر الجزء من وحدة التحكم الذي ينفذ الأوليات، لكننا نفترض أن هذه التعليمات تستخدم عمليات الحساب الأولية في مسارات بيانات الآلة. ضع في اعتبارك كم كان كود أقل سيُولد إذا تمكن المُترجِم من
#idx("compiler for Python", sub: "open coding of primitives")
#idx("open coding of primitives")
#emph[الترميز المفتوح] (#en[open-code]) للأوليات—أي، إذا استطاع توليد كود لاستخدام عمليات الآلة الأولية هذه مباشرة. التعبير #py("a + 1") قد يُترجم إلى شيء بسيط كالتالي:#footnote[استخدمنا نفس الرمز #py("+") هنا للإشارة إلى دالة اللغة المصدر وعملية الآلة كلاهما. بشكل عام لن يكون هناك تناظر واحد لواحد بين أوليات اللغة المصدر وأوليات الآلة.]
...
في هذا التمرين سنوسع مُترجِمنا لدعم الترميز المفتوح لأوليات محددة. سُيولد كود مخصص الأغراض لاستدعاءات هذه الدوال الأولية بدلاً من كود تطبيق الدوال العام. ولدعم ذلك، سنعزز آلتنا بمسجّلات وسائط خاصة #py("arg1") و #py("arg2").
تأخذ عمليات الحساب الأولية للآلة مدخلاتها من #py("arg1") و #py("arg2"). وقد توضع النتائج في #py("val") أو #py("arg1") أو #py("arg2").

يجب أن يكون المُترجِم قادرًا على التعرف على تطبيق أولية ذات ترميز مفتوح في البرنامج المصدر. سنعزز التوجيه في الدالة #py("compile") للتعرف على أسماء هذه الأوليات بالإضافة إلى الأشكال النحوية التي يتعرف عليها حاليًا.
لكل شكل نحوي يملك مُترجِمنا مولد كود. في هذا التمرين سنبني عائلة من مولدات الكود للأوليات ذات الترميز المفتوح.

+ الأوليات ذات الترميز المفتوح، على عكس الأشكال النحوية، تحتاج جميعها إلى تقييم تعبيرات وسائطها. اكتب مولد كود #py("spread_arguments") للاستخدام بواسطة جميع مولدات كود الترميز المفتوح. ينبغي أن تأخذ الدالة #py("spread_arguments") قائمة بتعبيرات الوسائط وتترجم تعبيرات الوسائط المعطاة موجهة إلى مسجّلات وسائط متتالية. لاحظ أن تعبير الوسيط قد يحتوي على استدعاء لأولية ذات ترميز مفتوح، لذا يجب حفظ مسجّلات الوسائط أثناء تقييم تعبير الوسيط.
+ معامِلات بايثون #py("===") و #py("*") و #py("-") و #py("+")، بين أمور أخرى، تُنفذ في آلة المسجّلات كدوال أولية ويُشار إليها في البيئة العالمية بالرموز #py("\"===\"") و #py("\"*\"") و #py("\"-\"") و #py("\"+\""). في بايثون، لا يمكن إعادة إعلان هذه الأسماء، لأنها لا تستوفي القيود النحوية للأسماء. هذا يعني أنه من الآمن ترميزها بشكل مفتوح. لكل من الدوال الأولية #py("===") و #py("*") و #py("-") و #py("+")، اكتب مولد كود تأخذ تطبيقًا مع تعبير دالة يسمي تلك الدالة، جنبًا إلى جنب مع هدف وواصف ارتباط، وتُنتج كودًا لنشر الوسائط في المسجّلات ثم تنفيذ العملية موجهة إلى الهدف المعطى مع الارتباط المعطى. اجعل #py("compile") توجه إلى مولدات الكود هذه.
