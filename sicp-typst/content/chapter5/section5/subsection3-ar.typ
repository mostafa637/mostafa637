...
بعد تقييم عناصر تطبيق الدالة، يجب على الكود المُترجَم تطبيق الدالة في #py("fun") على الوسائط في #py("argl"). ينفذ الكود التوجيه نفسه بالأساس مثل الدالة #py("apply") في المُقيِّم فوق الدائري (#en[metacircular]) في القسم @sec:core-of-evaluator أو نقطة الإدخال #py("apply_dispatch") في مُقيِّم التحكم الصريح في القسم @sec:procedure-application. حيث يفحص ما إذا كانت الدالة المراد تطبيقها دالة أولية أم دالة مُترجَمة.
بالنسبة للدالة الأولية، يستخدم #py("apply_primitive_function")؛ وسنرى قريباً كيف يتعامل مع الدوال المُترجَمة.
...
لاحظ أنه يجب على الفرع المُترجَم التخطي حول الفرع الأولي. لذلك، إذا كان الارتباط لاستدعاء الدالة الأصلي هو #py("\"next\"")، يجب أن يستخدم الفرع المركب ارتباطًا يقفز إلى تسمية تم إدراجها بعد الفرع الأولي.
...
الفرعان الأولي والمركب، مثل فرعي الصحة والخطأ في #py("compile_conditional")، يُلحقان باستخدام #py("parallel_instruction_sequences") بدلاً من #py("append_instruction_sequences") العادية، لأنهما لن يُنفذا تتابعيًا.
...
أو سيبدو هكذا—حافظًا مواصلة المستدعي عند البداية من أجل استعادتها والذهاب إليها عند النهاية—إذا كان الارتباط #py("\"return\"") (أي إذا كان التطبيق في تعليمة إرجاع وكانت قيمته هي النتيجة المراد إرجاعها):
...
افترض بدلاً من ذلك أننا تعاملنا مع حالة استدعاء دالة مع ارتباط #py("\"return\"") وهدف #py("val") بنفس الطريقة كما بالنسبة لهدف ليس #py("val"). كان هذا يدمر العودية الذيلية. كان نظامنا سيظل يُرجع استدعاء الدالة. ولكن في كل مرة نستدعي فيها دالة، كنا سنحفظ #py("continue") ونعود بعد الاستدعاء لـ إلغاء الحفظ (غير المفيد). كانت عمليات الحفظ الإضافية هذه ستتراكم أثناء تداخل استدعاءات الدوال.#footnote[جعل
#idx("compiler", sub: "tail recursion, stack allocation, and garbage-collection")
المُترجِم يولد كودًا عوديًا ذيليًا هو أمر مرغوب فيه، خاصة في النمط الوظيفي. ومع ذلك، فإن المُترجِمات للغات الشائعة، بما في ذلك C و C++، لا تفعل ذلك دائمًا، وبالتالي لا يمكن لهذه اللغات تمثيل العمليات التكرارية بدلالة استدعاء الدالة وحده. #idx("tail recursion", sub: "garbage collection and")
#idx("garbage collection", sub: "tail recursion and")
#idx("stack allocation and tail recursion")
الصعوبة مع العودية الذيلية في هذه اللغات هي أن تنفيذاتها تستخدم المكدس لتخزين وسائط الدوال والأسماء المحلية بالإضافة إلى عناوين العودة. تنفيذات بايثون الموصوفة في هذا الكتاب تخزن الوسائط والأسماء في الذاكرة لجمع القمامة. والسبب في استخدام المكدس للأسماء والوسائط هو أنه يتجنب الحاجة إلى جمع القمامة في اللغات التي لن تتطلب ذلك بخلاف ذلك، ويُعتقد عموماً أنه أكثر كفاءة. يمكن للمُترجِمات المتطورة، في الواقع، استخدام المكدس للوسائط دون تدمير العودية الذيلية. (انظر
#idx("Hanson, Christopher P.")
هانسون 1990 لوصف ذلك). هناك أيضًا بعض النقاش حول ما إذا كان تخصيص المكدس أكثر كفاءة في الواقع من جمع القمامة في المقام الأول، ولكن التفاصيل يبدو أنها تعتمد على نقاط دقيقة في بنية الحاسوب. (انظر
#idx("Appel, Andrew W.")
أبل 1987
و
#idx("Miller, James S.")
#idx("Rozas, Guillermo Juan")
ميلر وروتساس 1994 لوجهات نظر متعارضة حول هذه المسألة).]
...
أوضحنا كيفية توليد كود ارتباط عودي ذيلي لتطبيق دالة عندما يكون الارتباط هو #py("\"return\"")—أي عندما يكون التطبيق في تعليمة إرجاع وقيمته هي النتيجة المراد إرجاعها. وبالمثل، كما تم شرحه في القسم @sec:procedure-application، فإن آلية علامة المكدس المستخدمة هنا (وفي مُقيِّم التحكم الصريح) للاستدعاء والإرجاع تُنتج
#idx("return statement", sub: "tail recursion and")
#idx("tail recursion", sub: "return statement necessary for")
سلوكًا عوديًا ذيليًا في تلك الحالة فقط. هاتان الناحيتان من الكود المُولد لتطبيق الدالة تتحدان لضمان أنه عندما تنتهي دالة بإرجاع قيمة استدعاء دالة، فإنه لا يتراكم أي مكدس.
...
تعليمات التراجع عن المكدس باستخدام العلامة ثم استعادة #py("continue") تناظر التعليمات المُولدة بواسطة #py("compile_fun_appl") لحفظ #py("continue") وتظليل المكدس.
