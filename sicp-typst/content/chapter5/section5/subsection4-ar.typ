...
وصف هذا القسم تفاصيل كيفية تمثيل وتجميع تسلسلات التعليمات. تذكر من القسم @sec:instruction-sequences أن تسلسل التعليمات يُمثل كقائمة بالمسجّلات المطلوبة، والمسجّلات المـُعدلة، والتعليمات الفعلية. سنعتبر أيضًا التسمية (سلسلة نصية) حالة متدهورة لتسلسل تعليمات، لا تحتاج أو تعدل أي مسجّلات.
لذا لتحديد المسجّلات المطلوبة والمـُعدلة بواسطة تسلسلات التعليمات نستخدم المُحددات:
...
النقطة الدقيقة هي تحديد المسجّلات المطلوبة والمـُعدلة بواسطة التسلسل الناتج.
...
تُعطى الدالة #py("append_instruction_sequences") تسلسلي تعليمات #py("seq1") و #py("seq2") وتُرجع تسلسل التعليمات الذي تعليماته هي تعليمات #py("seq1") متبوعة بتعليمات #py("seq2")، والذي مسجّلاته المـُعدلة هي تلك المسجّلات التي تُعدل بواسطة إما #py("seq1") أو #py("seq2")، والذي مسجّلاته المطلوبة هي المسجّلات المطلوبة بواسطة #py("seq1") جنبًا إلى جنب مع تلك المسجّلات المطلوبة بواسطة #py("seq2") والتي لا تُعدل بواسطة #py("seq1"). (بدلالة عمليات المجموعات، فإن المجموعة الجديدة من المسجّلات المطلوبة هي اتحاد مجموعة المسجّلات المطلوبة بواسطة #py("seq1") مع الفرق المجموعي للمسجّلات المطلوبة بواسطة #py("seq2") والمسجّلات المـُعدلة بواسطة #py("seq1")). وبالتالي، فإن #py("append_instruction_sequences") تُنفذ كما يلي:
...
تستخدم هذه الدالة بعض العمليات البسيطة لمعالجة المجموعات المُمثلة كـ قوائم، بشكل يشبه تمثيل المجموعات (غير المرتبة) الموصوف في القسم @sec:representing-sets:
