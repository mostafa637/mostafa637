...
#idx("tagged data")
#idx("data", sub: "tagged")
"البيانات المعلّمة" (#en[tagged data]) نفسها التي قدمناها في الفصل @chap:data للتعامل مع العمليات العامة. هنا، ومع ذلك، يتم تضمين أنواع البيانات عند مستوى الآلة الأولي بدلاً من بنائها من خلال استخدام القوائم.
...
#idx("type field")
#emph[حقل نوع] (#en[type field]) يفرّغ نوع البيانات. تشمل الأسئلة المهمة التي يجب معالجتها في تصميم مثل هذا التمثيل ما يلي: كم عدد بتات النوع المطلوبة؟ ما مدى كبر أدلة الأشعة التي يجب أن تكون؟ ما مدى كفاءة استخدام تعليمات الآلة الأولية لمعالجة حقول النوع للمؤشرات؟ يُقال عن الآلات التي تتضمن أجهزة خاصة للتعامل الكفء مع حقول النوع إنها تملك
#idx("tagged architecture")
#emph[بنى معلّمة] (#en[tagged architectures]).] يُمكّن نوع البيانات النظام من التمييز بين مؤشر إلى زوج (والذي يتكون من نوع البيانات "زوج" ودليل في أشعة الذاكرة) والمؤشرات إلى أنواع أخرى من البيانات (والتي تتكون من نوع بيانات آخر وأي شيء يُستخدم لتمثيل البيانات من ذلك النوع). يُعتبر كائنا بيانات
...
قد تُمثَّل كمؤشر معلّم النوع يحدد تسلسلاً من الأحرف التي تشكل التمثيل المطبوع للسلسلة. يبني المحلل مثل هذا التسلسل عندما يواجه سلسلة نصية صريحة، والمُعامل التجميعي للسلاسل النصية #py("+") والدوال الأولية المنتجة للسلاسل مثل
#py("stringify")
...
#subheading([تنفيذ عمليات القائمة الأولية])

#anchor(<sec:impl-list-ops>)

بالنظر إلى مخطط التمثيل أعلاه، يمكننا استبدال كل عملية قائمة "أولية" لآلة مسجّلات بعملية شعاع أولية واحدة أو أكثر. سنستخدم مسجّلين،
...
و
#py("vector_set")
متاحان كعمليات أولية. نفترض أيضًا أن العمليات العددية على المؤشرات (مثل زيادة مؤشر، أو استخدام مؤشر زوج لدليل شعاع، أو إضافة عددين) تستخدم فقط جزء الدليل من المؤشر معلّم النوع.
...
إذا قمنا بتنفيذ هذه، على التوالي، كالتالي:
...
#idx("free list")
#emph[قائمة حرة] (#en[free list]). مواقعنا الحر متتالية (وبالتالي يمكن الوصول إليها عن طريق زيادة مؤشر) لأننا نستخدم جامع قمامة مُضغِّطًا، كما سنرى في القسم @sec:gc.]
...
ويمكن تنفيذ #py("perform(list(op(\"initialize_stack\")))") كالتالي:
...
افترض أن عمليات ذاكرة بنية القائمة متاحة كأوليات آلة.
...
افترض أن عمليات ذاكرة بنية القائمة متاحة كعمليات أولية.
])
