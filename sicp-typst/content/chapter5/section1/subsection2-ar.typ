...
غالبًا ما نحدد آلة لتشمل عمليات "أوّلية" تكون في الواقع معقدة جدًا. على سبيل المثال، في القسمين @sec:eceval و @sec:compilation سنتعامل مع معالجات البيئة الخاصة ببايثون كعمليات أوّلية. مثل هذا التجريد (abstraction) ذو قيمة لأنه يسمح لنا بتجاهل تفاصيل أجزاء من الآلة حتى نتمكن من التركيز على جوانب أخرى من التصميم. ومع ذلك، فإن حقيقة أننا أخفينا الكثير من التعقيد لا تعني أن تصميم الآلة غير واقعي. يمكننا دائمًا استبدال الـ "أوّليات" المعقدة بعمليات أوّلية أبسط.

ضع في اعتبارك آلة GCD. تحتوي الآلة على تعليمة تحسب باقي قسمة محتويات المسجّلين #py("a") و #py("b") وتسند النتيجة إلى المسجّل #py("t"). إذا أردنا بناء آلة GCD دون استخدام عملية باقي أوّلية، فيجب علينا تحديد كيفية حساب البواقي بدلالة عمليات أبسط، مثل الطرح.
...
ابدأ بافتراض أن العمليتين #py("is_good_enough") و #py("improve") متوفرتان كعمليات أوّلية. ثم وضح كيف يمكن توسيعها بدلالة العمليات الحسابية. صِف كل إصدار من تصميم آلة #py("sqrt") عن طريق رسم مخطط مسار بيانات وكتابة تعريف وحدة التحكم في لغة آلة المسجّلات.
])
