...
يُنجَز إنقاص #py("balance") عن طريق الإسناد
...
تغير إعادة الإسناد #meta("name") بحيث تصبح قيمته هي النتيجة التي تُحصَل عليها بتقييم #meta("new-value"). في الحالة المطروحة، نحن نغير #py("balance") بحيث تصبح قيمته الجديدة هي نتيجة طرح #py("amount") من القيمة السابقة لـ #py("balance").#footnote[تبدو تعليمات إعادة الإسناد وإسنادات التصريح متشابهة ويجب ألا يُخلط بينها وبين
...
الدالة التالية، #py("make_withdraw")، تنشئ "معالجات سحب". تحدد المعلمة #py("balance") في #py("make_withdraw") المبلغ الأولي للمال في الحساب.#footnote[على عكس #py("make_withdraw_balance_100") أعلاه، لا يتعين علينا استخدام إسناد تصريحي لجعل #py("balance") متغيراً محلياً، نظرًا لأن المعلمات هي بالفعل محلية. سيصبح هذا أكثر وضوحاً بعد مناقشة نموذج البيئة للتقييم في القسم @sec:environment-model. (انظر أيضاً التمرين @ex:local-state-variable).]<foot:make_withdraw>
...
تقوم كل مكالمة لـ #py("make_account") بإعداد بيئة تحتوي على متغير حالة محلي #py("balance"). داخل هذه البيئة، تُعرّف #py("make_account") دوال #py("deposit") و #py("withdraw") التي تصل إلى #py("balance")، ودالة إضافية #py("dispatch") تأخذ "رسالة" كدخل وترجع إحدى الدالتين المحليتين. الدالة #py("dispatch") نفسها تُرجَع كقيمة تمثل كائن الحساب البنكي. هذا بالضبط هو أسلوب
...
ترجع كل مكالمة لـ #py("acc") الدالة المعرفة محلياً #py("deposit") أو #py("withdraw")، والتي تُطبَّق بعد ذلك على الـ #py("amount") المحدد. كما كان الحال مع #py("make_withdraw")، فإن مكالمة أخرى لـ #py("make_account")
...
#idx("accumulator")
#emph[المُراكِم] (#en[accumulator]) هو دالة تُستدعى مرارًا وتكرارًا بوسيط رقمي واحد وتقوم بتراكم وسائطها في مجموع. في كل مرة تُستدعى فيها، ترجع المجموع المتراكم حالياً. اكتب دالة
#idx("makeaccumulator")
#py("make_accumulator")
