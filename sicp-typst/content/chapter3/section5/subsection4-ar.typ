...
حلقات تغذية راجعة. تـُمثل حلقة التغذية الراجعة للجامع الموضحة في الشكل @fig:integral بالحقيقة القائلة إن
#idx("integral", sub: "need for delayed evaluation")
التدفق الداخلي لـ #py("integral")،
#py("integ")،
تـُمّ تعريفه بدلالة نفسه:
...
وبشكل عام، فإن مثل هذا التأخير حاسم لاستخدام التدفقات لنمذجة أنظمة معالجة الإشارات التي تحتوي على حلقات. ودون تأخير، سيتعين صياغة نماذجنا بحيث تـُقيَّم مدخلات أي مكون معالجة إشارات بشكل كامل قبل إمكانية إنتاج الخرج. وكان هذا ليمنع الحلقات.
...
المعادلة التفاضلية $d y/d t=f(y)$ حيث $f$ دالة معطاة. يوضح الشكل مكون تعيين، يطبق $f$ على إشارة دخله، مرتبطاً في حلقة تغذية راجعة بمُكامل بطريقة مشابهة جداً لتلك الخاصة بدوائر الحاسوب التناظري التي تـُستخدم بالفعل لحل مثل هذه المعادلات.
...
عدل الدالة بحيث تتوقع #py("integrand") كـ وسيط مؤجل وبالتالي يمكن استخدامها في الدالة #py("solve") الموضحة أعلاه.
])
...
من مقاومة، ومكثف، ومحث موصلة على التوالي، كما هو موضح في الشكل @fig:series-rlc. إذا كانت $R$ و $L$ و $C$ هي المقاومة، والمحثية، والسعة، فإن العلاقات بين الجهد ($v$) والتيار ($i$) للمكونات الثلاثة تـُوصف بالمعادلات:
...
ويوضح دمج هذه المعادلات أن حالة الدائرة (الملخصة بـ $v_(C)$، الجهد عبر المكثف، و $i_(L)$، التيار في المحث) تـُوصف بـ زوج المعادلات التفاضلية:
...
بل إننا سنحتاج إلى دالة تعيين مختلفة لكل تركيبة مختلفة من أنواع بيانات الوسائط والنتائج التي قد تـُحدد لـ #py("fun").
...
(جوردون، وميلنر، وواذسوورث 1979)، والتي تتضمن "أنواع بياناتها المتعددة الأشكال بارامترياً" قوالب للتحويلات عالية الرتبة بين أنواع البيانات. علاوة على ذلك، فإن أنواع البيانات لمعظم الدوال في ML لا يـُعلن عنها المبرمج صراحة أبداً. وبدلاً من ذلك، تتضمن ML آلية
...
إحدى الطرق لتجنب الحاجة إلى فئتين مختلفين من الدوال هي جعل جميع الدوال تأخذ وسائط مؤجلة. ويمكننا تبني نموذج تقييم تـُأجَّل فيه جميع الوسائط للدوال تلقائياً وتـُجبر الوسائط فقط عندما تكون مطلوبة بالفعل (على سبيل المثال، عندما تتطلبها عملية أوليّة). وسيحول هذا لغتنا لاستخدام تقييم الترتيب العادي، والذي وصفناه لأول مرة عندما قدمنا نموذج الاستبدال للتقييم في القسم @sec:substitution-model.
إن التحويل إلى تقييم الترتيب العادي يوفر طريقة موحدة وأنيقة لتبسيط استخدام التقييم المؤجل، وسيكون هذا استراتيجية طبيعية نتبناها إذا كنا نهتم فقط بمعالجة التدفقات. في القسم @sec:lazy-evaluation، وبعد أن ندرس المـُقيِّم، سنرى كيفية تحويل لغتنا بهذه الطريقة بالضبط.
