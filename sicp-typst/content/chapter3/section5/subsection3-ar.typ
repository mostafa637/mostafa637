...
ليونارد أويلر (#en[Leonhard Euler])، يعمل بشكل جيد مع التسلسلات التي هي مجاميع جزئية لمتسلسلات متناوبة (متسلسلات ذات حدود متناوبة الإشارات). في تقنية أويلر، إذا كان $S_(n)$ هو الحد الـ $n$ لتسلسل المجموع الأصلي، فإن التسلسل المُسرّع يحتوي على الحدود:

$ S_(n+1) - frac((S_(n+1)-S_(n))^(2), S_(n-1)-2S_(n)+S_(n+1)) $

وهكذا، إذا تُمّ تمثيل التسلسل الأصلي كتدفق قيم، فإن التسلسل المُتحوّل يُعطى بـ
...
والأفضل من ذلك، يمكننا تسريع التسلسل المُسرّع، وتسريعه عودياً، وهكذا. أي أننا ننشئ تدفق تدفقات (هيكل سنسميه
#idx("tableau")
...
وأخيراً، نُشكّل تسلسلاً بأخذ الحد الأول في كل صف من اللوحة:
...
كان بإمكاننا تنفيذ تقنيات التسريع هذه دون استخدام التدفقات. ولكن صياغة التدفق أنيقة ومريحة بشكل خاص لأن تسلسل الحالات بأكمله متاح لنا كهيكل بيانات يمكن التلاعب به بمجموعة موحدة من العمليات.
...
للتعامل مع التدفقات اللانهائية، نحتاج إلى ابتكار ترتيب دمج يضمن الوصول إلى كل عنصر في النهاية إذا تركنا برنامجنا يعمل لوقت كافٍ. وطريقة أنيقة لتحقيق ذلك تكون بالدالة #py("interleave") التالية:#footnote[البيان الدقيق للخاصية المطلوبة في ترتيب الدمج هو كما يلي: يجب أن تكون هناك دالة $f$ بوسيطين بحيث أن الزوج المقابل لـ العنصر $i$ من التدفق الأول والعنصر $j$ من التدفق الثاني سيظهر كعنصر رقم $f(i,j)$ في تدفق الخرج. حيلة استخدام #py("interleave") لتحقيق ذلك عرضها لنا
#idx("Turner, David")
...
يمكننا بالتالي توليد التدفق المطلوب للأزواج كالتالي:
#idx("pairs", decl: true)
#snippet(```python
def pairs(s, t):
    return pair(llist(head(s), head(t)),
...
هل يمكنك إبداء أي تعليقات عامة حول الترتيب الذي تُوضع به الأزواج في التدفق؟ على سبيل المثال، كم عدداً تقريبياً من الأزواج تسبق الزوج (1,100)؟ والزوج (99,100)؟ والزوج (100,100)؟ (إذا كان بإمكانك تقديم بيانات رياضية دقيقة هنا، فهذا أفضل بكثير. لكن لا تتردد في تقديم إجابات وصفية أكثر إذا وجدت نفسك عالقاً.)
])
...
رامانوجان (هاردي 1921): "كان السيد ليتلوود (على ما أعتقد) هو من لاحظ أن 'كل عدد صحيح موجب كان صديقاً له.' أتذكر أنني ذهبت لرؤيته مرة عندما كان مريضاً في بوتني. ركبت في سيارة أجرة رقم 1729، ولاحظت أن الرقم بدا لي مملاً نوعاً ما، وأنني آمل ألا يكون نذير شؤم. فأجاب 'لا، إنه عدد ممتع للغاية؛ إنه أصغر عدد يمكن التعبير عنه كمجموع مكعبين بطريقتين مختلفتين.'" حيلة استخدام الأزواج الموزونة لتوليد أعداد رامانوجان عرضها علينا
#idx("Leiserson, Charles E.")
تشارلز ليسرسون.] توفر التدفقات المرتبة للأزواج حلاً أنيقاً لمشكلة حساب هذه الأعداد. وللعثور على عدد يمكن كتابته كمجموع مكعبين بطريقتين مختلفتين، نحتاج فقط إلى توليد تدفق أزواج الأعداد الصحيحة $(i,j)$ الموزونة وفقاً للمجموع $i^(3) + j^(3)$ (انظر التمرين @ex:weighted-pairs)، ثم البحث في التدفق عن زوجين متتاليين بنفس الوزن. اكتب دالة لتوليد أعداد رامانوجان. أول عدد من هذا القبيل هو 1,729. ما هي الأعداد الخمسة التالية؟
])

#exercise(label-name: <ex:3_72>, [
بطريقة مماثلة للتمرين @ex:ramanujan-nums ولد تدفق جميع الأعداد التي يمكن كتابتها كمجموع مربعين بثلاث طرق مختلفة (موضحاً كيف يمكن كتابتها كذلك).
])
...
وفي الواقع، يمكننا استخدام التدفقات لنمذجة أنظمة معالجة الإشارات بطريقة مباشرة للغاية، بمثيل قيم إشارة عند فترات زمنية متتالية كعناصر متتالية لـ تدفق. على سبيل المثال، يمكننا تنفيذ
...
دالة #py("integral") التالية تُذكرنا بـ تعريف "الأسلوب الضمني" لـ تدفق الأعداد الصحيحة (القسم @sec:infinite-streams):
...
يُقاس تدفق الدخل بـ $d t$ ويُمَرَّر عبر جامع، يُمَرَّر خرجه عائداً عبر الجامع نفسه.
...
#emph[دائرة RC] تتكون من مقاومة قيمتها $R$ ومكثف سعته $C$ على التوالي. استجابة الجهد $v$ للدائرة لتيار مُحقن $i$ تتحدد بالصيغة في الشكل @fig:rc، والذي يُعرض هيكله بـ مخطط سريان الإشارة المرفق.
...
يحدد هذا #py("RC1") كدالة تأخذ تدفقاً يمثل التسلسل الزمني للتيارات وجهاً أولياً للمكثف وتنتج تدفق الخرج للجهود.
])
...
في نظام أليسا، تُمثل الإشارة القادمة من المستشعر كتدفق #py("sense_data") وتدفق #py("zero_crossings") هو التدفق المقابل لـ تقاطعات الصفر. تكتب أليسا أولاً دالة #py("sign_change_detector") تأخذ قيمتين كوسائط وتقارن إشارات القيم لإنتاج $0$ أو $1$ أو $-1$ مناسب.
...
#idx("signal processing", sub: "smoothing a signal")
#idx("smoothing a signal")
البرنامج الذي كتبه ليس نمطياً، لأنه يخلط بين عملية التنعيم واستخراج تقاطعات الصفر. على سبيل المثال، ينبغي ألا يضطر المُستخرج إلى التغيير إذا وجدت أليسا طريقة أفضل لتهيئة إشارة الدخل الخاصة بها. ساعد لويس بكتابة دالة #py("smooth") تأخذ تدفقاً كدخل وتنتج تدفقاً يكون فيه كل عنصر هو متوسط عنصرين متتاليين في تدفق الدخل. ثم استخدم #py("smooth") كمكون لتنفيذ كاشف تقاطعات الصفر بأسلوب أكثر نمطية.
])
