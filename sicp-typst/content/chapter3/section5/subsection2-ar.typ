...
يـُستخدم العنصر الأول لبناء مرشح قابلية القسمة، والذي يمر من خلاله الباقي، ويـُغذى خرج المرشح إلى صندوق غربال آخر. ثم يـُضم العنصر الأول الأصلي إلى خرج الغربال الداخلي لتشكيل تدفق الخرج.
...
تـُمّ تعريف تدفقات #py("integers") و #py("fibs") أعلاه بتحديد دوال "مولدة" تحسب عناصر التدفق صراحة واحداً تلو الآخر. والطريقة البديلة لتحديد التدفقات هي الاستفادة من التقييم المؤجل لتعريف التدفقات ضمنياً. على سبيل المثال، تحدد العبارة التالية التدفق #py("ones") ليكون تدفقاً لانهائياً من الآحاد:
...
وبالتالي، فإن العنصر الثاني من #py("integers") هو 1 زائد العنصر الأول من #py("integers")، أو 2؛ والعنصر الثالث من #py("integers") هو 1 زائد العنصر الثاني من #py("integers")، أو 3؛ وهكذا. يعمل هذا التعريف لأنه في أي نقطة، تـُمّ توليد ما يكفي من تدفق #py("integers") بحيث يمكننا إعادته إلى التعريف لإنتاج العدد الصحيح التالي.
...
هذا تعريف عودي، حيث تـُمّ تعريف #py("primes") بدلالة دالة شرطية #py("is_prime")، والتي تستخدم نفسها تدفق #py("primes").
والسبب في عمل هذه الدالة هو أنه في أي نقطة، تـُمّ توليد ما يكفي من تدفق #py("primes") لاختبار أولية الأعداد التي نحتاج إلى فحصها بعد ذلك. أي أنه لكل $n$ نختبر أوليته، إما أن $n$ ليس أولياً (وفي هذه الحالة توجد بالفعل أعداد أولية تـُمّ توليدها تقسمه) أو أن $n$ أولي (وفي هذه الحالة توجد بالفعل أعداد أولية تـُمّ توليدها—أي أعداد أولية أقل من $n$—أكبر من $sqrt(n)$).#footnote[هذه النقطة الأخيرة دقيقة للغاية وتعتمد على حقيقة أن $p_(n+1) lt.eq p_(n)^(2)$. (هنا، يمثل $p_(k)$ العدد الأولي الـ $k$). تقديرات مثل هذه صعبة التحديد للغاية. يوضح إثبات اقليدس القديم
...
ولم تـُثبت أي نتيجة أفضل بشكل جوهري حتى عام 1851، عندما أثبت عالم الرياضيات الروسي
#idx("Chebyshev, Pafnutii L'vovich")
بافنوتي تشيبيشيف
أن $p_(n+1) lt.eq 2p_(n)$ لكل $n$. وتـُعرف هذه النتيجة، التي تـُمّ تخمينها في الأصل عام 1845، باسم
...
#py("merge") تدمج تدفقين مرتبين في تدفق نتيجة مرتب واحد، مـُستبعدةً التكرارات:
...
كم عدد عمليات الإضافة التي تـُجرى عندما نحسب عدد فيبوناتشي الـ $n$ باستخدام إعلان #py("fibs") القائم على الدالة #py("add_streams")؟ بين أن هذا العدد أكبر أسياً من عدد عمليات الإضافة المنفذة إذا كانت #py("add_streams") قد استخدمت الدالة #py("stream_map_2_optimized") الموصوفة في التمرين @ex:combine-streams، وإذا كنا قد أعلنا عن #py("fibs") كما يلي: #snippet(```python fibs = pair(0, memo(lambda: pair(1, memo(lambda: add_streams(stream_tail(fibs), fibs))))) ```)
])
...
+ الدالة $x arrow.r.bar e^(x)$ هي المشتقة الخاصة بها. هذا يعني أن $e^(x)$ وتكامل $e^(x)$ هما المتسلسلة نفسها، باستثناء الحد الثابت، وهو $e^(0) = 1$. وبناءً على ذلك، يمكننا توليد متسلسلة $e^(x)$ كـ #snippet(```python exp_series = pair(1, lambda: integrate_series(exp_series)) ```) بين كيفية توليد المتسلسلة لـ الجيب وجيب التمام، بدءاً من الحقائق أن مشتقة الجيب هي جيب التمام ومشتقة جيب التمام هي سالب الجيب: #syntax(" cosine_series = pair(1, ", metaphrase[??], ") sine_series = pair(0, ", metaphrase[??], ") ")
])
...
#idx("mulseries")
متسلسلات القوى كتدفقات من المعاملات كما في التمرين @ex:powerseries، يتـُمّ تنفيذ إضافة المتسلسلات بواسطة #py("add_streams").
...
بعبارة أخرى، $X$ هي متسلسلة القوى التي حدها الثابت هو 1 وحدودها ذات الرتب الأعلى تـُعطى بسالب $S_(R)$ مضروباً في $X$.
