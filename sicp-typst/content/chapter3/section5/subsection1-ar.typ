...
لرؤية سبب صحة ذلك، دعنا نقارن بين برنامجين لحساب مجموع جميع الأعداد الأولية في فترة محددة. يـُكتب البرنامج الأول بأسلوب تكراري قياسي:#footnote[افترض أن لدينا دالة شرطية #py("is_prime") (مثلاً، كما في القسم @sec:primality) تختبر ما إذا كان العدد أولياً.]
...
تُنتج التصفية قائمة أخرى، والتي تـُمَرَّر بدورها إلى #py("accumulate") قبل أن تـُقلَّص لتشكيل المجموع. مثل هذا التخزين الوسيط الكببر ليس مطلوباً في البرنامج الأول، الذي يمكننا التفكير فيه على أنه يُعدد الفترة تدريجياً، مُضيفاً كل عدد أولي إلى المجموع فور توليده.
...
في ذيل (#py("tail")) الزوج، سنضع هناك "وعداً" لحساب الباقي إذا تـُمّ طلبه على الإطلاق.
...
يـُذكرنا اختيار التنفيذ هذا بمناقشتنا للأعداد الكسرية في القسم @sec:abstraction-barriers، حيث رأينا أنه يمكننا اختيار تنفيذ الأعداد الكسرية بحيث يتم اختيار اختزال البسط والمقام إلى أردأ صورة إما في وقت البناء أو في وقت الاختيار. ينتج عن تنفيذي الأعداد الكسرية التجريد للبيانات نفسه، لكن الاختيار يؤثر على الكفاءة. وهناك علاقة مماثلة بين التدفقات والقوائم العادية. كتجريد للبيانات، فإن التدفقات هي نفسها القوائم. والفرق هو الوقت الذي تـُقيَّم فيه العناصر. فمع القوائم العادية، يـُقيَّم كل من #py("head") و #py("tail") في وقت البناء. أما مع التدفقات، فيـُقيَّم #py("tail") في وقت الاختيار.
...
أي أن #py("stream_enumerate_interval") ترجع تدفقاً ممثلاً كزوج رأسه #py("head") هو 10,000 وذيله #py("tail") هو وعد بتعداد المزيد من الفترة إذا طـُلِب منها ذلك. يتم الآن تصفية هذا التدفق للأعداد الأولية، باستخدام نظير التدفق للدالة #py("filter")
(القسم @sec:sequences-conventional-interfaces):
...
تـُمَرَّر هذه النتيجة الآن إلى #py("stream_tail") في تعبيرنا الأصلي. هذا يجبر #py("stream_filter") المؤجل، والذي يجبر بدوره #py("stream_enumerate_interval") المؤجل حتى يعثر على العدد الأولي التالي، وهو 10,009. وأخيراً، فإن النتيجة التي تـُمَرَّر إلى #py("head") في تعبيرنا الأصلي هي:
...
ترجع الدالة #py("head") القيمة 10,009، ويكتمل الحساب. تـُمّ اختبار الأعداد الصحيحة فقط للأولية بقدر ما كان ضرورياً للعثور على العدد الأولي الثاني، وتـُمّ تعداد الفترة فقط بقدر ما كان ضرورياً لتغذية مرشح الأعداد الأولية.

بشكل عام، يمكننا التفكير في التقييم المؤجل على أنه برمجة
#idx("programming", sub: "demand-driven")
"مدفوعة بالطلب" (#en[demand-driven])، حيث تـُنشّط كل مرحلة في عملية التدفق بقدر ما يكفي فقط لتلبية المرحلة التالية. ما فعلناه هو
#idx("order of events", sub: "decoupling apparent from actual")
فصل الترتيب الفعلي للأحداث في الحساب عن الهيكل الظاهري لدوالنا. نحن نكتب الدوال كما لو أن التدفقات توجد "دفعة واحدة" في حين أنه في الواقع، يـُجرى الحساب تدريجياً، كما في أساليب البرمجة التقليدية.
...
والحل هو بناء كائنات مؤجلة بحيث أنه في أول مرة تـُجبر فيها، تخزن القيمة التي تـُمّ حسابها. وسترجع عمليات الإجبار اللاحقة ببساطة القيمة المخزنة دون تكرار الحساب. بعبارة أخرى، ننفذ بناء أزواج التدفقات كـدالة
...
في أول مرة تـُشغل فيها الدالة المُحفظة، تحفظ النتيجة المحسوبة. وفي التقييمات اللاحقة، ترجع ببساطة النتيجة.#footnote[هناك العديد من التنفيذات المحتملة للتدفقات غير تلك الموصوفة في هذا القسم. كان التقييم المؤجل، وهو المفتاح لجعل التدفقات عملية، متأصلاً في طريقة تمرير الوسائط
...
#idx("Landin, Peter")
لاندين (1965). وتـُمّ تقديم التقييم المؤجل للتدفقات في لسب (#en[Lisp]) بواسطة
#idx("Friedman, Daniel P.")
#idx("Wise, David S.")
فريدمان ووايز (1976). في تنفيذهما، كان
#py("cons") (المكافئ في لسب لدالتنا #py("pair")) يؤجل دائمًا تقييم وسائطه، بحيث تتصرف القوائم تلقائياً كتدفقات. ويـُعرف تحسين التحفيظ أيضاً باسم
...
#emph[ثنكات التمرير بالاسم] وإلى النسخ المـُحسنة على أنها #emph[ثنكات التمرير بالحاجة].]
#idx("memo", decl: true)
#snippet(```python
def memo(fun):
    already_run = False
...
يمكننا تعريف دالة مـُحسنة #py("stream_map") كما يلي:
...
ما الذي يطبعه المفسر إذا تـُمّ استخدام #py("stream_map_optimized") بدلاً من #py("stream_map")؟
