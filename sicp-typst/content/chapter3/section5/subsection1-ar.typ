...
أي أن #py("stream_enumerate_interval") ترجع تدفقاً ممثلاً كزوج رأسه #py("head") هو 10,000 وذيله #py("tail") هو وعد بتعداد المزيد من الفترة إذا طُلِب منها ذلك. يُصفَّى الآن هذا التدفق للأعداد الأولية، باستخدام نظير التدفق للدالة #py("filter")
(القسم @sec:sequences-conventional-interfaces):
...
والحل هو بناء كائنات مؤجلة بحيث أنه في أول مرة تُجبر فيها، تخزن القيمة التي تُمّ حسابها. وسترجع عمليات الإجبار اللاحقة ببساطة القيمة المخزنة دون تكرار الحساب. بعبارة أخرى، ننفذ بناء أزواج التدفقات كدالة
...
لاحظ أن دالتنا الأولية #py("display") ترجع وسيطها بعد عرضه.
