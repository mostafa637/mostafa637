...
#section([Concurrency: Time Is of the Essence], label-name: <sec:time-is-of-the-essence>)
...
#emph[time] into our computational models. Before we introduced
...
section @sec:local-state-variables:
...
#py("balance")) delineates #emph[moments in time]
...
#emph[concurrently]—all at once. So it is often natural to
model systems as collections of
#idx("thread") #emph[threads] (sequences of computational steps)
that execute concurrently.#footnote[Such sequential threads are called "processes" throughout the book, but in this section we use the term "thread" to emphasize their access to shared memory.]
...
#idx("pipelining")
#emph[pipelining]. Although this technique greatly improves the
effective
...
#include "../../chapter3/section4/subsection1.typ"

#include "../../chapter3/section4/subsection2.typ"
