...
تمكننا المعدلات #py("set_head") و #py("set_tail") من استخدام الأزواج لبناء بنيات بيانات لا يمكن بناؤها باستخدام #py("pair") و #py("head") و #py("tail") وحدها. ويوضح هذا القسم كيفية استخدام الأزواج لتمثيل بنية بيانات تُسمى الطابور (#en[queue] / صف الانتظار). وسوف يوضح القسم @sec:tables كيفية تمثيل بنيات بيانات تُسمى الجداول.
...
- محدد: \ #idx("frontqueue") #py("front_queue(")#meta("queue")#py(")") \ يُرجع الكائن الموجود في مقدمة الطابور، ويرسل خطأ إذا كان الطابور خالياً؛ وهو لا يعدل الطابور.
- معدلان: \ #py("insert_queue(")#meta("queue")#py(",")#meta("item")#py(")") \ يُدرج #idx("insertqueue") العنصر عند خلفية الطابور ويُرجع الطابور المعدل كقيمته. #py("delete_queue(")#meta("queue")#py(")") \ يُزيل #idx("deletequeue") العنصر من مقدمة الطابور ويُرجع الطابور المعدل كقيمته، ويرسل خطأ إذا كان الطابور خالياً قبل الحذف.
...
تنشأ الصعوبة في تمثيل القائمة من الحاجة إلى الفحص للعثور على نهاية القائمة. والسبب في حاجتنا للفحص هو أنه على الرغم من أن الطريقة القياسية لتمثيل القائمة كـ سلسلة من الأزواج توفر لنا بسهولة مؤشراً إلى بداية القائمة، إلا أنها لا تعطينا مؤشراً يمكن الوصول إليه بسهولة إلى النهاية. والتعديل الذي يتجنب هذا العيب هو تمثيل الطابور كـ قائمة، إلى جانب مؤشر إضافي يشير إلى الزوج الأخير في القائمة. وبهذه الطريقة، عندما نذهب لإدراج عنصر، يمكننا فحص المؤشر الخلفي وبالتالي تجنب فحص القائمة.

وبالتالي، يُمثل الطابور كـ زوج من المؤشرات، #py("front_ptr") و #py("rear_ptr")، اللذين يشيران على التوالي إلى الزوجين الأول والأخير في قائمة عادية. وبما أننا نود أن يكون الطابور كائناً محدداً، يمكننا استخدام #py("pair") لجمع المؤشرين الاثنين. وهكذا، سيكون الطابور نفسه هو #py("pair") للمؤشرين الاثنين.
ويوضح الشكل @fig:queue-pointers هذا التمثيل.

#sicp-figure(image("/images/img_javascript/ch3-Z-G-19.svg", width: 70%), caption: [تنفيذ الطابور كـ قائمة مع مؤشري المقدمة والخلفية.], label-name: <fig:queue-pointers>)
...
يُرجع المنشئ #py("make_queue")، كـ طابور خالي في البداية، زوجاً كلاً من #py("head") و #py("tail") الخاصين به هما القائمة الخالية:
...
#idx("printqueue")
#py("print_queue")
تأخذ طابوراً كـ دخل وتطبع تسلسل العناصر في الطابور.
])

#exercise(label-name: <ex:3_22>, [
بدلاً من تمثيل الطابور كـ زوج من المؤشرات، يمكننا بناء الطابور كـ دالة
