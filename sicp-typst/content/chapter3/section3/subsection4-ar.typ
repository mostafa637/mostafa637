...
وميزة إجراء هذا التعريف هي أنه يمكننا استخدام #py("half_adder") نفسها كـ لَبِنة بناء في إنشاء دوائر أكثر تعقيداً. ويُظهر الشكل @fig:full-adder، على سبيل المثال،
#idx("full-adder")
#idx("adder", sub: "full")
...
وبعد تعريف #py("full_adder") كـ دالة، يمكننا الآن استخدامها كـ لَبِنة بناء لإنشاء دوائر أكثر تعقيداً. (على سبيل المثال، انظر التمرين @ex:ripple-carry.)

في الجوهر، يقدم محاكينا الأدوات لبناء لغة دوائر. وإذا اعتمدنا المنظور العام على اللغات الذي اقتربنا به من دراسة بايثون في القسم @sec:elements-of-programming، فيمكننا القول إن صناديق الدوال الأولية تشكل العناصر الأولية للغة، وإن ربط الصناديق معاً يوفر وسيلة التركيب، وإن تحديد أنماط الربط كـ دوال يخدم كـ وسيلة للتجريد.
...
بوابة أَوْ كـ صندوق دالة أولي. ويجب أن يكون منشئك #py("or_gate") مشابهاً لـ #py("and_gate").
])

#exercise(label-name: <ex:3_29>, [
الطريقة الأخرى لبناء بوابة
#idx("or-gate", sub: "orgate")
أَوْ هي كـ جهاز منطق رقمي مركب، يُبنى من بوابات وَ وعواكس. عرّف دالة #py("or_gate") تحقق ذلك. وما هو وقت التأخير لبوابة أَوْ بدلالة #py("and_gate_delay") و #py("inverter_delay")؟
])
...
في المحاكاة لدينا كائناً حسابياً بمتغيري حالة محليين: #py("signal_value") (يُؤخذ أولياً كـ 0) ومجموعة من #py("action_functions") سيتم تشغيلها عندما تغير الإشارة قيمتها. وننفذ السلك، باستخدام أسلوب
#idx("message passing", sub: "in digital-circuit simulation")
تمرير الرسائل، كـ مجموعة من الدوال المحلية إلى جانب دالة #py("dispatch") تختار العملية المحلية المناسبة، تماماً كما فعلنا مع كائن الحساب البنكي البسيط في القسم @sec:local-state-variables:
...
استخدام بناء الجمل الوظيفي العادي للوصول إلى الدوال المحلية للكائنات. ومن المدهش أنه يمكننا تبديل دور "الدوال" و "البيانات" بهذه الطريقة البسيطة. على سبيل المثال، إذا كتبنا #py("wire(\"get_signal\")") فنحن نفكر في #py("wire") كـ دالة تُستدعى مع الرسالة #py("\"get_signal\"") كـ دخل. وبدلاً من ذلك، فإن كتابة #py("get_signal(wire)") تشجعنا على التفكير في #py("wire") كـ كائن بيانات هو الدخل لدالة #py("get_signal"). والحقيقة في هذا الأمر هي أنه، في لغة يمكننا فيها التعامل مع الدوال كـ كائنات، لا يوجد فارق أساسي بين "الدوال" و "البيانات"، ويمكننا اختيار سكر بناء الجمل لدينا للسماح لنا بالبرمجة بأي أسلوب نختاره.]<foot:object-syntax>
...
والأسلاك، التي تمتلك إشارات متغيرة بمرور الوقت وقد ترتبط تدريجياً بأجهزة، هي نموذجية للكائنات القابلة للتغيير. ونحن نمذجناها كـ دوال بمتغيرات حالة محلية تُعدل بالإسناد. وعندما يُنشأ سلك جديد، تُخصص مجموعة جديدة من متغيرات الحالة و تُبنى دالة #py("dispatch") جديدة وتُرجع، الملتقطة للبيئة بمتغيرات الحالة الجديدة.
...
سطر (انظر التمرين @ex:agenda-list) يحتفظ بالدوال المجدولة ليتم تشغيلها خلال تلك الشريحة الزمنية.
...
وسنعمل على أسطار الشرائح الزمنية باستخدام عمليات السطر الموصوفة في القسم @sec:queues.
...
الدالة التي تزيل العنصر الأول من الأجندة تحذف العنصر عند مقدمة السطر في الشريحة الزمنية الأولى. وإذا جعل هذا الحذف الشريحة الزمنية خالية، فإننا نزيلها من قائمة الشرائح:#footnote[لاحظ أن التعليمة الشرطية في هذه الدالة تحتوي على تعليمة #idx("statement", sub: "pass", decl: true) #idx("pass statement", decl: true) #py("pass") كـ تعليمتها البديلة التي لا تفعل شيئاً.]<foot:one-armed>
...
يُعثر على عنصر الأجندة الأول عند مقدمة السطر في الشريحة الزمنية الأولى. وكلما استخرجنا عنصراً، نحدث أيضاً الوقت الحالي:#footnote[وبهذه الطريقة، سيكون الوقت الحالي دائماً هو وقت الفعل المعالج مؤخراً. وتخزين هذا الوقت عند رأس الأجندة يضمن أنه سيظل متاحاً حتى لو أُزيلت الشريحة الزمنية المرتبطة به.]
...
تُحفظ الدوال المراد تشغيلها خلال كل شريحة زمنية للأجندة في سطر.
