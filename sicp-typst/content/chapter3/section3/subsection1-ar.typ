...
كـ مثال، افترض أن #py("x") مرتبط بـ #py("llist(llist(\"a\", \"b\"), \"c\", \"d\")") و #py("y") بـ #py("llist(\"e\", \"f\")") كما هو موضح في الشكل @fig:two-lists.
...
تنفيذ #py("pair") بدلالة المعدلين الاثنين، إلى جانب دالة #py("get_new_pair")، التي تُرجع زوجاً جديداً ليس جزءاً من أي بنية قائمة موجودة.
...
#py("append_mutator")
تشبه #py("append")، ولكنها معدل بدلاً من أن تكون منشئاً. وهي تُلحق القوائم بربطها معاً، وتعديل الزوج الأخير لـ #py("x") بحيث يصبح #py("tail") الخاص به الآن هو #py("y"). (من الخطأ استدعاء #py("append_mutator") مع قائمة خالية #py("x").)
...
وعند التفكير فيها كـ قائمة، فإن كلاً من #py("z1") و #py("z2") تمثلان القائمة "نفسها":
...
ولكن إذا سمحنا بالمعدلات على بنية القائمة، يصبح التشارك مهماً. وكـ مثال على الفارق الذي يمكن أن يحدثه التشارك، فكر في الدالة التالية، التي تعدل #py("head") البنية المطبقة عليها:
...
#py("x is y")
ما إذا كان #py("x") و #py("y") هما الكائن نفسه (أي ما إذا كان #py("x") و #py("y") متساويين كـ مؤشرات).
...
والملاحظة نفسها صحيحة بالنسبة للبيانات القابلة للتغيير. يمكننا تنفيذ كائنات البيانات القابلة للتغيير كـ دوال باستخدام الإسناد والحالة المحلية. على سبيل المثال، يمكننا توسيع تنفيذ الأزواج أعلاه للتعامل مع #py("set_head") و #py("set_tail") بطريقة مماثلة للطريقة التي نفذنا بها الحسابات البنكية باستخدام #py("make_account") في القسم @sec:local-state-variables:
