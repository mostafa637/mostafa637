...
ويمكن التفكير في مثل هذا القيد كـ شبكة تتكون من قيود البوابة المضيفة والمضاعفة والثابت الأولية (الشكل @fig:constraint). ونرى في الشكل على اليسار صندوق مضاعف بثلاث محطات، مسمومة بـ $m_1$ و $m_2$ و $p$. وتصل هذه المضاعف ببقية الشبكة كما يلي:
...
#sicp-figure(image("/images/img_original/ch3-Z-G-30.svg", width: 70%), caption: [العلاقة $9C = 5(F - 32)$ معبراً عنها كـ شبكة قيود.], label-name: <fig:constraint>)
...
ويبني المنشئ #py("Adder") قيد مضيف بين موصلات المجموع الجزيئي #py("a1") و #py("a2") وموصل المجموع الكلي #py("sum"). ويُنفذ المضيف كـ دالة بالحالة المحلية (الدالة #py("me") أدناه):
...
تصل الدالة #py("adder") المضيف الجديد بالموصلات المحددة وتُرجعه كـ قيمته. والدالة #py("me")، التي تمثل المضيف، تعمل كـ موجه للدوال المحلية.
...
يُمثل الموصل كـ كائن إجرائي بمتغيرات حالة محلية: #py("value")، القيمة الحالية للموصل؛ و #py("informant")، الكائن الذي حدد قيمة الموصل؛ و #py("constraints")، قائمة القيود التي يشارك فيها الموصل.
...
تُستدعى الدالة المحلية للموصل #py("set_my_value") عندما يكون هناك طلب لتحديد قيمة الموصل. وإذا لم تكن للموصل قيمة حالية، فإنه سيحدد قيمته ويتذكر القيد كـ #py("informant") الذي طلب تحديد القيمة.#footnote[قد لا يكون #py("setter") قيداً. وفي مثال درجة الحرارة لدينا، استخدمنا #py("\"user\"") كـ #py("setter").] ثم يبلغ الموصل جميع القيود المشاركة باستثناء القيد الذي طلب تحديد القيمة. ويتحقق هذا باستخدام المكرر التالي، الذي يطبق دالة محدودة على كافة العناصر في قائمة باستثناء عنصر معطى:
...
وتخدم الدالة #py("me") للموصل كـ موجه للدوال الداخلية الأخرى وتمثل أيضاً الموصل كـ كائن. وتوفر الدوال التالية واجهة بناء جمل للتوجيه:
...
#py("averager") تأخذ ثلاثة موصلات #py("a") و #py("b") و #py("c") كـ مدخلات وتؤسس القيد بأن قيمة #py("c") هي المتوسط لقيمتي #py("a") و #py("b").
])
...
#idx("squarer (constraint)")
جهاز تربيعي كـ قيد أولي جديد. املأ الأجزاء المفقودة في مخطط بن لدالة تنفذ مثل هذا القيد:
...
وهنا #py("cplus") و #py("cmul") وإلخ هي نسخ "القيد" من العمليات الحسابية. على سبيل المثال، تأخذ #py("cplus") موصلين كـ وسائط وتُرجع موصلاً مرتبطاً بهما بقيد مضيف:
...
وبدلاً من ذلك، كان بإمكاننا التعامل مع التعبيرات، باستخدام دوال تُرجع متجهات كـ قيم، وبالتالي تجنب الإشارة صراحة إلى #py("temp1") و #py("temp2"):

#snippet(```python
answer = v_prod(v_sum("a", "b"), v_sum("c", "d"))
```)

وبما أن بايثون تسمح لنا بـ إرجاع كائنات مركبة كـ قيم للدوال، فيمكننا تحويل لغة القيود بأسلوبها الأمري إلى أسلوب موجه بالتعبير كما هو موضح في هذا التمرين.
