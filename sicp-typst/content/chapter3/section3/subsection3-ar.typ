...
مفهرسة بمفاتيح تعريفية. وفي تنفيذ البرمجة الموجهة بالبيانات في القسم @sec:data-directed، استخدمنا بكثرة الجداول ثنائية الأبعاد، التي تخزن وتسترجع فيها المعلومات باستخدام مفتاحين. وهنا نرى كيفية بناء الجداول كـ بنيات قائمة قابلة للتغيير.

ننظر أولاً في
#idx("table", sub: "one-dimensional")
جدول أحادي البعد، تُخزن فيه كل قيمة تحت مفتاح واحد. وننفذ الجدول كـ قائمة سجلات، يُنفذ كل منها كـ زوج يتكون من مفتاح والقيمة المرتبطة به. وتُرتبط السجلات معاً لتشكل قائمة بوساطة أزواج تشير #py("head")s الخاصة بها إلى السجلات المتتالية. وتُسمى هذه الأزواج المرتبطة بـ
#idx("table", sub: "backbone of")
#emph[العمود الفقري] (#en[backbone]) للجدول. وللحصول على مكان يمكننا تغييره عندما نضيف سجلاً جديداً إلى الجدول، نبني الجدول كـ
#idx("headed list")
...
#sicp-figure(image("/images/img_javascript/ch3-Z-G-22.svg", width: 70%), caption: [جدول ممثل كـ قائمة مروَّسة.], label-name: <fig:table>)

ولاستخراج المعلومات من جدول نستخدم الدالة
#py("lookup")،
والتي تأخذ مفتاحاً كـ وسيط وتُرجع القيمة المرتبطة به (أو
#py("None")
إذا لم تكن هناك قيمة مخزنة تحت ذلك المفتاح).
وتُعرف الدالة #py("lookup") بدلالة عملية #py("assoc")،
والتي تتوقع مفتاحاً وقائمة سجلات كـ وسائط. لاحظ أن
#py("assoc") لا ترى السجل الوهمي أبداً.
وتُرجع الدالة #py("assoc") السجل الذي يحتوي على المفتاح المعطى كـ #py("head") الخاص به.#footnote[بما أن #py("assoc") تستخدم
...
في الجدول ثنائي الأبعاد، تُفهرس كل قيمة بمفتاحين. ويمكننا بناء مثل هذا الجدول كـ جدول أحادي البعد يحدد فيه كل مفتاح جدولاً فرعياً.
...
تأخذ العمليتان #py("lookup") و #py("insert") المحدادتان أعلاه الجدول كـ وسيط. وهذا يمكننا من استخدام البرامج التي تصل إلى أكثر من جدول واحد. والطريقة الأخرى للتعامل مع الجداول المتعددة هي أن تكون لدينا دوال #py("lookup") و #py("insert") منفصلة لكل جدول. ويمكننا فعل ذلك بتمثيل الجدول إجرائياً، كـ كائن يحافظ على جدول داخلي كـ جزء من حالته المحلية. وعند إرسال رسالة مناسبة، يقدم "كائن الجدول" هذا الدالة لتشغيلها على الجدول الداخلي. وإليك مولد للجداول ثنائية الأبعاد الممثلة بهذه الطريقة:
...
تأخذ الدالة #py("get") مفتاحين كـ وسائط، وتأخذ #py("put") مفتاحين وقيمة كـ وسائط. وتصل كلا العمليتين إلى الجدول المحلي نفسه، والمغلف داخل الكائن المنشأ بالاستدعاء لـ #py("make_table").
#idx("table", sub: "local")
...
ويجب أن تأخذ الدالتان #py("lookup") و #py("insert") كـ دخل قائمة مفاتيح تُستخدم للوصول إلى الجدول.
])
...
وتحافظ الدالة المحفوظة بالتذكر على جدول تُخزن فيه قيم الاستدعاءات السابقة باستخدام الوسائط التي أنتجت القيم كـ مفاتيح. وعندما يُطلب من الدالة المحفوظة بالتذكر حساب قيمة، فإنها تفحص الجدول أولاً لرؤية ما إذا كانت القيمة موجودة بالفعل، وإذا كان الأمر كذلك، تُرجع تلك القيمة فقط. بخلاف ذلك، تحسب القيمة الجديدة بالطريقة العادية وتخزنها في الجدول.
وكـ مثال على التذكر، تذكر من القسم @sec:tree-recursion العملية الأسية لحساب أعداد فيبوناتشي:
