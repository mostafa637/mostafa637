...
لاحظ بنية البيئة.
الاسم #py("sqrt") مرتبط في بيئة البرنامج بكائن دالة بيئته المرتبطة هي بيئة البرنامج. وعندما أُستدعيت #py("sqrt")، أُنشئت بيئة جديدة، E1، تابعة لبيئة البرنامج، يُربط فيها المعلمة #py("x") بـ 2. ثم أُقيِّم جسم #py("sqrt") في E1.
...
لذلك أُستدعي كائن الدالة المرتبط بـ #py("sqrt_iter") في E2 مع كون 1 كوسيط. ينشئ هذا بيئة E3 يُربط فيها #py("guess")، معلمة #py("sqrt_iter")، بـ 1.
وتستدعي الدالة #py("sqrt_iter") بدورها #py("is_good_enough") بقيمة #py("guess") (من E3) كوسيط لـ #py("is_good_enough").
أنشأ هذا بيئة أخرى، E4، يُربط فيها #py("guess") (معلمة #py("is_good_enough")) بـ 1. وعلى الرغم من أن كل من #py("sqrt_iter") و #py("is_good_enough") لديهما معلمة مسمّاة #py("guess")، إلا أنهما متغيران محليان متميزان يقعان في إطارين مختلفين.
...
- يمكن للدوال المحلية الوصول إلى وسائط الدالة المحيطة، بمجرد استخدام أسماء المعلمات كأسماء حرة. وذلك لأن جسم الدالة المحلية يُقيّم في بيئة تابعة لبيئة التقييم للدالة المحيطة.
...
ومزودين بطريقة للتعامل مع التصريحات داخل الكتل، يمكننا إعادة زيارة تصريحات الأسماء عند المستوى الأعلى. في القسم @sec:env-model-rules، رأينا أن الأسماء المصرح بها عند المستوى الأعلى تُضاف إلى إطار البرنامج. والأدق أن نقول إن البرنامج بأكمله يوضع في كتلة ضمنية تُقيَّم في البيئة العالمية.
ثم تعامل معاملة الكتل الموصوفة أعلاه المستوى الأعلى:
تُوسَّع البيئة العالمية بإطار يحوي ربط جميع الأسماء المصرَّح بها في الكتلة الضمنية. وذلك الإطار هو إطار البرنامج والبيئة الناتجة هي
...
الوصول إلى قيمة اسم قبل تقييم تصريحه؛ وتقييم اسم غير مُسند يُصدر خطأ.#footnote[يشرح هذا سبب خطأ البرنامج في الحاشية في الفصل 1.
