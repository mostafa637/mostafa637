...
#chapter([Metalinguistic Abstraction], label-name: <chap:meta>)

#epigraph([…It's in words that the magic
...
It's as if—as if the key to the treasure #emph[is]
the treasure!
#idx("Barth, John")], author: [John Barth], title: [#emph[Chimera]])
...
of these are the language of electrical #emph[networks] and the
language of electrical #emph[systems]. The network language emphasizes
...
language of section @sec:graphics.]
...
#emph[Metalinguistic abstraction]—establishing
...
#idx("evaluator")
#emph[evaluator] (or #emph[interpreter]) for a programming language is a
function
...
#blockquote[The evaluator, which determines the meaning of
statements and
...
section @sec:symbolic-algebra embodies the rules of
...
section @sec:circuit-simulator and the constraint
propagator of section @sec:constraints are legitimate
...
discussion in chapter @chap:state. There, we mitigated some of the complexities
...
Python.
In section @sec:lazy-evaluation, we'll change
the underlying language to provide for a more elegant approach, by modifying
the evaluator to provide for #emph[normal-order evaluation].

Section @sec:nondeterministic-evaluation implements a
...
#emph[nondeterministic computing], it is natural to express processes that
...
In section @sec:logic-programming we implement a
#emph[logic-programming] language in which knowledge is expressed in terms
...
#include "../chapter4/section1/section1.typ"

#include "../chapter4/section2/section2.typ"

#include "../chapter4/section3/section3.typ"

#include "../chapter4/section4/section4.typ"
