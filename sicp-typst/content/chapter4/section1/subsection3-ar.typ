...
بالإضافة إلى تعريف تمثيل المكونات، يجب أن يحدد تنفيذ المـُقيِّم أيضاً هياكل البيانات التي يتعامل معها المـُقيِّم داخلياً، كجزء من تنفيذ البرنامج، مثل تمثيل الدوال والبيئات وتمثيل صح و خطأ.

#subheading([اختبار الدوال الشرطية])

#idx("metacircular evaluator for Python", sub: "representation of true and false")

من أجل حصر تعبيرات الدوال الشرطية للشرطيات على الشرطيات المناسبة (التعبيرات التي تـُقيَّم إلى قيمة منطقية) كما نفعل طوال هذا الكتاب، فإننا نصر هنا على أن الدالة
#idx("metacircular evaluator for Python", sub: "representation of true and false")
#py("is_truthy") تـُطبق فقط على القيم المنطقية، ونقبل فقط القيمة المنطقية
#py("true") لتكون صحيحة.
والعكس لـ
#py("is_truthy") يـُسمى
#py("is_falsy").#footnote[الشرطيات في Python الكاملة تتقبل #emph[أي] قيمة، وليس فقط قيمة منطقية، كـ نتيجة لتقييم تعبير "الدالة الشرطية". ومفهوم Python للصحة والخطأ يتحدد بالنسخ التالية من #py("is_truthy") و #py("is_falsy"):
#idx("truthiness")
#idx("falsiness")
...
#idx("NaN, not a typo")
والتي تـُعتبر عدداً خاطئاً (أيضاً ليس خطأ مطبعياً)، جنباً إلى جنب مع 0.
...
الدوال المركبة تـُبنى من البارامترات، وأجسام الدوال، والبيئات باستخدام المنشئ #py("make_function"):
...
عبارة إرجاع، نقدم #emph[قيم الإرجاع] كـ هياكل بيانات للمـُقيِّم.
...
يحتاج المـُقيِّم إلى عمليات لـ
#idx("metacircular evaluator for Python", sub: "environment operations")
#idx("symbol(s)", sub: "in environment operations")
التعامل مع البيئات. وكما يـُوضح في القسم @sec:environment-model، فإن البيئة هي تسلسل من الإطارات، حيث يكون كل إطار جدولاً من الارتباطات التي تربط الرموز بقيمها المقابلة. ونستخدم العمليات التالية للتعامل مع البيئات:
...
نمثل البيئة كـ قائمة من الإطارات. والبيئة المحيطة للبيئة هي #py("tail") القائمة. والبيئة الفارغة هي ببساطة القائمة الفارغة.
...
يـُمثَّل كل إطار في البيئة كـ زوج من القوائم: قائمة الأسماء المربوطة في ذلك الإطار وقائمة القيم المقابلة.#footnote[الإطارات ليست تجريداً حقيقياً للبيانات: والدالة #py("assign_symbol_value") أدناه تستخدم #py("set_head") لتعديل القيم في الإطار مباشرة. والغرض من دوال الإطار هو جعل دوال التعامل مع البيئة أسهل للقراءة.]
...
يـُستخدم هذا بواسطة #py("apply") في القسم @sec:core-of-evaluator لربط بارامترات الدالة بوسائطها.
...
تجريد البيانات لعزل باقي المـُقيِّم عن الاختيار التفصيلي للتمثيل، فيمكننا تغيير تمثيل البيئة إذا أردنا. (انظر التمرين @ex:alternate-frame-representation.) وفي نظام Python بجودة الإنتاج، فإن سرعة عمليات بيئة المـُقيِّم—خاصة سرعة البحث عن الرموز—لها تأثير كبير على أداء النظام. والتمثيل الموصوف هنا، على الرغم من بساطته المفهومية، ليس كفؤاً ولن يـُستخدم عادة في نظام إنتاجي.#footnote[العيب في هذا التمثيل (وكذلك التنويع في التمرين @ex:alternate-frame-representation) هو أن المـُقيِّم قد يضطر للبحث عبر إطارات عديدة للعثور على الارتباط لمتغير معطى. (ومثل هذه المقاربة تـُسمى
#idx("deep binding")
#idx("binding", sub: "deep")
#emph[الربط العميق] (#en[deep binding])). وإحدى الطرق لتجنب عدم الكفاءة هذه هي الاستفادة من استراتيجية تـُسمى #emph[العنونة المعجمية]، والتي ستـُناقش في القسم @sec:lexical-addressing.]

#idx("metacircular evaluator for Python", sub: "representation of environments")

#exercise(label-name: <ex:alternate-frame-representation>, [
بدلاً من تمثيل الإطار كـ زوج من القوائم، يمكننا تمثيل الإطار كـ قائمة من الارتباطات، حيث تكون كل ارتباطة عبارة عن زوج من رمز وقيمة. أعد كتابة عمليات البيئة لاستخدام هذا التمثيل البديل.
])
...
تميز لغتنا بين الثوابت والمتغيرات باستخدام كلمات مفتاحية مختلفة—#py("const") و #py("let")—وتمنع الإسناد للثوابت. ومع ذلك، فإن مـُفسِّرنا لا يستفيد من هذا التمييز؛ فالدالة #py("assign_symbol_value") ستسند بسعادة قيمة جديدة لـ رمز معطى، بغض النظر عما إذا كان مـُعلناً كـ ثابت أو متغير.
...
- غير #py("scan_out_declarations") و (إذا كان ذلك ضرورياً) #py("extend_environment") بحيث يمكن التمييز بين الثوابت والمتغيرات في الإطارات التي تـُرتبط فيها.
- غير #py("assign_symbol_value") بحيث تفحص ما إذا كان الرمز المعطى قد تـُمّ إعلانه كـ متغير أو كـ ثابت، وفي الحالة الأخيرة تعطي إشارة خطأ أن عمليات الإسناد غير مسموح بها على الثوابت.
...
+ يتطلب تحديد Python من التنفيذ إعطاء إشارة خطأ وقت التشغيل عند محاولة الوصول إلى قيمة اسم قبل تقييم إعلانه (انظر نهاية القسم @sec:env-internal-def). ولتحقيق هذا السلوك في المـُقيِّم، #idx("lookupsymbolvalue", sub: "for scanned-out declarations") غير #py("lookup_symbol_value") لإعطاء إشارة خطأ إذا كانت القيمة التي يعثر عليها هي #py("\"*unassigned*\"").
+ وبالمثل، يجب ألا نسند قيمة جديدة لـ متغير إذا لم نكن قد قمنا بتقييم إعلان #py("let") الخاص به بعد. غير تقييم الإسناد بحيث يعطي الإسناد لـ متغير مـُعلن بـ #py("let") إشارة خطأ في هذه الحالة.
])
...
+ قبل ECMAScript 2015، كانت الطريقة الوحيدة لإعلان متغير محلي في Python هي استخدام الكلمة المفتاحية #py("var") بدلاً من الكلمة المفتاحية #py("let"). ونطاق المتغيرات المـُعلنة بـ #py("var") هو الجسم بأكمله لـ تعريف الدالة أو تعبير #en[lambda] المحيط مباشرة، وليس فقط الكتلة المحيطة مباشرة. عدل #py("scan_out_declarations") و #py("eval_block") بحيث تتبع الأسماء المـُعلنة بـ #py("const") و #py("let") قواعد نطاق #py("var").
+ عند عدم وجود Strict Mode، تسمح Python لأسماء غير مـُعلنة بأن تظهر على يسار #py("=") في الإسنادات. ومثل هذا الإسناد يضيف الارتباط الجديد للبيئة العالمية. عدل الدالة #py("assign_symbol_value") لجعل الإسناد يتصرف بهذه الطريقة. وتـُمّ تقديم Strict Mode، الذي يمنع مثل هذه الإسنادات، في Python لجعل البرامج أكثر أماناً. ما هي قضية الأمان المعالجة بـ منع الإسناد من إضافة ارتباطات للبيئة العالمية؟
])
