...
في كل مرة تُستدعى فيها #py("factorial")، يجب على المُقيِّم
...
و
#py("n - 1")،
يجب على المُقيِّم إجراء تحليل الحالات في
#py("evaluate")
لتحديد أن التعبير هو تطبيق، ويجب عليه استخراج
تعبير الدالة وتعبيرات المعمولات.
هذا التحليل مكلف.
وإجراؤه بشكل متكرر ينطوي على هدر.

يمكننا تحويل المُقيِّم ليكون أكثر كفاءة بدرجة كبيرة عن طريق
...
فهي ترجع دالة تنفيذ
تتجاهل وسيط البيئة الخاص بها وترجع فقط
قيمة التعبير الحرفي:
...
تأخذ البيئةَ كوسيط وتستدعي بالتتابع كل دالة تنفيذ
فردية مع البيئة كوسيط.
...
يستخدم المُقيِّم الجديد نفس بنيات البيانات، ودوال بناء الجملة،
...
صمم ونفذ بعض التجارب لمقارنة سرعة المُقيِّم دائري التجريد الأصلي
