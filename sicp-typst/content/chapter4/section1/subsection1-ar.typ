...
كوسائط
#emph[مكون] برنامج—عبارة أو تعبير#footnote[لا داعي للتمييز بين العبارات والتعبيرات في مُقيِّمنا. على سبيل المثال، لا نفرق بين التعبيرات وعبارات التعبير؛ فنحن نمثلها على نحو مطابق وبالتالي يُتعامل معها بالطريقة نفسها بواسطة الدالة #py("evaluate"). وبالمثل، لا يفرض مُقيِّمنا القيد النحوي لـ Python بأن العبارات لا يمكن أن تظهر داخل التعبيرات بخلاف تعبيرات دالة غير مسماة (#en[lambda]).]—وبيئة.
وتصنف
المكون
وتوجه تقييمه.
الدالة #py("evaluate")
مُهيكلة كتحليل حالات للنوع النحوي لـ
المكون
...
- بالنسبة لـ تطبيق الدالة، يجب على #py("evaluate") تقييم تعبير الدالة وتعبيرات الوسائط للتطبيق عودياً. وتُمَرَّر الدالة والوسائط الناتجة إلى #py("apply")، والتي تتعامل مع تطبيق الدالة الفعلي.
- تُحوَّل تركيبات المعمولات إلى تطبيق دالة ثم تُقيَّم.

#subsubheading([الأشكال النحوية])

- يتطلب التعبير أو العبارة الشرطية معالجة خاصة لأجزائها، حتى تُقيَّم النتيجة إذا كانت الدالة الشرطية صحيحة، وإلا تُقيَّم البديلة.
- يجب تحويل تعبير الدالة غير المسماة (#en[lambda]) إلى دالة قابلة للتطبيق بحزمة المعلمات والجسم المحددين بـ تعبير #en[lambda] مع بيئة التقييم.
- يتطلب تسلسل العبارات تقييم مكوناته بالترتيب الذي تظهر به.
- تتطلب الكتلة تقييم جسمها في بيئة جديدة تعكس جميع الأسماء المُعلنة داخل الكتلة.
- يجب أن تنتج عبارة الإرجاع قيمة تصبح نتيجة استدعاء الدالة الذي أدى إلى تقييم عبارة الإرجاع.
- يُحوَّل تعريف الدالة إلى إعلان ثابت ثم يُقيَّم.
...
يُتعامل مع تمثيل الأسماء بواسطة تجريدات البناء النحوي. داخلياً،
يستخدم المُقيِّم السلاسل النصية لتمثيل الأسماء، ونشير إلى مثل هذه السلاسل النصية بـ
...
مركبة
تُبنى بـ توسيع البيئة الأساسية المحمولة بواسطة
الدالة
لتتضمن إطارًا يربط معلمات
الدالة
...
#emph[قيمة إرجاع]. وإذا أدى تقييم جسم الدالة إلى مثل قيمة الإرجاع هذه، فيتم استرداد محتوى قيمة الإرجاع؛ وإلا تُرجع القيمة
#py("None").#footnote[هذا الاختبار عملية مؤجلة، وبالتالي فإن مُقيِّمنا سيعطي مساراً لعملية
عودية حتى لو كان البرنامج المُفسَّر سيعطي مساراً لعملية
تكرارية وفقاً للوصف في
القسم @sec:recursion-and-iteration. وبعبارة أخرى، فإن تنفيذنا للمُقيِّم دائري التجريد لـ Python ليس
...
تجمع قائمة بجميع الرموز التي تمثل الأسماء Mُعلنة في الجسم.
وتستخدم
...
يُقيَّم النتيجة، وإلا تُقيَّم البديلة:
...
لاحظ أن المُقيِّم لا يحتاج إلى التمييز بين التعبيرات الشرطية والعبارات الشرطية.
...
يسلط الضوء على قضية الصلة بين اللغة المُنفذة
ولغة التنفيذ. فـالـ
#py("conditional_predicate")
يُقيَّم في اللغة الجاري تنفيذها وبالتالي يعطي قيمة في
تلك اللغة. ودالة الشرط المُفسِّرة
#py("is_truthy")
...
تُستخدم الدالة #py("eval_sequence")
بواسطة #py("evaluate")
...
باستثناء أنه إذا أدى تقييم أي عبارة في التسلسل إلى قيمة إرجاع، فإن تلك القيمة تُرجع وتُتجاهل العبارات اللاحقة.
...
تُستخدم الدالة #py("eval_return_statement")
لتقييم
...
تُعرف كل من إعلانات الثوابت والمتغيرات بواسطة الدالة الشرطية النحوية
#py("is_declaration").
وتُعامل بطريقة مماثلة لـ الإسنادات، لأن #py("eval_block")
...
لاحظ أنه لا يمكننا معرفة ما إذا كان المُقيِّم دائري التجريد
...
فـ ترتيب تقييمه مُوروث من JavaScript الأساسية:
...
الوسائط لـ #py("pair")
تُقيَّم من اليمين إلى اليسار، فإن
