...
وهنا، كما هو الحال في المُقَيِّمات الأخرى في هذا الفصل، نستخدم
#idx("بناء جملة تجريدي", sub: "في مفسر الاستعلام")
#idx("parse", sub: "في مفسر الاستعلام")
#py("parse") لتحويل مكوّن من لغة الاستعلام يُعْطَى كـ سلسلة نصية إلى تمثيل بناء جملة #en[Python]. (ونلحق فاصلة منقوطة بسلسلة تعبيرات المدخلات لأن #py("parse") تتوقع عبارة).
...
#subsubsection([المُقَيِّم], label-name: <sec:query-eval>)

الدالة
#idx("مفسر الاستعلام", sub: "مُقَيِّم الاستعلام")
#py("evaluate_query")،
المستدعاة بواسطة #py("query_driver_loop")، هي المُقَيِّم الأساسي لنظام الاستعلام. وتأخذ كمدخلات استعلامًا وتدفق إطارات، وترجع تدفق إطارات مُمَدَّدة.
وتحدد الأشكال
النحوية بواسطة
...
تتعامل الدالة
#py("simple_query")
مع الاستعلامات البسيطة. وتأخذ كمعاملات استعلامًا بسيطًا (نمطًا) جنبًا إلى جنب مع تدفق إطارات، وترجع التدفق المشكل عن طريق تمديد كل إطار بجميع مطابقات قاعدة البيانات للاستعلام.
...
وتأخذ الدالة #py("check_an_assertion") كمعاملات كائن بيانات (تقريراً جازماً)، ونمطًا، وإطارًا وترجع إما تدفقًا من عنصر واحد يحتوي على الإطار المُمَدَّد وإما #py("None") إذا فشلت المطابقة.
...
وتطبق الدالة #py("apply_a_rule") قاعدة باستخدام الطريقة الموضحة خطوطها العريضة في القسم @sec:how-query-works. فهي تمدد أولاً إطار معاملاتها عن طريق توحيد نتيجة القاعدة مع النمط في الإطار المعطى. وإذا نجح هذا، تقيم متن القاعدة في هذا الإطار الجديد.
...
كما يستخدم المُقَيِّم الدالة البسيطة التالية لتوليد تدفق يتكون من عنصر واحد:
...
وقائمة «المعاملات» (من عنصر واحد) لـ #py("javascript_predicate") تظل غير معالجة، كما هو موضح أدناه.
...
وتأخذ الدالة #py("instantiate_term") متغيرًا، أو زوجًا، أو قيمة أوليّة كمعامل أول وإطارًا كمعامل ثانٍ وتستبدل عوديًا المتغيرات في المعامل الأول بقيمها في الإطار حتى يتم الوصول إلى قيمة أوليّة أو متغير غير مرتبط.
وعندما تواجه العملية زوجًا، يُبْنَى زوج جديد تكون أجزاؤه هي النسخ المُجَسَّدة للأجزاء الأصلية.
على سبيل المثال، إذا كانت #py("$x") مرتبطة بالزوج $[mono("$y"), 5]$ في إطار $f$ كـ نتيجة للتوحيد، وكانت #py("$y") مرتبطة بدورها بـ 3، فإن نتيجة تطبيق #py("instantiate_term") على #py("list(\"name\", \"$x\")") و$f$ هي الزوج $[3, 5]$.
...
وهناك نهج آخر وهو معالجة بندي #py("and") بشكل منفصل، ثم البحث عن جميع أزواج إطارات المخرجات المتوافقة. إذا كان كل استعلام يُنتج $N/k$ إطار مخرجات، فإن هذا يعني أنه يجب علينا إجراء $N^(2)/k^(2)$ فحص توافق—وهو معامل $k$ أقل من عدد المطابقات المطلوبة في طريقتنا الحالية.
