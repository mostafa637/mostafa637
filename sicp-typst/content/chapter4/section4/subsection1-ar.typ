...
#idx("قاعدة بيانات", sub: "البرمجة المنطقية و")
...
#idx("لغة الاستعلام", sub: "قاعدة بيانات")

#idx("قاعدة بيانات", sub: "موظفو Gargle")
...
تبدو التقريرات الجازمة تمامًا مثل تطبيقات الدوال في #en[Python]، ولكنها تمثل في الواقع معلومات في قاعدة البيانات. وتصف الرموز الأولى — هنا #py("address") و#py("job") و#py("salary") — #emph[نوع المعلومات] المحتواة في التقرير الجازم المعني، والمعاملات عبارة عن قوائم أو قيم أوليّة مثل السلاسل النصية والأعداد. ولا يلزم التصريح عن الرموز الأولى، كما هو الحال مع الثوابت أو المتغيرات في #en[Python]؛ فنطاقها عام.
...
في هذا المثال، يحدد النمط #py("job") كـ نوع المعلومات التي نبحث عنها. ويمكن أن يكون العنصر الأول أي شيء، والعنصر الثاني هو القائمة الحرفية #py("llist(\"computer\", \"programmer\")").
...
#idx("اتفاقيات التسمية", sub: "$ لمتغيرات الأنماط")
#idx("$, متغيرات أنماط تبدأ بـ", sort: "0a4")
...
- يجد النظام جميع التعيينات للمتغيرات في نمط الاستعلام التي #idx("استيفاء نمط (استعلام بسيط)") #emph[تستوفي] النمط — أي جميع مجموعات القيم للمتغيرات بحيث إذا تم #idx("تجسيد نمط") #emph[تجسيد] متغيرات النمط بـ (استبدالها بـ) القيم، تكون النتيجة في قاعدة البيانات.
...
تشكل الاستعلامات البسيطة العمليات الأوليّة للغة الاستعلام. ولتشكيل عمليات مركبة، توفر لغة الاستعلام وسائل تركيب. وأحد الأشياء التي تجعل لغة الاستعلام لغة برمجة منطقية هو أن وسائل التركيب تعكس وسائل التركيب المستخدمة في تشكيل التعبيرات المنطقية: #py("and") و#py("or") و#py("not").

يمكننا استخدام
#idx("and (لغة الاستعلام)")
...
#idx("لغة الاستعلام", sub: "اختبار المساواة في")
...
#idx("عودية", sub: "في القواعد")
عوديًا. على سبيل المثال، القاعدة
#idx("outrankedby (قاعدة)", decl: true)
#snippet(```python
...
#idx("لغة الاستعلام", sub: "استنباطات منطقية")
...
#idx("appendtoform (قواعد)", decl: true)
#snippet(```python
