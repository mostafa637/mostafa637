...
المُقَيِّم المُحَلِّل من القسم @sec:separating-analysis.#footnote[اخترنا تنفيذ المُقَيِّم الكسول في القسم @sec:lazy-evaluation كتعديل للمُقَيِّم دائري التجريد الاعتيادي من القسم @sec:core-of-evaluator. وعلى النقيض من ذلك، سنبني مُقَيِّم #py("amb") على المُقَيِّم المُحَلِّل من القسم @sec:separating-analysis، لأن دوال التنفيذ في ذلك المُقَيِّم توفر إطارًا مريحًا لتنفيذ التتبع التراجعي.] وكما هو الحال في المُقَيِّم المُحَلِّل، يتم تقييم المكوّن عن طريق استدعاء
#idx("executionfunction", sub: "في المقييم غير الحتمي")
...
#idx("متابعة", sub: "في المقييم غير الحتمي")

تذكر أن دوال التنفيذ للمُقَيِّم الاعتيادي تأخذ معاملًا واحدًا: بيئة التنفيذ. وعلى النقيض من ذلك، تأخذ دوال التنفيذ في مُقَيِّم #py("amb") ثلاثة معاملات: البيئة، ودالتان تُسميان #emph[دوال المتابعة] (#en[continuation functions]).
...
#emph[متابعة الفشل] (#en[failure continuation]). وإنشاء ودعوة المتابعات المناسبة هما الآلية التي ينفذ بها المُقَيِّم غير الحتمي التتبع التراجعي.
...
#subheading([بنية المُقَيِّم])

دوال بناء الجملة وتمثيل البيانات لمُقَيِّم #py("amb")، وأيضًا دالة
#idx("analyze", sub: "nondeterministic")
#py("analyze") الأساسية، متطابقة مع تلك الموجودة في المُقَيِّم في القسم @sec:separating-analysis، باستثناء حقيقة أننا نحتاج إلى دوال بناء جملة إضافية للتعرف على الشكل النحوي #py("amb"):
#idx("isamb", decl: true)
#snippet(```python
...
نستمر في استخدام دالة التحليل في القسم @sec:representing-expressions، والتي لا تدعم #py("amb") كشكل نحوي وتتعامل بدلاً من ذلك مع #py("amb(") $dots.h$ #py(")") كتطبيق دالة. وتضمن الدالة #py("is_amb") أنه كلما ظهر الاسم #py("amb") كتعبير دالة لتطبيق ما، فإن المُقَيِّم يتعامل مع «التطبيق» كنقطة اختيار غير حتمية.#footnote[مع هذا التعامل، لم يعد #py("amb") اسمًا بنطاق سليم. ولتجنب الارتباك، يجب أن نتمتنع عن التصريح عن #py("amb") كاسم في برامجنا غير الحتمية.]
...
#idx("متابعة النجاح (مقييم غير حتمي)")
#idx("متابعة", sub: "في المقييم غير الحتمي")
هي دالة من معاملين: القيمة المحصول عليها حديثًا ومتابعة فشل أخرى لاستخدامها إذا أدت تلك القيمة إلى فشل لاحق.
ومتابعة
#idx("متابعة الفشل (مقييم غير حتمي)")
الفشل هي دالة بدون معاملات. لذا فإن الشكل العام لـ
#idx("execution function", sub: "في المقييم غير الحتمي")
دالة التنفيذ هو
...
وتُعَالَج المتتاليات بنفس الطريقة كما في المُقَيِّم السابق، باستثناء الترتيبات في الدالة الفرعية #py("sequentially") المطلوبة لتمرير المتابعات. أي، لتنفيذ #py("a") تتابعيًا ثم #py("b")، نستدعي #py("a") مع متابعة نجاح تستدعي #py("b").
...
#idx("تصريح داخلي", sub: "في المقييم غير الحتمي")
...
دالة التنفيذ للتطبيقات لا تحتوي على أفكار جديدة باستثناء التعقيد التقني لإدارة المتابعات. وينشأ هذا التعقيد في #py("analyze_application")، بسبب الحاجة إلى تتبع متابعات النجاح والفشل بينما نُقَيِّم تعبيرات الوسائط. وتستخدم دالة #py("get_args") لتقييم قائمة تعبيرات الوسائط، بدلاً من #py("map") بسيطة كما في المُقَيِّم الاعتيادي.
...
ويستخدم المحرك دالة تُسمى #py("internal_loop")، والتي تأخذ كمعامل دالة
...
ومتابعة النجاح للاستدعاء لـ #py("ambeval") أكثر دقة. فنطبع القيمة المحصول عليها ثم نستدعي مجددًا الحلقة الداخلية مع دالة #py("retry") ستكون قادرة على تجربة البديل التالي. ودالة #py("next_alternative") هذه هي المعامل الثاني الذي تُمُرِّر إلى متابعة النجاح. وعادة، نفكر في هذا المعامل الثاني كمتابعة فشل لاستخدامها إذا فشل فرع التقييم الحالي لاحقًا. ولكن في هذه الحالة، أتممنا تقييمًا ناجحًا، لذا يمكننا استدعاء فرع البديل «الفاشل» من أجل البحث عن تقييمات ناجحة إضافية.
