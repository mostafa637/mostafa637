...
ومع التقييم الكسول، يمكن أن تكون التدفقات والقوائم متطابقة، لذا لا داعي لعمليات منفصلة للقوائم والتدفقات. كل ما نحتاج إلى فعله هو ترتيب الأمور بحيث تكون #py("pair") غير صارمة. إحدى الطرق لتحقيق ذلك هي توسيع المُقَيِّم الكسول للسماح بأوليات غير صارمة، وتنفيذ #py("pair") كإحدى هذه الأوليات. والطريقة الأسهل هي أن نتذكر (القسم @sec:data-) أنه لا يوجد احتياج أساسي لتنفيذ #py("pair") كدالة أوليّة على الإطلاق. وبدلاً من ذلك، يمكننا تمثيل
#idx("زوج (أزواج)", sub: "تمثيل دالي لـ")
الأزواج كدوال:#footnote[هذا هو التمثيل الدالي الموصوف في التمرين @ex:lambda-cons. وحقيقةً، سيفيد أي تمثيل دالي (مثل تنفيذ يمرر الرسائل). لاحظ أنه يمكننا تثبيت هذه التعاريف في المُقَيِّم الكسول ببساطة عن طريق كتابتها في حلقة المحرك. وإذا كنا قد ضمنّا في الأصل #py("pair") و#py("head") و#py("tail") كأوليات في البيئة العالمية، فستُعاد إعادة تعريفها. (انظر أيضًا التمارين @ex:lazy-list-input و @ex:lazy-list-printing).]
...
ولدهشته، ينتج عن هذا خطأ. وبعد بعض التفكير، يدرك أن «القوائم» المحصول عليها من دالة #py("list") الأوليّة مختلفة عن القوائم التي تتعامل معها التعاريف الجديدة لـ #py("pair") و#py("head") و#py("tail"). عدّل المُقَيِّم بحيث تنتج تطبيقات دالة #py("list") الأوليّة المكتوبة في حلقة المحرك قوائم كسولة حقيقية.
])

#exercise(label-name: <ex:lazy-list-printing>, [
عدّل حلقة المحرك للمُقَيِّم بحيث تُطبَع الأزواج والقوائم الكسولة بطريقة معقولة. (ماذا ستفعل بشأن القوائم اللانهائية؟) قد تحتاج أيضًا إلى تعديل تمثيل الأزواج الكسولة بحيث يستطيع المُقَيِّم التعرف عليها من أجل طباعتها.
])
