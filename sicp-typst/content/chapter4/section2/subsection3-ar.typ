...
الأزواج كدوال:#footnote[هذا هو التمثيل الدالي الموصوف في التمرين @ex:lambda-cons. وحقيقةً، سيفيد أي تمثيل دالي (مثل تنفيذ يمرر الرسائل). لاحظ أنه يمكننا تثبيت هذه التعاريف في المُقَيِّم الكسول ببساطة عن طريق كتابتها في حلقة المحرك. وإذا كنا قد ضمنّا في الأصل #py("pair") و#py("head") و#py("tail") كأوليات في البيئة العامة، فستُعاد إعادة تعريفها. (انظر أيضًا التمارين @ex:lazy-list-input و @ex:lazy-list-printing).]
