...
في هذا القسم سننفذ لغة ذات ترتيب عادي تكون متطابقة مع #en[Python] باستثناء أن الدوال المركبة غير صارمة في كل وسيط. وستظل الدوال الأولية صارمة. ليس من الصعب تعديل المُقَيِّم في القسم @sec:core-of-evaluator بحيث تتصرف اللغة التي يفسرها بهذه الطريقة. وتتركز معظم التغييرات المطلوبة حول تطبيق الدوال.
...
#idx("فرض", sub: "الـ thunk")
#idx("thunk", sub: "فرض")
#emph[الفرض] (#en[forcing]).#footnote[هذا مماثل لفرض الكائنات المؤجلة التي قُدِّمت في الفصل @chap:state لتمثيل التدفقات. الفارق الحاسم بين ما نفعله هنا وما فعلناه في الفصل @chap:state هو أننا نبني التأجيل والفرض داخل المُقَيِّم نفسه، وبذلك نجعل هذا موحدًا وتلقائيًا في جميع أنحاء اللغة.]
وعمومًا، تُفْرَض الـ #en[thunk] فقط عندما تكون هناك حاجة إلى قيمتها: عندما تُمرَّر إلى دالة أوليّة ستستخدم قيمة الـ #en[thunk]؛ وعندما تكون قيمة للمحمول في تعبير شرطي؛ وعندما تكون قيمة لتعبير دالة على وشك أن تُطبَّق كدالة.
أحد خيارات التصميم المتاحة لدينا هو ما إذا كنا سنقوم بـ
#idx("تخزين موقت", sub: "للـ thunk")
...
#subheading([تعديل المُقَيِّم])

الفارق الرئيسي بين المُقَيِّم الكسول والمُقَيِّم في القسم @sec:mc-eval هو في كيفية التعامل مع تطبيقات الدوال في #py("evaluate") و#py("apply").
...
هذا يشبه تقريبًا بند #py("is_application") لـ #py("evaluate") في القسم @sec:core-of-evaluator. ومع ذلك، بالنسبة للتقييم الكسول، فإننا نستدعي #py("apply") مع تعبيرات الوسائط، بدلاً من الوسائط الناتجة عن تقييمها. وبما أننا سنحتاج إلى البيئة لإنشاء الـ #en[thunks] إذا كان يجب تأجيل الوسائط، فيجب علينا تمرير البيئة أيضًا. وما زلنا نُقَيِّم تعبير الدالة، لأن #py("apply") تحتاج إلى الدالة الفعلية المراد تطبيقها للتوجيه بناءً على نوعها (أوليّة مقابل مركبة) وتطبيقها.
...
المكان الآخر الذي يجب أن نغير فيه المُقَيِّم هو في التعامل مع العبارات الشرطية، حيث يجب استخدام #py("actual_value") بدلاً من #py("evaluate") للحصول على قيمة تعبير المحمول قبل اختباره ما إذا كان صحيحًا أم خادعًا:
...
#idx("حلقة المحرك", sub: "في المقييم الكسول")
#py("driver_loop") (من القسم @sec:running-eval) لتستخدم #py("actual_value") بدلاً من #py("evaluate")، بحيث إذا تم إرجاع قيمة مؤجلة إلى حلقة القراءة والتقييم والطباعة، فسيتم فرضها قبل طباعتها. ونغير المحثات أيضًا للإشارة إلى أن هذا هو المُقَيِّم الكسول:

#idx("محثات", sub: "المقييم الكسول")#idx("driverloop", sub: "للمقييم الكسول", decl: true)
#snippet(```python
...
مع إجراء هذه التغييرات، يمكننا تشغيل المُقَيِّم واختباره. يوضح التقييم الناجح لتعبير #py("try_me") المناقش في القسم @sec:evaluation-order أن المفسر يؤدي تقييمًا كسولاً:
...
#idx("تجميع المهملات", sub: "التخزين الموقّت و")
#idx("تخزين موقت", sub: "تجميع المهملات و")
#emph[تجميع المهملات] (#en[garbage collection]) وإعادة تدوير مساحتها، كما سنناقش في القسم @sec:storage-allocation.

وبالمثل، كان بإمكاننا السماح للبيئات غير الضرورية في الكائنات المؤجلة المخزنة موقتًا من القسم @sec:delayed-lists بالخضوع لتجميع المهملات، عن طريق جعل #py("memo") تفعل شيئًا مثل #py("fun = None;") للتخلص من الدالة #py("fun") (والتي تتضمن البيئة التي تم فيها تقييم تعبير #en[lambda] الذي يشكل ذيل التدفق) بعد تخزين قيمتها.]

#idx("forceit", sub: "نسخة خزن موقت", decl: true)
#snippet(```python
...
لاحظ أن نفس الدالة #py("delay_it") تعمل سواء مع أو بدون التخزين الموقّت.#idx("thunk", sub: "تنفيذ الـ")
...
اعطِ الإجابات عندما يقوم المُقَيِّم بالتخزين الموقّت وعندما لا يقوم به.
])

#exercise(label-name: <ex:force-sequence>, [
يشعر #en[Cy D. Fect]، وهو مبرمج #en[C] سابق تعافى، بالقلق من أن بعض الآثار الجانبية قد لا تحدث أبدًا، لأن المُقَيِّم الكسول لا يفرض العبارات في التتابع. وبما أن قيمة عبارة ما في تتابع قد لا تُستخدم (فقد تكون العبارة موجودة فقط لغرض أثرها، مثل الإسناد لمتغير أو الطباعة)، فقد لا يكون هناك استخدام لاحق لهذه القيمة (مثل وسيط لدالة أوليّة) يتسبب في فرضها. لذلك يعتقد ساين أنه عند تقييم المتتاليات، يجب فرض جميع العبارات في التتابع. ويقترح تعديل #py("evaluate_sequence") من القسم @sec:core-of-evaluator لتستخدم #py("actual_value") بدلاً من #py("evaluate"):
...
يمكنك افتراض أن تصريح البارامترات هو دائمًا العبارة الأولى في متن تعريف الدالة، وإذا أُهمل، فإن جميع البارامترات تكون صارمة. وبالتالي، سيتيح تعريف الدالة العادي نفس السلوك مثل #en[Python] العادية، بينما ستؤدي إضافة تصريح #py("\"lazy_memo\"") لكل بارامتر من كل دالة مركبة إلى إنتاج سلوك المُقَيِّم الكسول المحدَّد في هذا القسم. صمم ونفذ التغييرات المطلوبة لإنتاج مثل هذا التوسيع لـ #en[Python]. ستتعامل دالة #py("parse") مع تصريحات البارامترات كتطبيقات دوال، لذا يلزمك تعديل #py("apply") للتوجيه إلى تنفيذك للشكل النحوي الجديد.
