...
#subsection([الدوال كطرق عامة], label-name: <sec:proc-general-methods>)

#idx("higher-order functions", sub: "function as general method")

قدّمنا الدوال المركبة في القسم @sec:compound-procedures كآلية لتجريد أنماط العمليات العددية لجعلها مستقلة عن الأعداد المحددة المعنية. ومع الدوال من رتبة أعلى، مثل دالة #py("integral") في القسم @sec:procedures-as-parameters، بدأنا نرى نوعاً أكثر قوة من التجريد: دوال تُستخدَم للتعبير عن طرق عامة للحساب، مستقلة عن الدوال المحددة المعنية. وفي هذا القسم نناقش مثالين أكثر إتقاناً — طرقًا عامة لإيجاد أصفار الدوال ونقاطها الثابتة — ونوضح كيف يمكن التعبير عن هذه الطرق مباشرة كدوال.

#subheading([إيجاد جذور المعادلات بطريقة تنصيف الفترات])

#idx("half-interval method")
#emph[طريقة تنصيف الفترات] هي تقنية بسيطة ولكنها قوية لإيجاد جذور معادلة $f(x)=0$، حيث $f$ دالة مستمرة. والفكرة هي أنه، إذا أُعطِينَا نقطتين $a$ و $b$ بحيث $f(a) < 0 < f(b)$، فيجب أن يكون لـ $f$ صفر واحد على الأقل بين $a$ و $b$. ولتحديد موقع الصفر، لتكن $x$ متوسط $a$ و $b$ ونحسب $f(x)$. وإذا كان $f(x) > 0$، فإنه يجب أن يكون لـ $f$ صفر بين $a$ و $x$. وإذا كان $f(x) < 0$، فإنه يجب أن يكون لـ $f$ صفر بين $x$ و $b$. وبالمتابعة بهذه الطريقة، يمكننا تحديد فترات أصغر فأصغر يجب أن يكون لـ $f$ صفر فيها. وعندما نصل إلى نقطة تكون فيها الفترة صغيرة بدرجة كافية، تنتهي العملية. وبما أنّ فترة عدم اليقين تُنصَّف في كل خطوة من العملية، فإنّ أقصى عدد من الخطوات المطلوبة ينمو ك$Theta ( log ( L/T))$، حيث $L$ هو طول الفترة الأصلية و $T$ هو التسامح مع الخطأ (أي حجم الفترة التي سنعتبرها «صغيرة بدرجة كافية»).
...
#py("error")،
والتي تأخذ كوسيط سلسلةً نصية تُطبَع كرسالة خطأ جنباً إلى جنب مع رقم سطر البرنامج الذي أدى إلى استدعاء #py("error").]
...
#emph[نقطة ثابتة] لدالة $f$ إذا حقق $x$ المعادلة $f(x)=x$. وبالنسبة لبعض الدوال $f$ يمكننا تحديد موقع نقطة ثابتة بالبدء بتخمين أولي وتطبيق $f$ مراراً وتكراراً:

$ f(x), space f(f(x)), space f(f(f(x))), space dots.h $

حتى لا تتغير القيمة كثيراً. وباستخدام هذه الفكرة، يمكننا ابتكار دالة #py("fixed_point") تأخذ كمدخلات دالة وتخميناً أولياً وتنتج تقريباً لنقطة ثابتة للدالة. ونطبق الدالة مراراً وتكراراً حتى نجد قيمتين متتاليتين يكون الفرق بينهما أقل من تسامح محدد:
...
حساب الجذر التربيعي كبحث عن نقطة ثابتة. يتطلب حساب الجذر التربيعي لعدد $x$ إيجاد $y$ بحيث $y^(2) = x$. وبوضع هذه المعادلة في الشكل المكافئ $y = x/y$، ندرك أننا نبحث عن نقطة ثابتة للدالة#footnote[$arrow.r.bar$
...
$y arrow.r.bar x/y$، وبالتالي يمكننا محاولة حساب الجذور التربيعية كالتالي:
...
وإحدى الطرق للتحكم في مثل هذه الذبذبات هي منع التخمينات من التغير كثيراً. وبما أنّ الإجابة تقع دائماً بين تخميننا $y$ و $x/y$، يمكننا إنشاء تخمين جديد ليس بعيداً عن $y$ مثل $x/y$ بأخذ متوسط $y$ مع $x/y$، بحيث يكون التخمين التالي بعد $y$ هو $frac(1, 2)(y+x/y)$ بدلاً من $x/y$. وعملية إنشاء مثل هذه المتتالية من التخمينات هي ببساطة عملية البحث عن نقطة ثابتة لـ $y arrow.r.bar frac(1, 2)(y+x/y)$:
...
(القسم @sec:tree-recursion) هي نقطة ثابتة للتحويل $x arrow.r.bar 1 + 1/x$، واستخدم هذه الحقيقة لحساب $phi$ بوساطة دالة #py("fixed_point").
])
...
ثم أوجد حلاً لـ $x^(x) = 1000$ بإيجاد نقطة ثابتة لـ $x arrow.r.bar log (1000)/ log (x)$.
...
+ افترض أنّ #py("n") و #py("d") دالتان بوسيط واحد (مؤشر الحد $i$) ترجعان $N_(i)$ و $D_(i)$ لحدود الكسر المستمر. عرِّف دالة #py("cont_frac") بحيث يقيّم #py("cont_frac(n, d, k)") قيمة الكسر المستمر المنتهي ذي الـ $k$ من الحدود. وافحص دالتك بتقريب $1/ phi$ باستخدام #snippet(```python print(cont_frac(lambda i: 1, lambda i: 1, k)) ```) لقيم متتالية لـ #py("k"). كم يجب أن تجعل قيمة #py("k") للحصول على تقريب دقيق حتى 4 منازل عشرية؟
+ إذا كانت دالة #py("cont_frac") لديك تُنشئ عملية عودية، فاكتب دالة تُنشئ عملية تكرارية. وإذا كانت تُنشئ عملية تكرارية، فاكتب دالة تُنشئ عملية عودية.
])
...
حيث $x$ بالراديان.
عرِّف دالة #py("tan_cf(x, k)") تحسب تقريباً لدالة الظل بناءً على صيغة لامبرت.
