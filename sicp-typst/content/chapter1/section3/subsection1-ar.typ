...
#subsection([الدوال كوسائط], label-name: <sec:procedures-as-parameters>)
...
للتعبير عن هذا المفهوم. وتكمن قوة ترميز سيجما في أنه يتيح لعلماء الرياضيات التعامل مع مفهوم الجمع ذاته بدلاً من التعامل فقط مع مجاميع محددة — على سبيل المثال، صياغة نتائج عامة حول المجاميع تكون مستقلة عن المتسلسلة المحددة الجاري جمعها.

وبالمثل، كمصممي برامج، نودّ أن تكون لغتنا قوية بدرجة كافية بحيث يمكننا كتابة دالة تعبر عن مفهوم الجمع ذاته بدلاً من كتابة دوال فقط تحسب مجاميع محددة. ويمكننا القيام بذلك بسهولة في لغتنا الوظيفية بأخذ القالب المشترك المعروض أعلاه وتحويل «الفراغات» إلى معلمات:
#idx("sum", decl: true)
#snippet(```python
...
لاحظ أنّ #py("sum") تأخذ كوسيطَيْن الحدين الأدنى والأعلى #py("a") و #py("b") جنباً إلى جنب مع الدالتين #py("term") و #py("next"). ويمكننا استخدام #py("sum") تماماً كما نستخدم أي دالة. على سبيل المثال، يمكننا استخدامها (جنباً إلى جنب مع دالة #py("inc") التي تزيد معطيتها بمقدار 1) لتعريف #py("sum_cubes"):
...
ويمكننا أيضاً تعريف #py("pi_sum") بنفس الطريقة:#footnote[لاحظ أننا استخدمنا البنية الكتلية (القسم @sec:black-box) لضم تعاريف #py("pi_next") و #py("pi_term") داخل #py("pi_sum")، نظرًا لأنه من غير المرجح أن تكون هذه الدوال مفيدة لأي غرض آخر. وسنرى كيفية التخلص منها تماماً في القسم @sec:lambda.]
...
الطريقة الموضحة أعلاه. باستخدام قاعدة سيمبسون، يُقرَّب تكامل دالة $f$ بين $a$ و $b$ كالتالي:
...
عرِّف دالة تأخذ كوسائط $f$ و $a$ و $b$ و $n$ وترجع قيمة التكامل المحسوب بقاعدة سيمبسون. واستخدم دالتك لمكاملة #py("cube") بين 0 و 1 (مع $n=100$ و $n=1000$)، وقارن النتائج بتلك الخاصة بدالة #py("integral") الموضحة أعلاه.
...
#py("sum")
أعلاه عودية خطية. ويمكن إعادة كتابة الدالة بحيث يُنفَّذ الجمع تكرارياً. أظهر كيفية القيام بذلك بملء التعبيرات المفقودة في التعريف التالي:
...
+ إذا كانت دالة #py("product") لديك تُنشئ عملية عودية، فاكتب دالة تُنشئ عملية تكرارية. وإذا كانت تُنشئ عملية تكرارية، فاكتب دالة تُنشئ عملية عودية.
])
...
+ إذا كانت دالة #py("accumulate") لديك تُنشئ عملية عودية، فاكتب دالة تُنشئ عملية تكرارية. وإذا كانت تُنشئ عملية تكرارية، فاكتب دالة تُنشئ عملية عودية.
])
...
يأخذ نفس وسائط #py("accumulate")، جنباً إلى جنب مع محمول إضافي ذي معطى واحد يحدد المرشِّح. اكتب #py("filtered_accumulate") كدالة. وأظهر كيفية التعبير عما يلي باستخدام #py("filtered_accumulate"):
