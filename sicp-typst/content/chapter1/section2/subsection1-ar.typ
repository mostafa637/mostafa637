...
#subsection([العودية والتكرار الخطّيّان], label-name: <sec:recursion-and-iteration>)
...
ومرة أخرى، يمكننا إعادة صياغة وصفنا كدالة
دالة
...
#sicp-figure(image("/images/img_javascript/ch1-Z-G-7.svg", width: 70%), caption: [عملية عودية خطية لحساب !6.], label-name: <fig:recursive-factorial-javascript>)
...
#emph[عملية عودية]. يتطلب إجراء هذه العملية أن يحتفظ المُفسِّر بمعلومات
...
في الحالة التكرارية، توفر متغيرات الحالة وصفاً كاملاً لحالة العملية عند أي نقطة. وإذا أوقفنا الحساب بين الخطوات، فإنّ كل ما نحتاج إلى القيام به لاستئناف الحساب هو تزويد المُفسِّر بقيم متغيرات الحالة الثلاثة. وليس الأمر كذلك مع العملية العودية. في هذه الحالة، هناك بعض المعلومات «الخفية» الإضافية، يحتفظ بها المُفسِّر ولا تحتويها متغيرات الحالة، والتي تشير إلى «مكان العملية» في تنفيذ سلسلة العمليات المؤجَّلة. وكلما طالت السلسلة، وجب الاحتفاظ بمعلومات أكثر.#footnote[حين نناقش تنفيذ الدوال على آلات السجلات في الفصل @chap:reg، سنرى أنّ أي عملية تكرارية يمكن تحقيقها «في العتاد» كآلة تحتوي على مجموعة ثابتة من السجلات وبدون ذاكرة مساعدة. وفي المقابل، فإنّ تحقيق عملية عودية يتطلب آلة تستخدم هيكل بيانات مساعداً يُعرَف بـ
#idx("stack")
#emph[المكدس (#en[stack])].]
#idx("substitution model of function application", sub: "shape of process")

عند المقارنة بين التكرار والعودية، يجب أن نكون حريصين على عدم الخلط بين مفهوم
#idx("recursive function", sub: "recursive process vs.")
#idx("recursive process", sub: "recursive function vs.")
#emph[العملية] العودية ومفهوم #emph[الدالة] العودية.
حين نحدد دالة بأنها عودية، فإننا نشير إلى حقيقة نحوية مفادها أنّ تعريف الدالة يشير (سواء بشكل مباشر أو غير مباشر) إلى الدالة نفسها. ولكن حين نحدد عملية بأنها تتبع نمطاً، ولنقل، عودياً خطياً، فإننا نتحدث عن كيفية تطور العملية، وليس عن بناء الجملة (النحو) لكتابة الدالة. قد يبدو من المربك أننا نشير إلى دالة عودية مثل #py("fact_iter") بأنها تُنشئ عملية تكرارية. ومع ذلك، فإنّ العملية تكرارية حقاً: حالتها ملتقطة بالكامل بواسطة متغيرات حالتها الثلاثة، ولا يحتاج المُفسِّر إلا لمتابعة ثلاثة أسماء فقط من أجل تنفيذ العملية.
...
مُصمَّمة بطريقة تجعل تفسير أي دالة عودية يستهلك مقداراً من الذاكرة ينمو مع عدد استدعاءات الدالة، حتى عندما تكون العملية الموصوفة، من حيث المبدأ، تكرارية.
...
$mono("for")$، و
$mono("while")$.
 وتنفيذ بايثون الذي سننظر فيه في الفصل @chap:reg لا يشترك في هذا العيب. فهو سينفذ العملية التكرارية في مساحة ثابتة، حتى لو وُصِفَت العملية التكرارية بواسطة دالة عودية.

ويُسمّى التنفيذ الذي يتمتع بهذه الخاصية
#idx("tail recursion")
#emph[ذيلِيّ العودية (#en[tail-recursive])].#footnote[عُرِفَت العودية الذيلية لفترة طويلة كحيلة لتحسين المترجمات. وقدم كارت هويت
#idx("Hewitt, Carl Eddie")
(1977) أساساً دلالياً متماسكاً للعودية الذيلية، وشرحها من حيث نموذج «تمرير الرسائل» للحساب الذي سنناقشه في الفصل @chap:state. وإلهاماً من هذا، قام جيرالد جاي سسمان و
#idx("Steele, Guy Lewis Jr.")
غاي لويس ستيل جونيور (انظر Steele 1975) بإنشاء مُفسِّر ذيلي العودية لـ #en[Scheme]. وأظهر ستيل لاحقاً كيف أنّ العودية الذيلية هي نتيجة للطريقة الطبيعية لتجميع استدعاءات الدوال
...
ذيلية العودية. لا يحدد مرجع لغة بايثون العودية الذيلية بطريقة أو بأخرى. في حين أنّ معظم تنفيذات بايثون ليست ذيلية العودية، فإننا نفترض في هذا الكتاب تنفيذاً يمتلك هذه الخاصية.]
مع التنفيذ ذيلي العودية،
#idx("iterative process", sub: "implemented by function call")
يمكن التعبير عن التكرار باستخدام آلية استدعاء الدوال العادية، بحيث تكون بنى التكرار الخاصة مفيدة فقط كالتالي:
...
#py("inc")، التي تزيد وسيطها بمقدار 1،
و#py("dec")، التي تنقص وسيطها بمقدار 1.
...
#py("plus(4, 5)").
هل هذه العمليات تكرارية أم عودية؟
])
...
#py("f")، و#py("g")، و#py("h") لقيم $n$ الصحيحة الموجبة. على سبيل المثال،
$k(n)$ تحسب
$5n^(2)$.
])
