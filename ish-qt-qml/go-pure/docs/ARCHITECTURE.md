...
هذا الفرع مخصص لإعادة كتابة iSH core في Go، وليس مجرد adapter لـC. أُنجزت طبقة التخزين، وfakefs فوق rootfs، وbootstrap metadata، وCoreSession المرحلي الذي يحافظ على عقد PTY، وحد FPU 80-bit المعزول داخل `internal/core/emu/fpu`. أضيفت الآن `internal/core/cpu` لصفحات الذاكرة وCOW وgrows-down وMachineState، وdecoder/executor x86 أولي يدعم ModRM/SIB وguest memory operands، و`internal/core/syscall` لـi386 syscall ABI الأولي، و`internal/core/kernel` لعملية Go تجمع الذاكرة والسجلات والsyscalls وfakefs. يدعم executor حاليًا مجموعة تعليمات أساسية تشمل MOV وALU وCMP/TEST وstack وcall/ret وrelative branches وINT 0x80 وHLT، مع load/store وLEA وindirect call/jump عبر الذاكرة، بينما ما زال ELF loader والتعليمات الأوسع وجدولة المهام وإشارات Linux خارج هذا الحد المرحلي؛ لذلك يستمر PTY المضيف كمسار نقل مؤقت في CoreSession. تبقى نسخة C في مجلد `upstream` مرجعًا للمقارنة والاختبارات، لكنها ليست اعتمادًا للبناء. لا يجوز إدخال CGo إلى `go-pure`; أما فرع Qt/QML فيبقى مستقلًا ولا يتأثر بهذا القرار.
