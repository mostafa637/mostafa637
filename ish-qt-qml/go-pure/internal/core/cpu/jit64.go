...
func compileInstruction64(inst x86asm.Inst, address uint64) (microOp64, bool, error) {
	...
	case x86asm.AESENC, x86asm.AESENCLAST, x86asm.AESDEC, x86asm.AESDECLAST:
		destination, err := operand64FromArg(arg(0), 16)
		if err != nil || destination.Kind != operand64XMM {
			return microOp64{}, false, fmt.Errorf("%s destination: %v", inst.Op, err)
		}
		source, err := operand64FromArg(arg(1), 16)
		if err != nil || (source.Kind != operand64XMM && source.Kind != operand64Mem) {
			return microOp64{}, false, fmt.Errorf("%s round key: %v", inst.Op, err)
		}
		return makeAESRound64(address, uint8(inst.Len), inst.Op, destination, source), false, nil
	case x86asm.AESIMC:
		destination, err := operand64FromArg(arg(0), 16)
		if err != nil || destination.Kind != operand64XMM {
			return microOp64{}, false, fmt.Errorf("%s destination: %v", inst.Op, err)
		...
		return makeAESIMC64(address, uint8(inst.Len), destination, source), false, nil
	case x86asm.AESKEYGENASSIST:
		...
		immediate, ok := arg(2).(x86asm.Imm)
		if !ok {
			return microOp64{}, false, fmt.Errorf("%s requires an immediate round constant", inst.Op)
		}
		return makeAESKeygenAssist64(address, uint8(inst.Len), destination, source, uint8(immediate)), false, nil
	case x86asm.SHA1NEXTE:
		destination, err := operand64FromArg(arg(0), 16)
		if err != nil || destination.Kind != operand64XMM {
			return microOp64{}, false, fmt.Errorf("%s destination: %v", inst.Op, err)
		}
		...
		return makeSHA1NextE64(address, uint8(inst.Len), destination, source), false, nil
	case x86asm.SHA1RNDS4:
		destination, err := operand64FromArg(arg(0), 16)
		if err != nil || destination.Kind != operand64XMM {
			return microOp64{}, false, fmt.Errorf("%s destination: %v", inst.Op, err)
		}
		...
		immediate, ok := arg(2).(x86asm.Imm)
		if !ok {
			return microOp64{}, false, fmt.Errorf("%s requires an immediate round selector", inst.Op)
		}
		return makeSHA1Rounds4_64(address, uint8(inst.Len), destination, source, uint8(immediate)), false, nil
...
func makeSHA1NextE64(address uint64, size uint8, destination, source operand64) microOp64 {
	return microOp64{Address: address, Size: size, Run: func(state *MachineState64, next uint64) (Flow64, error) {
		...
		result := src2
		putSHADword64(&result, 3, shaDword64(src2, 3)+sha1RotateLeft64(shaDword64(src1, 3), 30))
		if err := writeVector64(state, destination, next, result); err != nil {
			return Flow64Stop, err
		}
		state.RIP = next
...
func sha1RoundFunction64(b, c, d uint32, selector uint8) uint32 {
	switch selector & 3 {
	case 0:
		return (b & c) | (^b & d)
	case 1, 3:
...
func sha1RoundConstant64(selector uint8) uint32 {
	switch selector & 3 {
	case 0:
		return 0x5a827999
	case 1:
...
func makeSHA1Rounds4_64(address uint64, size uint8, destination, source operand64, selector uint8) microOp64 {
	return microOp64{Address: address, Size: size, Run: func(state *MachineState64, next uint64) (Flow64, error) {
		src1, err := readVector64(state, destination, next)
		if err != nil {
			return Flow64Stop, err
		...
		selector &= 3
		constant := sha1RoundConstant64(selector)
		a, b, c, d := shaDword64(src1, 3), shaDword64(src1, 2), shaDword64(src1, 1), shaDword64(src1, 0)
		words := [4]uint32{shaDword64(src2, 3), shaDword64(src2, 2), shaDword64(src2, 1), shaDword64(src2, 0)}
		var e uint32
		for round, word := range words {
			if round != 0 {
				word += e
			}
			t := sha1RoundFunction64(b, c, d, selector) + sha1RotateLeft64(a, 5) + word + constant
			a, b, c, d, e = t, a, sha1RotateLeft64(b, 30), c, d
		}
		var result [16]byte
		putSHADword64(&result, 3, a)
		putSHADword64(&result, 2, b)
		putSHADword64(&result, 1, c)
		putSHADword64(&result, 0, d)
		if err := writeVector64(state, destination, next, result); err != nil {
			return Flow64Stop, err
		}
		state.RIP = next
...
func aesGFMul64(left, right byte) byte {
	var result byte
	for right != 0 {
		if right&1 != 0 {
			result ^= left
...
func aesShiftRows64(input [16]byte, inverse bool) (output [16]byte) {
	for column := 0; column < 4; column++ {
		for row := 0; row < 4; row++ {
			sourceColumn := (column + row) & 3
			if inverse {
				sourceColumn = (column - row + 4) & 3
			}
			output[column*4+row] = input[sourceColumn*4+row]
		}
	}
	return output
}

func aesSubBytes64(input [16]byte, inverse bool) (output [16]byte) {
	for index, value := range input {
		if inverse {
			output[index] = aesInvSBox64[value]
		} else {
			output[index] = aesSBox64[value]
		}
	}
	return output
}

func aesMixColumns64(input [16]byte, inverse bool) (output [16]byte) {
	for column := 0; column < 4; column++ {
		base := column * 4
		a0, a1, a2, a3 := input[base], input[base+1], input[base+2], input[base+3]
		if inverse {
			output[base] = aesGFMul64(a0, 0x0e) ^ aesGFMul64(a1, 0x0b) ^ aesGFMul64(a2, 0x0d) ^ aesGFMul64(a3, 0x09)
			output[base+1] = aesGFMul64(a0, 0x09) ^ aesGFMul64(a1, 0x0e) ^ aesGFMul64(a2, 0x0b) ^ aesGFMul64(a3, 0x0d)
			output[base+2] = aesGFMul64(a0, 0x0d) ^ aesGFMul64(a1, 0x09) ^ aesGFMul64(a2, 0x0e) ^ aesGFMul64(a3, 0x0b)
			output[base+3] = aesGFMul64(a0, 0x0b) ^ aesGFMul64(a1, 0x0d) ^ aesGFMul64(a2, 0x09) ^ aesGFMul64(a3, 0x0e)
		} else {
			output[base] = aesGFMul64(a0, 0x02) ^ aesGFMul64(a1, 0x03) ^ a2 ^ a3
			output[base+1] = a0 ^ aesGFMul64(a1, 0x02) ^ aesGFMul64(a2, 0x03) ^ a3
			output[base+2] = a0 ^ a1 ^ aesGFMul64(a2, 0x02) ^ aesGFMul64(a3, 0x03)
			output[base+3] = aesGFMul64(a0, 0x03) ^ a1 ^ a2 ^ aesGFMul64(a3, 0x02)
		}
	}
	return output
}

func aesRound64(state, roundKey [16]byte, decrypt, last bool) (output [16]byte) {
	state = aesShiftRows64(state, decrypt)
	state = aesSubBytes64(state, decrypt)
	if !last {
		state = aesMixColumns64(state, decrypt)
	}
...
func makeAESRound64(address uint64, size uint8, op x86asm.Op, destination, source operand64) microOp64 {
	return microOp64{Address: address, Size: size, Run: func(state *MachineState64, next uint64) (Flow64, error) {
		data, err := readVector64(state, destination, next)
		if err != nil {
			return Flow64Stop, err
		}
		roundKey, err := readVector64(state, source, next)
		if err != nil {
			return Flow64Stop, err
		}
		decrypt := op == x86asm.AESDEC || op == x86asm.AESDECLAST
		last := op == x86asm.AESENCLAST || op == x86asm.AESDECLAST
		result := aesRound64(data, roundKey, decrypt, last)
		if err := writeVector64(state, destination, next, result); err != nil {
			return Flow64Stop, err
		}
		state.RIP = next
...
func makeAESIMC64(address uint64, size uint8, destination, source operand64) microOp64 {
	return microOp64{Address: address, Size: size, Run: func(state *MachineState64, next uint64) (Flow64, error) {
		value, err := readVector64(state, source, next)
		if err != nil {
			return Flow64Stop, err
		}
		result := aesMixColumns64(value, true)
		if err := writeVector64(state, destination, next, result); err != nil {
			return Flow64Stop, err
		}
		state.RIP = next
...
func aesSubWord64(value uint32) uint32 {
	return uint32(aesSBox64[byte(value)]) |
		uint32(aesSBox64[byte(value>>8)])<<8 |
		uint32(aesSBox64[byte(value>>16)])<<16 |
		uint32(aesSBox64[byte(value>>24)])<<24
}

func makeAESKeygenAssist64(address uint64, size uint8, destination, source operand64, immediate uint8) microOp64 {
	return microOp64{Address: address, Size: size, Run: func(state *MachineState64, next uint64) (Flow64, error) {
		value, err := readVector64(state, source, next)
		if err != nil {
			return Flow64Stop, err
		}
		x1 := shaDword64(value, 1)
		x3 := shaDword64(value, 3)
		sub1, sub3 := aesSubWord64(x1), aesSubWord64(x3)
		rcon := uint32(immediate)
		var result [16]byte
		putSHADword64(&result, 0, sub1)
		putSHADword64(&result, 1, bits.RotateLeft32(sub1, -8)^rcon)
