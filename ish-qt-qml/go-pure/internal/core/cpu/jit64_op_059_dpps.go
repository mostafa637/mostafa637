...
func jit64CompileCaseDPPS(ctx jit64CompileContext64) (microOp64, bool, error) {
	...
	return makeSSEDotProduct64(ctx.address, uint8(ctx.inst.Len), ctx.inst.Op, destination, source, uint8(immediate)), false, nil
}

func init() {
	jit64InstructionHandlers[x86asm.DPPS] = jit64CompileCaseDPPS
	jit64InstructionHandlers[x86asm.DPPD] = jit64CompileCaseDPPS
}
