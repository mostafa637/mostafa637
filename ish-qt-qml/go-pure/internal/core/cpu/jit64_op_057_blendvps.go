...
func jit64CompileCaseBLENDVPS(ctx jit64CompileContext64) (microOp64, bool, error) {
	...
	return makeSSEBlend64(ctx.address, uint8(ctx.inst.Len), ctx.inst.Op, destination, source, 0, true), false, nil
}

func init() {
	jit64InstructionHandlers[x86asm.BLENDVPS] = jit64CompileCaseBLENDVPS
	jit64InstructionHandlers[x86asm.BLENDVPD] = jit64CompileCaseBLENDVPS
}
